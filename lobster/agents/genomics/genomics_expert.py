...
from lobster.services.analysis.variant_annotation_service import VariantAnnotationService
...
def genomics_expert(
    ...
    if callback_handler and hasattr(llm, "with_config"):
        callbacks = (
            callback_handler if isinstance(callback_handler, list) else [callback_handler]
        )
    ...
    def load_plink(
        ...
        try:
            ...
            data_manager.log_tool_usage(
                ...
                description=f"Loaded PLINK file: {n_individuals} individuals × {n_snps} SNPs",
            )
...
"""
    ...
    def assess_quality(
        ...
        try:
            ...
            logger.info(
                f"Assessing quality for '{modality_name}': {adata.shape[0]} samples × {adata.shape[1]} variants"
            )
...
"""
    ...
    def run_gwas(
        ...
        try:
            ...
            # Get modality
            adata = data_manager.get_modality(modality_name)
            logger.info(f"Running GWAS on '{modality_name}': phenotype={phenotype}, model={model}")
            ...
            data_manager.modalities[gwas_modality_name] = adata_gwas
...
⚠️ **Population Stratification Detected**
...
**Recommendation**: Run calculate_pca("{modality_name}") and re-run GWAS with PC1-PC10 as covariates."""
            ...
            if stats['n_variants_significant'] > 0 and stats.get('top_variants'):
                ...
                for i, var in enumerate(stats['top_variants'][:5], 1):
                    response += f"\n{i}. {var['variant_id']}: β={var['beta']:.4f}, p={var['pvalue']:.2e}"
    ...
    def calculate_pca(
        ...
        try:
            ...
            # Get modality
            adata = data_manager.get_modality(modality_name)
            logger.info(f"Calculating PCA on '{modality_name}': n_components={n_components}")
            ...
            data_manager.modalities[pca_modality_name] = adata_pca
...
**PCA results stored in**: adata.obsm['X_pca']"""
    ...
    def annotate_variants(
        ...
        try:
            ...
            # Get modality
            adata = data_manager.get_modality(modality_name)
            logger.info(f"Annotating variants in '{modality_name}': source={annotation_source}")
            ...
            for mod_name in all_modalities:
                ...
                if adata.uns.get("data_type") == "genomics":
                    ...
                    genomics_modalities.append(
                        f"  - {mod_name} ({modality_type}): {n_obs:,} samples × {n_vars:,} variants"
                    )
...
- Data type: {data_type}
- Modality: {modality}
- Dimensions: {n_obs:,} samples × {n_vars:,} variants
...
"""
            ...
            if has_qc:
                ...
                n_variants_pass = adata.var["qc_pass"].sum() if "qc_pass" in adata.var.columns else 0
