...
def ms_proteomics_expert(
    ...
    if not isinstance(data_manager, DataManagerV2):
        raise ValueError(
            "MSProteomicsExpert requires DataManagerV2 for modular analysis"
        )
            ...
            if modality_name == "":
                ...
                for mod_name in ms_modalities:
                    ...
                    if "contaminant_proteins" in metrics:
                        response += (
                            f"  Contaminants: {metrics['contaminant_proteins']}\n"
                        )
    ...
    def assess_ms_proteomics_quality(
        ...
        try:
            ...
            if "n_peptides" in final_adata.var.columns:
                low_peptide_proteins = (
                    final_adata.var["n_peptides"] < min_peptides_per_protein
                ).sum()
            ...
            if combined_stats.get("samples_high_missing", 0) > 0:
                response += (
                    "- Consider filtering samples with excessive missing values\n"
                )
    ...
    def filter_ms_proteomics_data(
        ...
        try:
            ...
            if "n_peptides" in adata_filtered.var.columns:
                peptide_filter = (
                    adata_filtered.var["n_peptides"] >= min_peptides_per_protein
                )
...
- Fold change threshold: {fold_change_threshold}"""
    ...
    def add_peptide_mapping_to_ms_modality(
        modality_name: str, peptide_file_path: str, save_result: bool = True
    ) -> str:
        """
