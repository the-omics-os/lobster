...
def _format_data_preview(
    matrix, max_rows: int = 5, max_cols: int = 5
) -> Dict[str, Any]:
    """
...
def modality_describe(
    client: "AgentClient", output: OutputAdapter, modality_name: Optional[str] = None
) -> Optional[str]:
    """
    ...
    try:
        ...
        if matrix_info["sparse"]:
            ...
            basic_table_data["rows"].append(
                ["Non-zero", f"{matrix_info['nnz']:,} elements"]
            )
        else:
        ...
        if (
            ...
            if "geo_accession" in metadata:
                meta_table_data["rows"].append(
                    ["GEO Accession", metadata["geo_accession"]]
                )
