        ...
        if hasattr(client, "data_manager") and hasattr(
            ...
            if has_data:
                ...
                if console is not None:
                    ...
                    # Show summary
                    output.print("[green]✓ Export complete:[/green]", style="success")
            ...
            output.print(
                "[yellow]Exporting session data and plots...[/yellow]", style="info"
            )
            export_path = client.export_session()
            output.print(
                f"[green]✓ Session exported to:[/green] [grey74]{export_path}[/grey74]",
...
def plot_show(
    client: "AgentClient", output: OutputAdapter, plot_identifier: Optional[str] = None
) -> Optional[str]:
    """
    ...
    try:
        ...
        if not plot_identifier:
            ...
            if not plots_dir.exists():
                ...
                if client.data_manager.latest_plots:
                    saved_files, _, _ = (
                        client.data_manager.plot_manager.save_plots_to_workspace()
                    )
                    if saved_files:
                        output.print(
            ...
            if success:
                ...
                output.print(
                    f"[red]Failed to open plots directory: {message}[/red]",
                    style="error",
                )
                output.print(
                    f"[grey50]Plots directory: {plots_dir}[/grey50]", style="info"
                )
        ...
        if found_plot:
            ...
            for file_path in saved_files:
                ...
                if file_path_obj.parent == plots_dir and file_path_obj.stem.startswith(
                    plot_id
                ):
            ...
            if matching_file and matching_file.exists():
                ...
                if success:
                    ...
                    output.print(
                        f"[red]Failed to open plot: {message}[/red]", style="error"
                    )
                    output.print(
                        f"[grey50]Plot file: {matching_file}[/grey50]", style="info"
                    )
            ...
            output.print(f"[red]Plot not found: {plot_identifier}[/red]", style="error")
            output.print(
