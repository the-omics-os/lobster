...
class LobsterPaths:
    ...
    def get_global_config_dir() -> Path:
        ...
        if platform.system() == "Windows":
            ...
            return Path(os.environ.get("XDG_CONFIG_HOME", Path.home() / ".config")) / "lobster"
...
def _verify_lobster_directory(path: Path, markers: List[str]) -> Tuple[bool, Optional[str]]:
    """
...
def purge(
    ...
    if not verified_targets:
        output.print("[yellow]Found directories but none verified as Lobster AI:[/yellow]")
    ...
    # Summary
    output.print("\n[dim]" + "─" * 60 + "[/dim]")
    output.print(f"[bold]Total to remove:[/bold] {_format_size(total_size)} across {total_files} files")
