...
class CustomAgentConfig:
    ...
    model_reference: Optional[str] = (
        None  # Agent name to copy config from (e.g., "research_agent")
    )
...
class LobsterAgentConfigurator:
    ...
    # Pre-defined testing profiles (all official agents)
    ...
    TESTING_PROFILES = {
        ...
        "performance": {
            ...
            "data_expert_assistant": "claude-4-5-sonnet",
            # Transcriptomics
        ...
        try:
            ...
            for agent_name, custom_config in custom_configs.items():
                try:
                    ...
                    ref_or_preset = (
                        custom_config.model_reference or custom_config.model_preset
                    )
                    logger.debug(
    ...
    def print_current_config(self, show_all: bool = False):
        ...
        if filtered_count > 0 and not show_all:
            ...
            print(
                "   Tip: Use 'lobster config show-config --show-all' to see all configured agents"
            )
