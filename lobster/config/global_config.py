...
if platform.system() == "Windows":
    ...
    CONFIG_DIR = (
        Path(os.environ.get("XDG_CONFIG_HOME", Path.home() / ".config")) / "lobster"
    )
...
class GlobalProviderConfig(ProviderConfigBase):
    ...
    default_profile: str = Field(
        "production", description="Default agent configuration profile"
    )

    # Per-provider default models
    anthropic_default_model: Optional[str] = Field(
        None, description="Default Anthropic model (e.g., 'claude-sonnet-4-20250514')"
    )
    ...
    ollama_default_model: Optional[str] = Field(
        None, description="Default Ollama model (e.g., 'llama3:70b-instruct')"
    )

    gemini_default_model: Optional[str] = Field(
        None, description="Default Gemini model (e.g., 'gemini-3-pro-preview')"
    )
    ...
    def load(cls) -> "GlobalProviderConfig":
        ...
        if not config_path.exists():
            logger.debug(f"No global config found at {config_path}, using defaults")
            return cls()
