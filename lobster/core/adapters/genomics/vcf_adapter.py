...
class VCFAdapter(BaseAdapter):
    ...
    def from_source(
        ...
        try:
            ...
            for variant in variant_iter:
                ...
                if filter_pass and variant.FILTER is not None and variant.FILTER != "PASS":
                    continue
            ...
            X = np.vstack(genotypes).T  # Transpose: (n_variants, n_samples) → (n_samples, n_variants)
            ...
            adata.layers["GT"] = (
                X.toarray() if hasattr(X, "toarray") else X.copy()
            )
    ...
    def validate(
        self, adata: anndata.AnnData, strict: bool = None
    ) -> ValidationResult:
        """
    ...
    def get_quality_metrics(self, adata: anndata.AnnData) -> Dict[str, Any]:
        ...
        if "FILTER" in adata.var.columns:
            ...
            metrics["pass_rate"] = float((filter_vals == "PASS").sum() / len(filter_vals))
