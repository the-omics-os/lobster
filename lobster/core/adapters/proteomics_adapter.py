...
class ProteomicsAdapter(BaseAdapter):
    ...
    def __init__(
        ...
        if data_type not in ["mass_spectrometry", "affinity"]:
            raise ValueError(
                f"Unknown data_type: {data_type}. Must be 'mass_spectrometry' or 'affinity'"
            )
    ...
    def from_source(
        ...
        try:
            ...
            self.logger.info(
                f"Loaded proteomics data: {adata.n_obs} obs × {adata.n_vars} vars"
            )
            return adata
    ...
    def _drop_high_missing_features(
        ...
        if vars_keep.sum() < adata.n_vars:
            ...
            self.logger.info(
                f"Removed {(~vars_keep).sum()} proteins with >{max_missing_vars*100}% missing values"
            )
    ...
    def _flag_contaminants_and_reverse(self, adata: anndata.AnnData) -> anndata.AnnData:
        ...
        if protein_ids is not None:
            ...
            if "is_contaminant" not in adata.var.columns:
                ...
                is_contaminant = protein_ids.str.contains(
                    "|".join(contaminant_patterns), case=False, na=False
                )
            ...
            if "is_reverse" not in adata.var.columns:
                ...
                is_reverse = protein_ids.str.contains(
                    "|".join(reverse_patterns), case=False, na=False
                )
    ...
    def get_quality_metrics(self, adata: anndata.AnnData) -> Dict[str, Any]:
        ...
        if "n_proteins" in adata.obs.columns:
            ...
            metrics["median_proteins_per_sample"] = float(
                adata.obs["n_proteins"].median()
            )
        ...
        if "is_contaminant" in adata.var.columns:
            metrics["contaminant_proteins"] = int((adata.var["is_contaminant"] == True).sum())
            metrics["contaminant_percentage"] = float(
                ((adata.var["is_contaminant"] == True).sum() / len(adata.var)) * 100
            )

        if "is_reverse" in adata.var.columns:
            metrics["reverse_hits"] = int((adata.var["is_reverse"] == True).sum())

        return metrics

    def add_peptide_mapping(
        self, adata: anndata.AnnData, peptide_data: Union[pd.DataFrame, str, Path]
    ) -> anndata.AnnData:
        """
        ...
        try:
            ...
            if "protein_id" in peptide_df.columns:
                    ...
                    .agg(
                        {
                            ...
                            "is_unique": lambda x: (
                                x.sum() if "is_unique" in peptide_df.columns else None
                            ),
                            "sequence_coverage": (
                                "first"
                                if "sequence_coverage" in peptide_df.columns
                                else None
                            ),
                        }
                    )
                ...
                for idx, row in peptide_groups.iterrows():
                    ...
                    if mask.any():
                        ...
                        adata.var.loc[mask, "n_unique_peptides"] = row[
                            "n_unique_peptides"
                        ]
                        ...
                        if row["sequence_coverage"] is not None:
                            ...
                            adata.var.loc[mask, "sequence_coverage"] = row[
                                "sequence_coverage"
                            ]
            ...
            self.logger.info(
                f"Added peptide mapping: {len(peptide_df)} peptides for {adata.uns['peptide_to_protein']['n_proteins']} proteins"
            )
    ...
    def detect_data_type(self, adata: anndata.AnnData) -> str:
        ...
        ms_score = sum(
            1 for indicator in ms_indicators if indicator in adata.var.columns
        )

        # Check for affinity-specific metadata
        affinity_indicators = ["antibody_id", "antibody_clone", "array_type"]
        affinity_score = sum(
            1 for indicator in affinity_indicators if indicator in adata.var.columns
        )
