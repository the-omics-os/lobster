...
# Lazy scanpy import for optional 10X matrix loading
_scanpy_module = None


def _ensure_scanpy():
    """Lazily import scanpy with helpful error message."""
    global _scanpy_module
    if _scanpy_module is None:
        try:
            import scanpy as sc
...
class TranscriptomicsAdapter(BaseAdapter):
    ...
    def _load_h5_transcriptomics_data(
        ...
        try:
            # Try to load as H5AD format first (anndata native)
    ...
    def _load_mtx_data(self, path: Union[str, Path], **kwargs) -> anndata.AnnData:
        ...
        # Load using scanpy (lazy import - only needed for 10X format)
        try:
            sc = _ensure_scanpy()
