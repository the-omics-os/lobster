...
class ConfigResolver:
    ...
    @classmethod
    def get_instance(
        cls, workspace_path: Optional[Path] = None, force_reload: bool = False
    ) -> "ConfigResolver":
        """
        ...
        norm_cached = (
            cls._instance_workspace.resolve() if cls._instance_workspace else None
        )
    ...
    def resolve_provider(
        ...
        if env_provider:
            if env_provider in VALID_PROVIDERS:
                return (
                    env_provider,
                    f"environment variable {LOBSTER_LLM_PROVIDER_ENV}",
                )
            else:
