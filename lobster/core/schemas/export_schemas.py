...
**Use case**: Microbiome harmonization workflows
"""
...
class ExportSchemaRegistry:
    ...
    **Example Schema**:
    ...
    def get_sra_amplicon_export_schema() -> Dict[str, Any]:
        """
        Export schema for SRA 16S/ITS amplicon data (microbiome use case).
        ...
        return {
            ...
            "priority_groups": {
                ...
                ExportPriority.HARMONIZED_METADATA: [
                    # Harmonized fields (standardized by metadata_assistant)
...
def get_ordered_export_columns(
    ...
    **Algorithm**:
    ...
    Args:
        ...
        data_type: Modality type for schema lookup
            Default: "sra_amplicon" (microbiome use case)
    ...
    **Fallback behavior**:
    ...
    # Final dedup pass (defensive programming)
...
def infer_data_type(samples: List[Dict[str, Any]]) -> str:
    ...
    **Examples**:
    ...
    # Default fallback: SRA amplicon (microbiome use case)
    return "sra_amplicon"
...
def harmonize_column_names(
    samples: List[Dict[str, Any]], track_provenance: bool = True
    ...
    for idx, sample in enumerate(samples):
        sample_id = (
            sample.get("run_accession") or sample.get("biosample") or f"sample_{idx}"
        )
        ...
        # Fix for CSV alignment bug (Jan 2026):
        ...
        # Fix for array serialization bug (Jan 2026):
    ...
    for sample in samples:
        for col, value in sample.items():
            ...
            if (
                value is not None
                and str(value).strip()
                and str(value).lower() not in ("nan", "na", "none", "")
            ):
                col_counts[col] += 1
...
def remove_sparse_columns(
    ...
    filtered = [{k: v for k, v in s.items() if k not in sparse_cols} for s in samples]

    return filtered, sparse_cols


def remove_constant_columns(
    samples: List[Dict[str, Any]], protected_columns: Optional[Set[str]] = None
) -> Tuple[List[Dict[str, Any]], Set[str]]:
    """
    ...
    for col in all_cols:
        try:
            ...
            normalized = {
                v if v is not None and str(v).strip() else None for v in values
            }
    ...
    filtered = [{k: v for k, v in s.items() if k not in constant_cols} for s in samples]
...
def harmonize_disease_field(
    ...
    disease_flag_cols = [
        c
        for c in df.columns
        if c.lower().endswith("_disease") or c.lower().endswith("disease")
    ]
    disease_flag_cols = [
        c
        for c in disease_flag_cols
        if c.lower() not in ("disease", "host_disease", "host_disease_stat")
    ]
    ...
    if track_provenance:
        ...
        df = df.drop(
            columns=["_disease_strategy", "_disease_source_col"], errors="ignore"
        )
...
def harmonize_samples_for_export(
    ...
    result, col_prov_log = harmonize_column_names(
        samples, track_provenance=track_provenance
    )
    all_provenance_logs.extend(col_prov_log)

    # Step 2: Extract disease
    result, disease_prov_log = harmonize_disease_field(
        result, track_provenance=track_provenance
    )
    ...
    if remove_sparse:
        result, sparse_removed = remove_sparse_columns(
            result, threshold=sparse_threshold
        )
        stats["sparse_removed"] = sorted(
            list(sparse_removed)
    ...
    if remove_constant:
        ...
        stats["constant_removed"] = sorted(
            list(constant_removed)
        )  # Convert set to list for JSON
