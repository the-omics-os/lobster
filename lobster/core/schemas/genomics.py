...
class GenomicsSchema:
    ...
    def create_validator(
        ...
        validator.add_custom_rule("check_chromosome_format", _validate_chromosome_format)
        validator.add_custom_rule("check_genotype_data", _validate_genotype_data)
        validator.add_custom_rule("check_variant_positions", _validate_variant_positions)

        if schema_type == "wgs":
            validator.add_custom_rule("check_vcf_fields", _validate_vcf_fields)
            validator.add_custom_rule("check_allele_frequency", _validate_allele_frequency)
        elif schema_type == "snp_array":
            validator.add_custom_rule("check_plink_fields", _validate_plink_fields)
            validator.add_custom_rule("check_maf_distribution", _validate_maf_distribution)
...
def _validate_genotype_data(adata) -> "ValidationResult":
    ...
    if hasattr(gt, "min") and hasattr(gt, "max"):
        ...
        if total_count > 0:
            ...
            if missing_rate > 0.1:
                result.add_warning(
                    f"High missing genotype rate: {missing_rate:.2%}"
                )
            else:
                result.add_info(
                    f"Missing genotype rate: {missing_rate:.2%}"
                )
...
def _validate_variant_positions(adata) -> "ValidationResult":
    ...
    if "POS" in adata.var.columns:
        ...
        if hasattr(positions, "min"):
            ...
            if min_pos < 0:
                result.add_error(
                    f"Invalid bp_position values found (min: {min_pos})"
                )
...
def _validate_allele_frequency(adata) -> "ValidationResult":
    ...
    if "AF" in adata.var.columns:
        ...
        if hasattr(af, "min") and hasattr(af, "max"):
            ...
            if very_rare > 0:
                result.add_info(
                    f"{very_rare} very rare variants (AF < 0.001)"
                )
...
def _validate_plink_fields(adata) -> "ValidationResult":
    ...
    fam_fields = ["individual_id", "family_id", "father_id", "mother_id", "sex", "phenotype"]
    present_fam_fields = [f for f in fam_fields if f in adata.obs.columns]

    if present_fam_fields:
        result.add_info(
            f"Found PLINK .fam fields: {present_fam_fields}"
        )

    # Check for typical PLINK .bim fields in var
    bim_fields = ["chromosome", "snp_id", "cm_position", "bp_position", "allele_1", "allele_2"]
    present_bim_fields = [f for f in bim_fields if f in adata.var.columns]

    if present_bim_fields:
        result.add_info(
            f"Found PLINK .bim fields: {present_bim_fields}"
        )
