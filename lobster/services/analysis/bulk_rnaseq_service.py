...
Cross-package dependency:
    This service imports DifferentialFormulaService from lobster-transcriptomics.
    The import uses PEP 420 namespace merging - differential_formula_service is
    located in the lobster-transcriptomics package but importable via the unified
    lobster.services.analysis namespace.

    Required package: lobster-transcriptomics
    Install with: pip install lobster-transcriptomics
"""
...
Next suggested step: Import quantification data with tximport for differential expression analysis."""
    ...
    def merge_salmon_results(
        ...
        if (salmon_dir / "quant.sf").exists() or (
    ...
    def _detect_quantification_tool(self, directory: Path) -> str:
        ...
        if (
            ...
            kallisto_count += 1
            logger.info(
                f"Detected single Kallisto sample in root directory: {directory}"
            )
    ...
    def _create_de_ir(
        ...
        # Ensure string types for Jinja2 template rendering
        group1 = str(group1)
        group2 = str(group2)
...
adata_de.uns['de_results_{{ group1 }}_vs_{{ group2 }}'] = results_df.to_dict()
"""
            library = "scipy"
            imports = [
                "import numpy as np",
