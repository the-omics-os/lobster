...
adata_modules, _, _ = service.identify_modules(
    ...
    def pick_soft_threshold(
        ...
        try:
            ...
            for result in power_results:
                ...
                if (result['r_squared'] > r_squared_cutoff and
                    result['slope'] < 0 and
            ...
            ir = self._create_ir_pick_soft_threshold(
                powers, r_squared_cutoff, mean_connectivity_cutoff,
                n_top_variable, correlation_method
            )
    ...
    def identify_modules(
        ...
        try:
            ...
            adata_modules.uns["wgcna"] = {
                "modules": {
                    color: [proteins_used[i] for i, c in enumerate(module_colors) if c == color]
            ...
            analysis_stats = {
                ...
                "module_sizes": {k: v for k, v in module_sizes_final.items() if k != GREY_MODULE},
                "module_colors": list(set(module_colors) - {GREY_MODULE}),
    ...
    def correlate_modules_with_traits(
        ...
        try:
            ...
            for result in correlation_results:
                corr_matrix.loc[result["module"], result["trait"]] = result["correlation"]
            ...
            # Statistics
            significant_results = [r for r in correlation_results if r.get("significant", False)]
            ...
            analysis_stats = {
                ...
                "significant_pairs": [
                    {"module": r["module"], "trait": r["trait"], "correlation": r["correlation"]}
            ...
            # Create IR
            ir = self._create_ir_correlate_modules_with_traits(traits, correlation_method)
    ...
    def calculate_module_membership(
        ...
        try:
                ...
                for i, protein in enumerate(adata_kme.var_names):
                    ...
                    if module == GREY_MODULE:
                        ...
                        if kme_col in adata_kme.var.columns:
                            ...
                            is_hub.append(adata_kme.var.loc[protein, kme_col] >= threshold)
                        else:
                            is_hub.append(False)
            ...
            # Statistics
            n_hubs = sum(adata_kme.var.get("is_hub", [])) if "is_hub" in adata_kme.var else 0
            ...
            raise ProteomicsNetworkError(f"Module membership calculation failed: {str(e)}")
