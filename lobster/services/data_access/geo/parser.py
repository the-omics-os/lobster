...
class GEOParser:
    ...
    def parse_expression_file(self, file_path: Path) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            # Early detection of unsupported R serialized format
            if str(file_path).endswith(".rds") or str(file_path).endswith(".rds.gz"):
                logger.warning(
            ...
            if n_cells and n_genes:
                ...
                if not mem_check["can_load"]:
                    ...
                    try:
                        ...
                        logger.info(f"Chunked reading successful for {n_cells:,} cells")
                        return result
            ...
            try:
                ...
                if use_chunks:
                    ...
                    df = pd.read_csv(
                        ...
                        # NOTE: Do NOT use dtype_backend="pyarrow" here.
                        # PyArrow-backed strings (ArrowStringArray) are incompatible
                        # with anndata's HDF5 writer, causing serialization failures
                        # on every H5AD save. Standard numpy/object dtypes work reliably.
                    )
    ...
    def parse_10x_data(
        ...
        try:
            ...
            logger.debug("Reading 10X mtx with scanpy (sparse format)...")
        ...
        try:
            ...
            with h5py.File(file_path, "r") as f:
                ...
                if obs_names is None:
                    ...
                    logger.warning("No cell names found, using generic indices")
                ...
                if var_names is None:
                    ...
                    logger.warning("No gene names found, using generic indices")
