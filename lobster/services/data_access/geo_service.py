...
class GEOService:
    ...
    # ██         (delegated to geo.helpers._is_data_valid)                         ██
    # ██                                                                            ██
    # ████████████████████████████████████████████████████████████████████████████████
    ...
    # ██         (delegated to geo.helpers._retry_with_backoff)                    ██
    # ██                                                                            ██
    # ████████████████████████████████████████████████████████████████████████████████
            ...
            if not adapter:
                ...
                try:
                    ...
                    if (
                        ...
                        and clean_geo_id in self.data_manager.metadata_store
                    ):
                        metadata_for_detection = self.data_manager.metadata_store[
                            clean_geo_id
                        ].get("metadata", {})
                    ...
                    if modality_result and modality_result.modality == "bulk_rna":
                        ...
                        # scRNA-seq datasets often have few GSMs (e.g., 40) but
                        # hundreds of thousands of cells per GSM.
                        if n_obs > 10000:
                            adapter_name = "transcriptomics_single_cell"
                            logger.warning(
            ...
            if self._is_data_valid(data):
                ...
                try:
                    ...
                    if modality_result and modality_result.modality == "bulk_rna":
                        ...
                        # Fallback heuristic based on sample count + cell count
                        n_samples = len(gse.gsms) if hasattr(gse, "gsms") else 0
...
The actual expression data download will be much faster now that metadata is prepared."""
    ...
    def _process_tar_file(
        ...
        try:
            ...
            if nested_archives:
                ...
                estimated_sparse_mb = (
                    n_samples * 50
                )  # Conservative estimate (50MB per sample)
                try:
                    import psutil

                    available_mb = psutil.virtual_memory().available / 1024 / 1024
                    if (
                        estimated_sparse_mb > available_mb * 0.5
                    ):  # Would use >50% available RAM
                        logger.warning(
                ...
                for i, archive_path in enumerate(nested_archives, start=1):
                    try:
                        ...
                        if adata_sample is not None:
                            ...
                            logger.warning(
                                f"⚠️  Sample {i}/{n_samples} returned None (skipping)"
                            )
    ...
    def _detect_features_format(self, features_path: Path) -> str:
        ...
        Delegates to TenXGenomicsLoader.detect_features_format().
        ...
        """
        return self.tenx_loader.detect_features_format(features_path)
    ...
    def _load_10x_manual(
        ...
        Delegates to TenXGenomicsLoader.load_10x_manual().
        ...
        """
        return self.tenx_loader.load_10x_manual(temp_dir, features_format, gse_id)
    ...
    def _try_series_level_10x_trio(
        ...
        return self.tenx_loader.try_series_level_10x_trio(suppl_files, gse_id)
            ...
            for gsm_id, matrix in validated_matrices.items():
                try:
                    ...
                    adata = self.data_manager.load_modality(
                        ...
                        ),  # SOFT format requires transpose for bulk
                        **enhanced_metadata,
                    )
        ...
        try:
            ...
            try:
                ...
                if gse_id:
                    ...
                    metadata_for_detection = self.data_manager.metadata_store.get(
                        gse_id, {}
                    )
                else:
                    # Use sample metadata
