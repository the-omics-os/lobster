    ...
    def _detect_format(self, df: pd.DataFrame) -> str:
        ...
        if has_sample_col and has_pg_columns:
            ...
            if "PG.ProteinGroups" in columns:
                ...
                if n_unique_proteins < len(df) * 0.8:  # Significantly fewer unique proteins
...
'''
        ...
        return AnalysisStep(
            ...
            parameters={"file_path": str(file_path), "format_type": format_type, **parameters},
            ...
            output_entities=[
                {"name": "adata", "type": "AnnData", "description": "Parsed proteomics data"},
    ...
    def _parse_long_format(
        ...
        if "PG.Qvalue" in df.columns:
            ...
            df = df[(df["PG.Qvalue"] <= qvalue_threshold) & (df["PG.Qvalue"] >= 0)].copy()
    ...
    def _detect_protein_column(self, df: pd.DataFrame) -> str:
        ...
        for col in ["PG.ProteinGroups", "ProteinGroups", "Protein.Groups", "ProteinGroup"]:
            if col in df.columns:
                return col
