...
class DatasetContent(BaseModel):
    ...
    platform_id: Optional[Union[str, List[str]]] = Field(
        None,
        description="Platform ID(s) - can be single string or list for multi-platform datasets (e.g., 'GPL570' or ['GPL570', 'GPL96'])",
    )
...
class WorkspaceContentService:
    ...
    **Storage Structure:**
    ...
    def list_download_queue_entries(
        ...
        """
        try:
            if not self.data_manager or not hasattr(
                self.data_manager, "download_queue"
            ):
                return []
            ...
            if entries is None:
                logger.warning(
                    "download_queue.list_entries() returned None (expected list)"
                )
                return []
    ...
    def list_publication_queue_entries(
        ...
        try:
            if not self.data_manager or not hasattr(
                self.data_manager, "publication_queue"
            ):
                return []
            ...
            try:
                ...
                logger.warning(
                    "Publication queue schema not available (premium feature)"
                )
                return []
            ...
            entries = self.data_manager.publication_queue.list_entries(
                status=status_enum
            )
            ...
            if entries is None:
                logger.warning(
                    "publication_queue.list_entries() returned None (expected list)"
                )
                return []
            ...
            logger.error(
                f"Failed to list publication queue entries: {e}", exc_info=True
            )
            return []  # Graceful degradation
    ...
    def list_export_files(
        self, pattern: str = "*", category: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        """
    ...
    def _categorize_export_file(self, file_path: Path) -> str:
        ...
        if any(k in name_lower for k in ["sample", "metadata", "filtered"]):
            ...
            k in name_lower for k in ["result", "analysis", "de_genes", "differential"]
        ):
            return "results"
        elif any(
            k in name_lower for k in ["plot", "figure", "visualization", "umap", "pca"]
        ):
            return "plots"
        else:
            return "custom"
    ...
    def get_all_metadata_sources(self) -> Dict[str, List[Dict]]:
        ...
        for key, value in metadata_store.items():
            sources["in_memory"].append(
                {"identifier": key, "type": "memory", "data": value}
            )
        ...
        if exports_dir.exists():
            for export_file in exports_dir.iterdir():
                if export_file.is_file() and export_file.suffix in {
                    ".csv",
                    ".tsv",
                    ".xlsx",
                }:
