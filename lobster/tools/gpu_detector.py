...
def format_installation_message(availability_info: Dict[str, any]) -> str:
    ...
    if availability_info["ready_for_scvi"]:
        hardware_rec = availability_info.get("hardware_recommendation", {})
        device = hardware_rec.get("device", "cpu")
        info = hardware_rec.get("info", "Unknown hardware")
    ...
    message += f"💡 Recommended installation: {hardware_rec.get('command', '')}\n"
    message += f"🔧 Hardware detected: {hardware_rec.get('info', 'Unknown hardware')}"
