...
class ProteomicsDifferentialService:
    ...
    def perform_differential_expression(
        ...
        try:
            logger.info(
                f"Starting differential expression analysis with method: {test_method}"
            )
            ...
            if group_column not in adata_de.obs.columns:
                raise ProteomicsDifferentialError(
            ...
            volcano_data = self._generate_volcano_plot_data(
                de_results, fdr_threshold, fold_change_threshold
            )
            ...
            raise ProteomicsDifferentialError(
                f"Differential expression analysis failed: {str(e)}"
            )
    ...
    def perform_time_course_analysis(
        ...
        try:
            ...
            # Filter significant results
            significant_tc = [
                r for r in tc_results if r.get("p_adjusted", 1.0) < fdr_threshold
            ]
            ...
            tc_stats = {
                ...
                "significance_rate": (
                    len(significant_tc) / len(tc_results) if tc_results else 0.0
                ),
                "fdr_threshold": fdr_threshold,
    ...
    def perform_correlation_analysis(
        ...
        try:
            logger.info(
                f"Starting correlation analysis with method: {correlation_method}"
            )
            ...
            # Apply FDR correction
            correlation_results = self._apply_fdr_correction(
                correlation_results, "benjamini_hochberg"
            )
            ...
            significant_corr = [
                ...
                if r.get("p_adjusted", 1.0) < fdr_threshold
                and abs(r.get("correlation", 0)) >= min_correlation
            ]
            ...
            corr_stats = {
                ...
                "target_column": target_column,
                "target_range": (
                    float(target_values.min()),
                    float(target_values.max()),
                ),
                "n_tests_performed": len(correlation_results),
                "n_significant_results": len(significant_corr),
                "significance_rate": (
                    len(significant_corr) / len(correlation_results)
                    if correlation_results
                    else 0.0
                ),
                "median_abs_correlation": float(
                    np.median([abs(r["correlation"]) for r in correlation_results])
                ),
                "max_abs_correlation": float(
                    max([abs(r["correlation"]) for r in correlation_results])
                ),
                "fdr_threshold": fdr_threshold,
                "min_correlation": min_correlation,
    ...
    def _perform_de_analysis(
        ...
        for group1, group2 in comparison_pairs:
            ...
            if (
                group1_data.shape[0] < min_samples_per_group
                or group2_data.shape[0] < min_samples_per_group
            ):
    ...
    def _perform_statistical_test(
        self, group1: np.ndarray, group2: np.ndarray, method: str
    ...
    def _filter_significant_results(
        ...
        for result in results:
            is_significant = result.get("p_adjusted", 1.0) < fdr_threshold and abs(
                result.get("log2_fold_change", 0)
            ) > np.log2(fold_change_threshold)
    ...
    def _add_significance_flags(
        self, adata: anndata.AnnData, significant_results: List[Dict[str, Any]]
    ):
        ...
        for group in groups:
            ...
            for i, protein_name in enumerate(protein_names):
                ...
                if test_method == "linear_trend":
                    slope, p_value, r_squared = self._linear_trend_test(
                        valid_time, valid_protein
                    )
                    test_statistic = slope
                elif test_method == "polynomial":
                    test_statistic, p_value, r_squared = self._polynomial_trend_test(
                        valid_time, valid_protein
                    )
                else:
