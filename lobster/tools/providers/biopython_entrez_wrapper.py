...
def _configure_ssl_context() -> None:
    ...
    if settings.SSL_CERT_PATH:
        ...
        if os.path.exists(settings.SSL_CERT_PATH):
            logger.info(
                f"Using custom SSL certificate bundle: {settings.SSL_CERT_PATH}"
            )
...
class BioPythonEntrezWrapper:
    ...
    def esearch(
        ...
        **kwargs,
        ...
        try:
            ...
            if _is_ssl_error(e):
                logger.error(
                    f"Bio.Entrez.esearch SSL error: {e}{_get_ssl_error_guidance()}"
                )
            else:
    ...
    def esummary(self, db: str, id: str, **kwargs) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if _is_ssl_error(e):
                logger.error(
                    f"Bio.Entrez.esummary SSL error: {e}{_get_ssl_error_guidance()}"
                )
            else:
    ...
    def efetch(
        ...
        try:
            ...
            if _is_ssl_error(e):
                logger.error(
                    f"Bio.Entrez.efetch SSL error: {e}{_get_ssl_error_guidance()}"
                )
            else:
    ...
    def elink(self, dbfrom: str, db: str, id: str, **kwargs) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if _is_ssl_error(e):
                logger.error(
                    f"Bio.Entrez.elink SSL error: {e}{_get_ssl_error_guidance()}"
                )
            else:
