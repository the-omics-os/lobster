...
class GEOProvider(BasePublicationProvider):
                ...
                if "ERROR" in esearch_result:
                    ...
                    if (
                        ...
                        if retry < self.config.max_retry:
                            ...
                            logger.error(
                                f"NCBI API error after {self.config.max_retry + 1} attempts: {error_msg}"
                            )
                            raise ValueError(
    ...
    def get_download_urls(self, geo_id: str) -> "DownloadUrlResult":
        ...
        try:
            ...
            pre_download_soft_file(geo_id, self.cache_dir)
    ...
    def format_geo_search_results(
        ...
        if not search_result.summaries:
            ...
            response += (
                "- Use filters to narrow down results (organism, date range, etc.)\n"
            )
            return response
        else:
