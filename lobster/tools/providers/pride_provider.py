...
class PRIDEProvider(BasePublicationProvider):
    ...
    def __init__(
        ...
        logger.debug(
            f"Initialized PRIDE provider with base URL: {self.config.base_url}"
        )
        ...
        **kwargs,
        ...
        try:
            ...
            if isinstance(response_data, list):
                ...
                logger.debug(
                    f"API returned dict with _embedded: {len(projects)} projects"
                )
            else:
            ...
            for i, project in enumerate(projects[:10], 1):
                ...
                logger.debug(
                    f"  organisms type: {type(organisms)}, length: {len(organisms) if isinstance(organisms, list) else 'N/A'}"
                )
                ...
                organism_str = (
                    ", ".join(organism_names) if organism_names else "Unknown"
                )
