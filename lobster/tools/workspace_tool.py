...
from lobster.core.queue_storage import atomic_write_jsonl
        ...
        try:
            ...
            def _adapt_general_content(
                ...
                if filter_pattern:
                    ...
                    logger.debug(
                        f"Pattern '{filter_pattern}' matched {len(items)} items in '{ws}' workspace"
                    )
            ...
            def _adapt_download_queue(
                ...
                if entries is None:
                    logger.warning(
                        "list_download_queue_entries returned None (service bug)"
                    )
                    return []
            ...
            def _adapt_publication_queue(
                ...
                if entries is None:
                    logger.warning(
                        "list_publication_queue_entries returned None (service bug)"
                    )
                    return []
            ...
            def _adapt_exports(
                ...
                for file_info in export_files:
                    ...
                    size_str = (
                        f"{size_mb:.2f} MB" if size_mb > 0.01 else f"{file_size} bytes"
                    )
            ...
            def _render_queue_metadata_hybrid(
                ...
                if queue_type == "publication":
                    ...
                    priority_medium = sum(
                        1 for e in entries if 4 <= e.get("priority", 5) <= 7
                    )
            ...
            WORKSPACE_ADAPTERS: Dict[
                str, Callable[[Optional[str], Optional[str]], List[WorkspaceItem]]
            ] = {
                ...
                "exports": lambda s, p: _adapt_exports(
                    s, p
                ),  # Uses pattern for file filtering
                "download_queue": lambda s, p: _adapt_download_queue(
                    s
                ),  # Queues ignore pattern
                "publication_queue": lambda s, p: _adapt_publication_queue(
                    s
                ),  # Queues ignore pattern
            }
            ...
            if identifier is None:
                ...
                if workspace == "publication_queue" and level == "summary":
                    ...
                    if level == "summary":
                        ...
                        # Build response
                        response = "## Publication Queue Summary\n\n"
                ...
                logger.info(
                    f"Listing workspace content (workspace={workspace}, level={level}, filter={status_filter})"
                )

                # Determine which workspaces to list
                workspaces_to_list = (
                    [workspace] if workspace else list(WORKSPACE_ADAPTERS.keys())
                )
                ...
                if level == "metadata":
                    ...
                    if workspace == "publication_queue":
                        ...
                        if entries is None or not isinstance(entries, list):
                            logger.error(
                                f"Invalid entries type: {type(entries)} (expected list)"
                            )
            ...
            if workspace == "exports" and identifier:
                ...
                if not matching_files:
                    ...
                    available = [f.name for f in exports_dir.iterdir() if f.is_file()][
                        :10
                    ]
            ...
            if output_format == "csv" and isinstance(content_data, dict):
                if "samples" in content_data and isinstance(
                    ...
                    if use_rich_format or export_mode == "strict":
                        ...
                        harmonized_samples, harmonization_stats, provenance_log = (
                            harmonize_samples_for_export(
                                enriched_samples,
                                remove_sparse=True,
                        ...
                        rich_filename = (
                            f"{base_filename}_rich_{timestamp}.csv"
                            if timestamp
                            else f"{base_filename}_rich.csv"
                        )
                        ...
                        strict_filename = (
                            f"{base_filename}_strict_{timestamp}.csv"
                            if timestamp
                            else f"{base_filename}_strict.csv"
                        )
                        strict_file_path = exports_dir / strict_filename
                        df_strict.to_csv(
                            strict_file_path, index=False, encoding="utf-8"
                        )
                        logger.info(
                        ...
                        if provenance_log:
                            prov_filename = (
                                f"{base_filename}_harmonization_log_{timestamp}.tsv"
                                if timestamp
                                else f"{base_filename}_harmonization_log.tsv"
                            )
                            prov_log_path = exports_dir / prov_filename
                            prov_df = pd.DataFrame(provenance_log)
                            prov_df.to_csv(
                                prov_log_path, sep="\t", index=False, encoding="utf-8"
                            )
                            logger.info(
                        ...
                        if identifier in data_manager.metadata_store:
                            ...
                            update_dict = {
                                ...
                                "harmonization_stats": make_hashable(
                                    harmonization_stats
                                ),
                            }
                            if prov_log_path:
                                update_dict["provenance_log_path"] = str(prov_log_path)
                                update_dict["provenance_log_filename"] = (
                                    prov_log_path.name
                                )
                            data_manager.metadata_store[identifier].update(update_dict)
                            logger.info(
                                f"Updated metadata_store['{identifier}'] with dual export paths + stats + provenance"
                            )
...
"""
                        ...
                        if identifier in data_manager.metadata_store:
                            data_manager.metadata_store[identifier][
                                "csv_export_path"
                            ] = str(cache_file_path)
                            data_manager.metadata_store[identifier][
                                "csv_export_filename"
                            ...
                            logger.info(
                                f"Updated metadata_store['{identifier}'] with csv_export_path"
                            )
                else:
                ...
                if target_content_type == ContentType.PUBLICATION:
                    ...
                    content_model = DatasetContent(
                        ...
                        sample_count=metadata.get(
                            "n_samples", metadata.get("sample_count", 0)
                        ),
        ...
        try:
            ...
            if not confirmation:
                ...
                if deletion_summary["items_found"] == 0:
                    ...
                    response += "**To execute deletion**: Set `confirmation=True`\n"
            ...
            # Execution mode - perform actual deletion
                ...
                response = (
                    f"## Deleting '{identifier}' from workspace '{workspace}'\n\n"
                )
                    ...
                    try:
                        if item["type"] == "modality":
                            ...
                            if success:
                                ...
                                response += (
                                    f"✓ Deleted modality: {item['identifier']}\n"
                                )
                            else:
                            ...
                            if queue_file.exists():
                                ...
                                deletion_summary["items_deleted"] += 1
                                response += (
                                    f"✓ Deleted queue entry: {item['identifier']}\n"
                                )
                            else:
                                deletion_summary["errors"].append(
