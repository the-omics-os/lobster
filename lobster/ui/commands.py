...
class LobsterCommands(Provider):
    ...
    async def discover(self) -> Hits:
        ...
        # Data commands
        yield DiscoveryHit(
            "Data: Show loaded modalities", self._cmd_data, "View current datasets"
        )
        yield DiscoveryHit(
            "Data: Show dataset metadata",
            self._cmd_dataset_metadata,
            "View AnnData metadata",
        )
        yield DiscoveryHit(
            "Files: List workspace files", self._cmd_files, "Show workspace contents"
        )
        yield DiscoveryHit(
            "Files: Show directory tree", self._cmd_tree, "Tree view of workspace"
        )

        # Metadata commands
        yield DiscoveryHit(
            "Metadata: Overview",
            self._cmd_metadata_overview,
            "Smart overview with stats",
        )
        yield DiscoveryHit(
            "Metadata: Publications",
            self._cmd_metadata_publications,
            "Publication queue breakdown",
        )
        yield DiscoveryHit(
            "Metadata: Samples", self._cmd_metadata_samples, "Sample statistics"
        )
        yield DiscoveryHit(
            "Metadata: Workspace", self._cmd_metadata_workspace, "File inventory"
        )
        yield DiscoveryHit(
            "Metadata: Exports", self._cmd_metadata_exports, "Export files"
        )

        # Plot commands
        yield DiscoveryHit(
            "Plots: List generated plots", self._cmd_plots, "View all plots"
        )
        yield DiscoveryHit(
            "Plots: Open plots folder", self._cmd_open_plots, "Open in file manager"
        )

        # Queue commands
        yield DiscoveryHit(
            "Queue: Show download status",
            self._cmd_queue_downloads,
            "Download queue status",
        )
        yield DiscoveryHit(
            "Queue: Show publication status",
            self._cmd_queue_publications,
            "Publication queue status",
        )

        # Pipeline commands
        yield DiscoveryHit(
            "Pipeline: Export notebook",
            self._cmd_pipeline_export,
            "Export as Jupyter notebook",
        )
        yield DiscoveryHit(
            "Pipeline: List notebooks", self._cmd_pipeline_list, "Show saved notebooks"
        )

        # System commands
        yield DiscoveryHit(
            "Session: Show session status", self._cmd_session, "Current session info"
        )
        yield DiscoveryHit(
            "Status: Show installation status",
            self._cmd_status,
            "Tier, packages, and agents",
        )
        yield DiscoveryHit(
            "Tokens: Show token usage", self._cmd_tokens, "Token usage and costs"
        )
        yield DiscoveryHit(
            "Workspace: Show info", self._cmd_workspace, "Workspace details"
        )

        # Actions
        yield DiscoveryHit("Action: Save session", self._cmd_save, "Save current state")
        yield DiscoveryHit(
            "Action: Clear results", self._cmd_clear, "Clear results display"
        )
        yield DiscoveryHit(
            "Action: Refresh all", self._cmd_refresh, "Refresh all panels"
        )
    ...
    async def _cmd_status(self) -> None:
        ...
        for pkg_name, version in packages.items():
            status_icon = (
                "✓"
                if version not in ["missing", "dev"]
                else ("✗" if version == "missing" else "⚡")
            )
