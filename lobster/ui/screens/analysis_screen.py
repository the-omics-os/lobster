...
class AnalysisScreen(Screen):
    ...
    def _load_file_from_workspace(self, filepath: str) -> None:
        ...
        try:
            ...
            # Load the file
            logger.info(
                f"Loading {file_path.name} as '{modality_name}' using adapter '{adapter}'"
            )
            ...
            results.append_system_message(
                f"❌ Failed to load {Path(filepath).name}: {str(e)[:100]}"
            )
...
- Source: {entitlement.get("source", "default")}"""
            ...
            results.append_system_message(
                f"Plots refreshed. {plot_count} plots available."
            )
            ...
            saved, _, _ = (
                self.client.data_manager.plot_manager.save_plots_to_workspace()
            )
            if saved:
            ...
            try:
                if not subcommand or subcommand == "status":
                    ...
                    if not arg:
                        ...
                        queue_load_file(
                            self.client, arg, output, current_directory=None
                        )
        ...
        # Metadata commands (shared implementation with CLI)
            ...
            try:
                if subcommand == "clear":
                    ...
                    metadata_publications(
                        self.client, output, status_filter=status_filter
                    )
        ...
        try:
            ...
            for event in self.client.query(query, stream=True):
                ...
                if event_type == "stream":
                    ...
                    self.app.call_from_thread(self._handle_query_error, error, query)

        except ConnectionError as e:
            self.app.call_from_thread(self._handle_connection_error, str(e), query)
        except TimeoutError as e:
            self.app.call_from_thread(
                self._handle_connection_error, f"Request timed out: {e}", query
            )
        except Exception as e:
            self.app.call_from_thread(self._handle_query_error, str(e), query)
        finally:
    ...
    def _handle_connection_error(self, error_msg: str, original_query: str) -> None:
        ...
        if self.error_service:
            ...
            def retry_query():
                # Re-submit the query
                self.app.call_from_thread(self._retry_last_query)
