...
def get_file_info(filepath: str) -> Dict[str, Any]:
    ...
    icon = (
        format_info.get("icon", ICONS["unsupported"])
        if supported
        else ICONS["unsupported"]
    )
...
def is_10x_folder(path: Path) -> bool:
    ...
    gz_files = {
        f.name.replace(".gz", "") for f in path.iterdir() if f.name.endswith(".gz")
    }
...
class WorkspaceView(ListView):
        ...
        for item in self.workspace_path.iterdir():
            if item.is_file():
                ...
                file_info = {
                    ...
                    "name": item.name,
                    "ext": "",
                    "size_mb": sum(f.stat().st_size for f in item.rglob("*"))
                    / (1024 * 1024),
                    "icon": ICONS["folder"],
                    "supported": True,
                    "adapter": "10x",
    ...
    def on_list_view_selected(self, event: ListView.Selected) -> None:
        ...
        if isinstance(event.item, WorkspaceFileItem):
            ...
            if (
                file_info.get("supported", False)
                and event.item.state == FileState.WORKSPACE
            ):
                self.post_message(
                    FileLoadRequested(
                        filepath=event.item.filepath, filename=event.item.filename
                    )
                )
...
class DataHub(Static, can_focus=True):
    ...
    def on_paste(self, event: events.Paste) -> None:
        ...
        if filepaths:
            ...
            self.notify(
                f"No valid files found in paste: {text[:50]}", severity="warning"
            )
                ...
                if path.is_file():
                    ...
                    if is_10x_folder(path):
                        ...
                        for item in path.rglob("*"):
                            if (
                                item.is_file()
                                and item.suffix.lower() in SUPPORTED_FORMATS
                            ):
    ...
    def _handle_file_drop(self, filepaths: List[str]) -> None:
        ...
        for filepath in filepaths:
            ...
            try:
                ...
                if self._workspace_view and self._workspace_view.add_file(
                    str(dest_path)
                ):
                    added_count += 1
        ...
        if added_count > 0:
            self.notify(
                f"Added {added_count} file(s) to workspace", severity="information"
            )
        if error_count > 0:
        ...
        self.post_message(
            FileDropped(
                filepaths=[
                    str(self.workspace_path / Path(f).name)
                    for f in filepaths[:added_count]
                ],
