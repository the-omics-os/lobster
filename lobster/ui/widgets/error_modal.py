            ...
            if ctx.suggestions:
                with Vertical(classes="suggestions"):
                    ...
                    for i, suggestion in enumerate(ctx.suggestions, 1):
                        yield Static(f"  {i}. {suggestion}", classes="suggestion-item")

            # Buttons
            with Horizontal(classes="button-row"):
                if ctx.can_retry:
                    yield Button(
                        "Retry", variant="warning", id="retry", classes="retry"
                    )
                if ctx.can_dismiss:
                    yield Button(
                        "Dismiss", variant="primary", id="dismiss", classes="dismiss"
                    )
...
class ConnectionErrorModal(ErrorModal):
    ...
    def __init__(self, error_message: str, provider: str = "Unknown", **kwargs):
...
class AgentErrorModal(ErrorModal):
    ...
    def __init__(self, error_message: str, agent_name: str = "Agent", **kwargs):
        context = ErrorContext(
            title="Agent Error",
...
class DataErrorModal(ErrorModal):
    ...
    def __init__(self, error_message: str, data_source: str = "Data", **kwargs):
        context = ErrorContext(
            title="Data Error",
