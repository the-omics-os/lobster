...
class TerminalCallbackHandler(BaseCallbackHandler):
    ...
    def on_tool_end(self, output: str, **kwargs) -> None:
        ...
        if tool_name.startswith("handoff_to_") or tool_name.startswith(
            "transfer_back_to_"
        ):
            return
...
class SimpleTerminalCallback(TerminalCallbackHandler):
    ...
    def __init__(
        self, console: Console, show_reasoning: bool = True, minimal: bool = False
    ):
        super().__init__(
            console=console,
...
class TokenTrackingCallback(BaseCallbackHandler):
    ...
    def _update_current_agent(
        self, detected_agent: Optional[str], run_id: Optional[str] = None
    ) -> None:
        """
    ...
    def on_chat_model_start(
        ...
        **kwargs,
        ...
        if os.getenv("LOBSTER_LOG_LEVEL") == "DEBUG":
            ...
            logger.debug(
                f"on_chat_model_start: tags={tags}, metadata={metadata}, run_name={kwargs.get('run_name')}"
            )
