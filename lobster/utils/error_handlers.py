...
class StructuredErrorParser:
    ...
    def parse(self, error: Union[Exception, str]) -> ParsedError:
        ...
        error_code = self._map_http_to_error_code(
            http_status, error_str, structured_data
        )
    ...
    def _extract_rpc_types(
        self, structured_data: Optional[Dict[str, Any]]
    ) -> List[str]:
    ...
    def _extract_rate_limit_info(
        ...
        if not structured_data:
            ...
            match = re.search(
                r"retry\s*(?:in|after)\s*(\d+(?:\.\d+)?)\s*s", error_str, re.I
            )
            if match:
...
class AnthropicErrorParser(ProviderErrorParser):
    ...
    def can_parse(self, error_str: str) -> bool:
        ...
        return (
            ...
            and any(
                p in error_lower for p in ["rate_limit", "429", "too many requests"]
            )
        ) or "exceeded your organization" in error_lower

    def parse(self, error_str: str) -> RateLimitInfo:
        retry_delay = self._extract_number(
            r"retry.?after['\"]?\s*[:=]\s*(\d+)", error_str, default=60
        )
        ...
        for parser in self._parsers:
            try:
                if parser.can_parse(error_str):
                    logger.debug(
                        f"Using {parser.display_name} parser for rate limit error"
                    )
    ...
    def _create_provider_guidance(
        ...
        if info.additional_info:
            if "usage_monitor" in info.additional_info:
                solutions.append(
                    f"Monitor usage: {info.additional_info['usage_monitor']}"
                )
...
class ContextLimitErrorHandler(ErrorHandler):
    ...
    def _build_solutions(
        self, provider: str, context_info: Dict[str, Any]
    ) -> List[str]:
...
def is_retryable_error(
    error: Exception, config: Optional[LLMRetryConfig] = None
) -> bool:
    """
