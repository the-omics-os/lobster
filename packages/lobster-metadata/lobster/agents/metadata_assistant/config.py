...
def suppress_logs(min_level: int = logging.CRITICAL + 1):
    ...
    for logger_name in list(logging.Logger.manager.loggerDict.keys()):
        if logger_name.startswith("lobster.") or logger_name.startswith(
            "lobster_custom_"
        ):
...
def convert_list_to_dict(samples_list: list, key_field: str = "run_accession") -> dict:
    ...
    return {s.get(key_field, f"sample_{i}"): s for i, s in enumerate(samples_list)}
        ...
        for sample in samples:
            ...
            for col_name, col_value in sample.items():
                ...
                if matches:
                    ...
                    enriched_count += 1
                    log.append(
                        f"  - Sample {sample.get('run_accession', 'unknown')}: "
    ...
    try:
        response = llm.invoke(
            [
                {
                    "role": "system",
                    "content": "You are a biomedical information extraction expert. Return valid JSON only.",
                },
        ...
        return {
            "disease": "unknown",
            "confidence": 0.0,
            "evidence": f"Extraction error: {str(e)}",
            "source": source,
        }


def phase2_llm_abstract_extraction(
    samples: List[Dict], data_manager: Any, confidence_threshold: float, llm: Any
) -> tuple:
    """
    ...
    log.append(
        f"  - Grouped samples by publication: {len(pub_groups)} publications to process"
    )
    ...
    for pub_id, pub_samples in pub_groups.items():
        ...
        metadata_path = (
            data_manager.workspace_path / "metadata" / f"{pub_id}_metadata.json"
        )

        if not metadata_path.exists():
            log.append(
                f"  - {pub_id}: No cached metadata (skipping {len(pub_samples)} samples)"
            )
            continue
        ...
        if not abstract_text:
            log.append(
                f"  - {pub_id}: Empty abstract (skipping {len(pub_samples)} samples)"
            )
            continue
        ...
        if disease_info["confidence"] < confidence_threshold:
            log.append(
                f"  - {pub_id}: Low confidence {disease_info['confidence']:.2f} "
        ...
        log.append(
            f"  - {pub_id}: Extracted '{disease_info['disease']}' "
...
def phase3_llm_methods_extraction(
    ...
    for pub_id, pub_samples in pub_groups.items():
        # Load cached methods
        methods_path = (
            data_manager.workspace_path / "metadata" / f"{pub_id}_methods.json"
        )
        ...
        if disease_info["confidence"] < confidence_threshold:
            log.append(
                f"  - {pub_id}: Low confidence {disease_info['confidence']:.2f} (skipping)"
            )
            continue
        ...
        log.append(
            f"  - {pub_id}: Extracted '{disease_info['disease']}' from methods "
            f"(confidence: {disease_info['confidence']:.2f}, "
            f"enriched {len(pub_samples)} samples)"
        )
...
def phase4_manual_mappings(
    samples: List[Dict], manual_mappings: Dict[str, str]
) -> tuple:
    """
    ...
    for sample in samples:
        ...
        if pub_id in manual_mappings:
            ...
            if disease not in ["crc", "uc", "cd", "healthy", "unknown"]:
                log.append(
                    f"  - Warning: Invalid disease '{disease}' for {pub_id}, skipping"
                )
                continue
