...
class ClinicalMetadataService:
    ...
    def __init__(
        self, data_manager: DataManagerV2, cycle_length_days: int = 21
    ) -> None:
    ...
    def create_responder_groups(
        ...
        **DCR (Disease Control Rate)** - for immunotherapy trials:
        ...
        if grouping_strategy == "orr":
            stats = {
                ...
                "non_responder_percentage": (
                    round(len(groups["non_responder"]) / total * 100, 1)
                    if total > 0
                    else 0
                ),
            }
            ...
            stats = {
                ...
                "disease_control_rate": (
                    round(len(groups["disease_control"]) / total * 100, 1)
                    if total > 0
                    else 0
                ),
                "progression_rate": (
                    round(len(groups["progressive"]) / total * 100, 1)
                    if total > 0
                    else 0
                ),
            }
        ...
        logger.info(
            f"Created groups ({grouping_strategy}): {summary}, {groups['unknown']} unknown"
        )
