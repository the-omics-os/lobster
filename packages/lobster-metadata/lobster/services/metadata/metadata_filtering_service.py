...
# Disease source columns for fallback chain
# Priority order: most specific → least specific
DISEASE_SOURCE_COLUMNS = [
    "disease",  # Internal field (if already populated)
    "clinical condition",  # Common field in microbiome studies
    "ibd",  # 24.3% fill
...
def extract_disease_with_fallback(
    ...
    Designed for microbiome harmonization workflows.
...
class MetadataFilteringService:
    ...
    def __init__(
        ...
        disease_extractor: Optional[
            Callable[[pd.DataFrame, Optional[Dict]], Optional[str]]
        ] = None,
    ):
        """
    ...
    def parse_criteria(self, criteria: str) -> Dict[str, Any]:
        ...
        region_match = re.search(
            r"\b(V\d+(?:-V\d+)?|full-length)\b", criteria_lower, re.IGNORECASE
        )
        if region_match:
        ...
        if (
            "gut_mucosal_biopsy" in criteria_lower
            or "gut mucosal" in criteria_lower
            or "mucosal biopsy" in criteria_lower
        ):
        ...
        # Always enable disease extraction for microbiome studies
    ...
    def apply_filters(
        ...
        if not self.is_available:
            ...
            return (
                samples,
                {"skipped": True, "reason": "services_unavailable"},
                self._create_ir(
        ...
        if parsed_criteria.get("check_16s") or parsed_criteria.get("check_shotgun"):
            ...
            filter_steps.append(
                f"sequencing({seq_stats['retained']}/{seq_stats['input']})"
            )
        ...
        if parsed_criteria.get("check_16s") and parsed_criteria.get("amplicon_region"):
            ...
            allowed_regions = [
                parsed_criteria["amplicon_region"]
            ]  # Single region required
            failed_samples = []
            passed_samples = []

            for sample in filtered:
                result_dict, validation_stats, _ = (
                    self.microbiome_service.validate_amplicon_region(
                        sample,
                ...
                if validation_stats["is_valid"]:
                    ...
                    sample["amplicon_region_detected"] = validation_stats.get(
                        "detected_region"
                    )
                    sample["amplicon_region_confidence"] = validation_stats.get(
                        "confidence", 0.0
                    )
    ...
    def _apply_disease_filter(
        ...
        if disease_col:
            ...
            mapped_samples = total_samples - mapping_stats.get(
                "unmapped", total_samples
            )
            ...
            return filtered, {
                "applied": True,
                "rate": stats.get("standardization_rate", 0),
            }
    ...
    def _apply_sequencing_filter(
        ...
        for s in samples:
            ...
            if check_16s:
                is_16s = self.microbiome_service.validate_16s_amplicon(s, strict=False)[
                    0
                ]

            if check_shotgun:
                is_shotgun = self.microbiome_service.validate_shotgun(s, strict=False)[
                    0
                ]
        ...
        logger.debug(
            f"After sequencing filter: {len(sequencing_filtered)} samples retained"
        )
    ...
    def _create_ir(
        ...
        return AnalysisStep(
            ...
            parameters={
                ...
                "standardize_disease": parsed_criteria.get(
                    "standardize_disease", False
                ),
            },
            parameter_schema={
                "check_16s": {
                    "type": "boolean",
                    "description": "Filter for 16S amplicon data",
                },
                "check_shotgun": {
                    "type": "boolean",
                    "description": "Filter for shotgun/WGS data",
                },
                "amplicon_region": {
                    "type": "string",
                    "description": "Required amplicon region (V4, V3-V4, etc.)",
                },
                "host_organisms": {
                    "type": "array",
                    "items": {"type": "string"},
                    "description": "Allowed host organisms",
                },
                "sample_types": {
                    "type": "array",
                    "items": {"type": "string"},
                    "description": "Allowed sample types",
                },
                "standardize_disease": {
                    "type": "boolean",
                    "description": "Apply disease term standardization",
                },
            },
