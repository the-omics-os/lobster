...
class TestMetadataAssistantFallback:
    """Test metadata_assistant.phase1_column_rescan() fallback."""

    def test_fallback_to_hardcoded_keywords(self):
        """Test that phase1_column_rescan works when DiseaseOntologyService is unavailable."""
        # Simulate public version
        with patch.dict(
            sys.modules, {"lobster.services.metadata.disease_ontology_service": None}
        ):
            # Force reimport of the package and all sub-modules that cache
            # HAS_ONTOLOGY_SERVICE so the fallback path is exercised
            ...
            enriched_count, log = phase1_column_rescan(samples)
