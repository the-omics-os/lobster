...
class ProteomicsSurvivalService:
    ...
    def _fit_single_protein_cox(
        ...
        df_cox["protein"] = (df_cox["protein"] - df_cox["protein"].mean()) / df_cox[
            "protein"
        ].std()
        ...
        try:
            ...
            hr_ci_lower = float(
                np.exp(cph.confidence_intervals_.loc["protein", "95% lower-bound"])
            )
            hr_ci_upper = float(
                np.exp(cph.confidence_intervals_.loc["protein", "95% upper-bound"])
            )
...
print(f"Median survival by group: {stats['median_survival_by_group']}")""",
    ...
    def perform_cox_regression(
        ...
        try:
            ...
            if n_jobs > 1:
                ...
                logger.info(
                    f"Running Cox regression on {n_proteins} proteins with {n_jobs} parallel jobs..."
                )
                ...
                with ThreadPoolExecutor(max_workers=n_jobs) as executor:
                    ...
                    for future in as_completed(futures):
                        ...
                        if completed % PROGRESS_LOG_INTERVAL == 0:
                            logger.info(
                                f"Progress: {completed}/{n_proteins} proteins processed"
                            )
            ...
            significant_results = [
                r for r in cox_results if r.get("significant", False)
            ]
            top_hr = sorted(
    ...
    def kaplan_meier_analysis(
        ...
        try:
            ...
            if stratify_method == "median":
                ...
                groups = (protein_valid > cutpoint).astype(int)
                group_labels = ["Low", "High"]
                logger.info(f"Optimal cutpoint: {cutpoint:.3f} (p={best_p:.4f})")
            else:
                raise ProteomicsSurvivalError(
    ...
    def _find_optimal_cutpoint(
        ...
        for cutpoint in cutpoints:
            ...
            if (
                high_mask.sum() < MIN_CUTPOINT_GROUP_SIZE
                or (~high_mask).sum() < MIN_CUTPOINT_GROUP_SIZE
            ):
                continue
