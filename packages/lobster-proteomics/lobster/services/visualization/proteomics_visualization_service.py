        ...
        try:
            ...
            if adata.n_obs > max_samples:
                ...
                sample_indices = np.random.choice(
                    adata.n_obs, max_samples, replace=False
                )
            ...
            fig = go.Figure(
                data=go.Heatmap(
                    ...
                    customdata=[
                        ["Present" if val == 0 else "Missing" for val in row]
            ...
            fig.update_layout(
                title=title
                or f"Missing Value Pattern ({missing_percentage:.1f}% missing)",
            ...
            raise ProteomicsVisualizationError(
                f"Failed to create missing value heatmap: {str(e)}"
            )
    ...
    def create_intensity_distribution_plot(
        ...
        try:
            ...
            if group_by and group_by in adata.obs.columns:
                ...
                fig.update_layout(xaxis_title=group_by, yaxis_title=y_title)
                ...
                fig.update_layout(xaxis_title=y_title, yaxis_title="Frequency")
            ...
            fig.update_layout(
                title=title
                or f"Protein Intensity Distribution (n={stats_dict['n_values']:,})",
            ...
            raise ProteomicsVisualizationError(
                f"Failed to create intensity distribution plot: {str(e)}"
            )
        ...
        try:
            ...
            if group_by and group_by in adata.obs.columns:
                ...
                for group in groups:
                    ...
                    cv_data.extend(
                        [
                            ...
                            for protein, cv in zip(adata.var_names, group_cvs)
                ...
                fig.update_layout(
                    ...
                    title=title
                    or f"Protein CV Distribution ({high_cv_proteins} proteins >{cv_threshold}% CV)",
                )
            ...
            raise ProteomicsVisualizationError(
                f"Failed to create CV analysis plot: {str(e)}"
            )
        ...
        try:
            ...
            if comparison_results is None:
                if "differential_expression" in adata.uns:
                    ...
                    raise ProteomicsVisualizationError(
                        "No differential expression results found"
                    )
            else:
            ...
            if fold_change_col not in df.columns:
                ...
                for candidate in fc_candidates:
                    ...
                    raise ProteomicsVisualizationError(
                        f"Fold change column not found. Available: {list(df.columns)}"
                    )
            ...
            if pvalue_col not in df.columns:
                ...
                for candidate in pval_candidates:
                    ...
                    raise ProteomicsVisualizationError(
                        f"P-value column not found. Available: {list(df.columns)}"
                    )
            ...
            neg_log_pval = -np.log10(
                df[pvalue_col] + 1e-300
            )  # Add small value to avoid log(0)

            # Determine significance categories
            significant_up = (df[fold_change_col] > fc_threshold) & (
            ...
            fig.update_layout(
                title=title
                or f"Volcano Plot ({n_up} up, {n_down} down, {n_total} total)",
            ...
            raise ProteomicsVisualizationError(
                f"Failed to create volcano plot: {str(e)}"
            )
        ...
        try:
            ...
            for i, protein in enumerate(protein_names):
                ...
                if color_values is not None:
                    node_attrs["color_value"] = (
                        color_values.iloc[i]
                        if hasattr(color_values, "iloc")
                        else color_values[i]
                    )
            ...
            raise ProteomicsVisualizationError(
                f"Failed to create protein correlation network: {str(e)}"
            )
        ...
        try:
            ...
            if enrichment_results is None:
                if "pathway_enrichment" in adata.uns:
                    ...
                    raise ProteomicsVisualizationError(
                        "No pathway enrichment results found"
                    )
            else:
            ...
            if plot_type == "bubble":
                ...
                fig.update_layout(xaxis_title="Enrichment Ratio", yaxis_title="Pathway")
                ...
                fig.update_layout(xaxis_title="-log10(p-value)", yaxis_title="Pathway")
            ...
            raise ProteomicsVisualizationError(
                f"Failed to create pathway enrichment plot: {str(e)}"
            )
        ...
        try:
            ...
            if "batch" in adata.obs.columns:
                ...
                for batch in adata.obs["batch"].unique():
                    ...
                    batch_data.extend(
                        [
                            {"batch": batch, "intensity": np.log10(val + 1)}
                            for val in batch_clean[:1000]
                        ]
            ...
            fig.update_layout(
                title=title
                or f"Proteomics QC Dashboard - {platform_type.replace('_', ' ').title()}",
                width=1400,
                height=1000,
                showlegend=False,
                plot_bgcolor="white",
            ...
            raise ProteomicsVisualizationError(
                f"Failed to create proteomics QC dashboard: {str(e)}"
            )
    ...
    def create_kaplan_meier_curve(
        ...
        try:
            ...
            for i, (group_name, curve_data) in enumerate(survival_curves.items()):
                ...
                if show_censoring_marks and "censoring_times" in curve_data:
                    ...
                    for cens_time in censoring_times:
                        ...
                        surv_at_cens = (
                            survival_func[idx]
                            if idx < len(survival_func)
                            else survival_func[-1]
                        )
                        fig.add_trace(
                            go.Scatter(
                                x=[cens_time],
            ...
            if log_rank_p is not None:
                p_text = "p < 0.001" if log_rank_p < 0.001 else f"p = {log_rank_p:.3f}"
                fig.add_annotation(
                    x=0.95,
                    y=0.95,
                    xref="paper",
            ...
            if show_at_risk_table:
                ...
                time_intervals = np.linspace(
                    0, max_time, min(6, int(max_time / 100) + 1)
                )
                    ...
                    if n_at_risk:
                        ...
                        for t in time_intervals:
                            ...
                            if idx >= len(n_at_risk):
                                idx = len(n_at_risk) - 1
                            risk_vals.append(
                                str(n_at_risk[idx]) if idx < len(n_at_risk) else "0"
                            )
            ...
            # Update layout
            plot_title = title or "Kaplan-Meier Survival Curves" + (
            ...
            fig.update_layout(
                ...
                margin=dict(
                    b=120 if show_at_risk_table else 50
                ),  # Extra margin for at-risk table
            )
