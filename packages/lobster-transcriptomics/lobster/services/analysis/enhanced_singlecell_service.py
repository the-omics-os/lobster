    ...
    def _calculate_per_cell_confidence(
        ...
        for ct, markers in reference_markers.items():
            ...
            if len(valid_markers) > 0:
                # Mean expression per marker gene across all cells (reference profile)
                marker_expr = adata[:, valid_markers].X
                if hasattr(marker_expr, "toarray"):
                    marker_expr = marker_expr.toarray()
                signatures[ct] = np.mean(
                    marker_expr, axis=0
                )  # shape: (n_valid_markers,)
            else:
...
"""
    ...
    def run_pathway_enrichment(
        ...
        try:
            ...
            if PathwayEnrichmentError is not None and isinstance(
                e, PathwayEnrichmentError
            ):
    ...
    def _extract_marker_gene_list(
        ...
        if "rank_genes_groups" in adata.uns:
            ...
            logger.info(
                f"Extracted {len(marker_genes)} unique markers from {len(groups)} clusters"
            )
            ...
            if "highly_variable" in adata.var.columns:
                marker_genes = adata.var_names[adata.var["highly_variable"]].tolist()[
                    :500
                ]
            else:
                # Last resort: use all genes (not recommended)
                logger.warning(
                    "No highly variable genes found. Using all genes (not optimal)."
                )
