...
class ManualAnnotationService:
    ...
    def initialize_annotation_session(
        self,
        adata: sc.AnnData,
        ...
        for cluster_id in unique_clusters:
            ...
            if "total_counts" in adata.obs.columns:
                qc_scores["mean_total_counts"] = adata.obs.loc[
                    cluster_mask, "total_counts"
                ].mean()
            if "n_genes_by_counts" in adata.obs.columns:
                qc_scores["mean_genes"] = adata.obs.loc[
                    cluster_mask, "n_genes_by_counts"
                ].mean()
            if "pct_counts_mt" in adata.obs.columns:
                qc_scores["mean_mt_pct"] = adata.obs.loc[
                    cluster_mask, "pct_counts_mt"
                ].mean()
        ...
        self.cluster_colors = {
            cluster_id: info.color for cluster_id, info in clusters.items()
        }
    ...
    def rich_annotation_interface(self) -> Dict[str, str]:
        ...
        if not self.state:
            raise ValueError(
                "No annotation session initialized. Call initialize_annotation_session first."
            )
...
The colors in this terminal [bold]exactly match[/bold] your UMAP plot colors for easy identification.
        ...
        return Panel(
            layout, title="Manual Annotation Session", border_style="bright_blue"
        )
    ...
    def _annotate_clusters_interactive(self):
        ...
        unannotated_clusters = [
            (cid, info)
            for cid, info in self.state.clusters.items()
        ...
        for cluster_id, info in unannotated_clusters:
            color_style = Style(color=info.color, bold=True)
            qc_text = (
                f"Genes: {info.qc_scores.get('mean_genes', 0):.0f}, "
                + f"MT%: {info.qc_scores.get('mean_mt_pct', 0):.1f}"
            )

            cluster_table.add_row(
                cluster_id, "●●●●", str(info.cell_count), qc_text, style=color_style
            )
    ...
    def _annotate_single_cluster(self, cluster_id: str, info: ClusterInfo):
        ...
        self.console.print(
            f"[green]✅ Cluster {cluster_id} annotated as '{cell_type}'[/green]"
        )
    ...
    def _manual_debris_marking(self):
        ...
        if not non_debris:
            self.console.print(
                "[yellow]No clusters available for debris marking.[/yellow]"
            )
            return
        ...
        while True:
            ...
            for cluster_id, info in non_debris:
                ...
                cluster_table.add_row(
                    cluster_id, "●●●", str(info.cell_count), status, style=color_style
                )
            ...
            if (
                cluster_choice in self.state.clusters
                and cluster_choice not in self.state.debris_clusters
            ):
                self._mark_as_debris(cluster_choice, "Manual selection")
            else:
                self.console.print(
                    "[red]Invalid cluster ID or already marked as debris.[/red]"
                )
    ...
    def _mark_as_debris(self, cluster_id: str, reason: str):
        ...
        self.console.print(
            f"[red]🗑️ Cluster {cluster_id} marked as debris: {reason}[/red]"
        )
    ...
    def _collapse_clusters_interactive(self):
        ...
        collapse_table = Table(
            title="Cell Types with Multiple Clusters", box=box.ROUNDED
        )
        ...
        for cell_type, clusters in cell_type_groups.items():
            if len(clusters) > 1:
                total_cells = sum(
                    self.state.clusters[cid].cell_count for cid in clusters
                )
                cluster_display = ", ".join(str(c) for c in sorted(clusters))
        ...
        if not collapsible_types:
            self.console.print(
                "[yellow]No cell types have multiple clusters to collapse.[/yellow]"
            )
            return
    ...
    def _collapse_clusters_for_type(self, cell_type: str, cluster_ids: List[str]):
        ...
        for cluster_id in sorted(cluster_ids):
            ...
            detail_table.add_row(
                cluster_id, "●●●", str(info.cell_count), style=color_style
            )

        self.console.print(detail_table)

        if Confirm.ask(
            f"Collapse these {len(cluster_ids)} clusters into '{cell_type}'?"
        ):
            ...
            self.console.print(
                f"[green]✅ Clusters collapsed into '{collapsed_id}'[/green]"
            )
    ...
    def _show_annotation_summary(self):
        ...
        stats_table.add_row(
            "Annotated", str(annotated), f"{100 * annotated / total_clusters:.1f}%"
        )
        stats_table.add_row(
            "Debris", str(debris), f"{100 * debris / total_clusters:.1f}%"
        )
        stats_table.add_row(
            "Pending", str(pending), f"{100 * pending / total_clusters:.1f}%"
        )
        ...
        if cell_type_counts:
            ...
            for cell_type in sorted(cell_type_counts.keys()):
                ...
                annotation_table.add_row(
                    cell_type, str(clusters), str(cells), f"{avg_cells:.0f}"
                )
        ...
        layout.split_column(
            ...
            Layout(
                annotation_table if cell_type_counts else Panel("No annotations yet"),
                name="annotations",
            ),
        )
    ...
    def _undo_last_action(self):
        ...
        if action_type == "annotate":
            ...
            self.console.print(
                "[yellow]Collapse actions cannot be undone automatically.[/yellow]"
            )
            return
    ...
    def _import_annotations(self):
        ...
        try:
            ...
            self.state.cell_type_mapping.update(
                import_data.get("cell_type_mapping", {})
            )
            ...
            self.console.print(
                f"[green]✅ Annotations imported from {filename}[/green]"
            )
    ...
    def apply_annotations_to_adata(
        self,
        adata: sc.AnnData,
        cluster_key: str = "leiden",
        cell_type_column: str = "cell_type_manual",
    ) -> sc.AnnData:
        """
        ...
        if not self.state:
            raise ValueError(
                "No annotation session available. Run rich_annotation_interface first."
            )
        ...
        # Apply annotations
        adata_copy.obs[cell_type_column] = (
    ...
    def create_annotation_umap_with_palette(
        self, adata: sc.AnnData, cluster_col: str = "leiden"
    ...
    def validate_annotation_coverage(
        self, adata: sc.AnnData, annotation_col: str = "cell_type_manual"
    ) -> Dict[str, Any]:
        """
