...
class BulkVisualizationService:
    ...
    def create_volcano_plot(
        ...
        try:
            ...
            if len(log2fc) == 0:
                # Return empty figure for zero-gene case
                fig = go.Figure()
                fig.add_annotation(
                    text="No genes available for plotting",
                    xref="paper",
                    yref="paper",
                    x=0.5,
                    y=0.5,
                ...
                stats = {
                    ...
                    "n_genes_up": 0,
                    "n_genes_down": 0,
                    "n_genes_not_significant": 0,
                }
                ir = (
                    self._create_volcano_ir(
                        fdr_threshold=fdr_threshold,
                        fc_threshold=fc_threshold,
                        top_n_genes=top_n_genes,
                    )
                    if hasattr(self, "_create_volcano_ir")
                    else AnalysisStep(
