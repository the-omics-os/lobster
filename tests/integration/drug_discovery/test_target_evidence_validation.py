...
class TestTargetDiseaseEvidence:
    ...
    def test_egfr_top_disease_includes_lung_cancer(self, ot):
        ...
        _, stats, ir = ot.get_target_disease_evidence(KNOWN_TARGETS["EGFR"], limit=10)
        if "error" in stats:
            pytest.skip(f"Open Targets unavailable: {stats['error'][:80]}")

        assert stats["approved_symbol"] == "EGFR"

        disease_names = [a["disease_name"].lower() for a in stats["associations"]]
        assert any(
            "lung" in d for d in disease_names
    ...
    def test_egfr_has_high_association_score(self, ot):
        ...
        _, stats, _ = ot.get_target_disease_evidence(KNOWN_TARGETS["EGFR"], limit=5)
        if "error" in stats:
            pytest.skip(f"Open Targets unavailable: {stats['error'][:80]}")
    ...
    def test_egfr_has_many_associated_diseases(self, ot):
        ...
        assert (
            stats["total_associated_diseases"] > 50
    ...
    def test_braf_top_disease_includes_melanoma(self, ot):
        ...
        _, stats, _ = ot.get_target_disease_evidence(KNOWN_TARGETS["BRAF"], limit=10)
        if "error" in stats:
            pytest.skip(f"Open Targets unavailable: {stats['error'][:80]}")
        ...
        disease_names = [a["disease_name"].lower() for a in stats["associations"]]
        assert any(
            "melanoma" in d for d in disease_names
    ...
    def test_tp53_has_cancer_associations(self, ot):
        ...
        _, stats, _ = ot.get_target_disease_evidence(KNOWN_TARGETS["TP53"], limit=10)
        if "error" in stats:
            pytest.skip(f"Open Targets unavailable: {stats['error'][:80]}")
    ...
    def test_abl1_associates_with_leukemia(self, ot):
        ...
        _, stats, _ = ot.get_target_disease_evidence(KNOWN_TARGETS["ABL1"], limit=10)
        if "error" in stats:
            pytest.skip(f"Open Targets unavailable: {stats['error'][:80]}")
    ...
    def test_evidence_returns_valid_ir(self, ot):
        ...
        _, stats, ir = ot.get_target_disease_evidence(KNOWN_TARGETS["EGFR"], limit=5)
        if "error" in stats:
            pytest.skip(f"Open Targets unavailable: {stats['error'][:80]}")
