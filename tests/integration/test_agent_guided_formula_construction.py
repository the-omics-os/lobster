...
class TestAgentGuidedFormulaConstruction:
    ...
    def test_workflow_tracker_basic_functionality(self, workflow_tracker):
        ...
        results_df = pd.DataFrame(
            {"log2FoldChange": [2.0, -1.5, 0.5], "padj": [0.001, 0.01, 0.8]},
    ...
    def test_workflow_tracker_comparison(self, workflow_tracker):
        ...
        results_df2 = pd.DataFrame(
            {"log2FoldChange": [1.8, -1.2, 1.5], "padj": [0.002, 0.02, 0.001]},
    ...
    def test_error_handling(self, formula_service, mock_data_manager):
        ...
        metadata = pd.DataFrame({"condition": ["A", "A", "B", "B"]})
    ...
    def test_workflow_export_import(self, workflow_tracker):
        ...
        results_df = pd.DataFrame(
            {"log2FoldChange": [2.0, -1.5], "padj": [0.001, 0.01]},
            index=["GENE1", "GENE2"],
        )
        ...
        with tempfile.NamedTemporaryFile(
            ...
            export_path = workflow_tracker.export_iteration_history(
                "test_modality", tmp_file.name
            )
            ...
            try:
                ...
                # Import back
                imported_modality = workflow_tracker.import_iteration_history(
                    export_path
                )
...
class TestFormulaConstructionWorkflow:
    ...
    def test_complete_workflow_simulation(self):
        ...
        suggestions = formula_service.suggest_formulas(
            metadata, analysis_goal="Compare disease vs healthy"
        )
        ...
        power_estimate = formula_service.estimate_statistical_power(
        ...
        if len(suggestions) > 1:
            ...
            batch_design = formula_service.construct_design_matrix(
                batch_components, metadata
            )

            # Should have more coefficients (intercept + condition + batch levels)
            assert (
                batch_design["design_matrix"].shape[1]
                > design_result["design_matrix"].shape[1]
            )
    ...
    def test_formula_validation_edge_cases(self):
        ...
        try:
            components = formula_service.parse_formula(
                "~condition + batch + perfect_correlation", rank_deficient_metadata
            )
            design_result = formula_service.construct_design_matrix(
                components, rank_deficient_metadata
            )
            # Rank should be less than number of columns due to correlation
            print(
                f"Rank: {design_result['rank']}, Columns: {design_result['n_coefficients']}"
            )
        except Exception as e:
    ...
    def test_workflow_tracker_comprehensive(self):
        ...
        # Test comparisons
        comparison = tracker.compare_iterations(
            "test_modality", "simple", "batch_corrected"
        )
        assert (
            comparison["overlap_stats"]["overlapping_genes"] >= 1
        )  # At least GENE1, GENE2
...
def run_integration_tests():
    ...
    results_df = pd.DataFrame(
        {"log2FoldChange": [2.0, -1.5, 0.8], "padj": [0.001, 0.01, 0.03]},
        index=["GENE1", "GENE2", "GENE3"],
    )
