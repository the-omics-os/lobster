...
class TestBasicAgentWorkflows:
    ...
    def test_supervisor_agent_initialization(
        ...
        state["messages"] = [
            {"content": "Analyze single-cell data from GEO", "sender": "human"}
        ]
        ...
        with patch("lobster.agents.supervisor.supervisor_agent") as mock_supervisor:
            mock_supervisor.return_value = {
                ...
                + [
                    {
                        "content": "I'll help you analyze single-cell data",
                        "sender": "assistant",
                    }
                ],
    ...
    def test_data_expert_to_singlecell_handoff(
        self, mock_agent_client, mock_workflow_state, sample_geo_dataset
    ):
    ...
    def test_research_agent_workflow(self, mock_agent_client, mock_workflow_state):
        ...
        state = mock_workflow_state.copy()
        state["messages"] = [
            {"content": "Find papers about T cell exhaustion", "sender": "human"}
        ]
        ...
        with patch("lobster.agents.research_agent.research_agent") as mock_research:
            mock_research.return_value = {
                ...
                + [
                    {
                        "content": "Found 15 relevant papers about T cell exhaustion",
                ...
                "search_results": [
                    ...
                    {
                        "pmid": "87654321",
                        "title": "Molecular markers of T cell dysfunction",
                        "journal": "Cell",
                    },
                ],
...
class TestComplexMultiAgentWorkflows:
    ...
    def test_complete_single_cell_analysis_workflow(
        self, mock_agent_client, mock_workflow_state, sample_geo_dataset
    ):
        ...
        initial_state["messages"] = [
            {
                "content": "Perform complete single-cell analysis of GSE123456",
                "sender": "human",
            }
        ]
        ...
        with patch("lobster.agents.supervisor.supervisor_agent") as mock_supervisor:
            mock_supervisor.return_value = {
                ...
                + [
                    {
                        "content": "I'll coordinate a complete single-cell analysis",
        ...
        with patch("lobster.agents.data_expert.data_expert_agent") as mock_data_expert:
            mock_data_expert.return_value = {
                "messages": data_state["messages"]
                + [
                    {
                        "content": "Successfully loaded GSE123456 dataset",
                        "sender": "data_expert_agent",
                    }
                ],
    ...
    def test_parallel_agent_execution(self, mock_agent_client, mock_workflow_state):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=3) as executor:
            future_to_task = {
                executor.submit(execute_agent_task, task): task for task in tasks
            }
            results = {}
    ...
    def test_workflow_error_recovery(self, mock_agent_client, mock_workflow_state):
        ...
        error_state["messages"] = [
            {"content": "Load invalid dataset XYZ", "sender": "human"}
        ]
        error_state["current_agent"] = "data_expert_agent"

        with patch("lobster.agents.data_expert.data_expert_agent") as mock_data_expert:
            mock_data_expert.return_value = {
                "messages": error_state["messages"]
                + [
                    {
                        "content": "Failed to load dataset XYZ",
                        "sender": "data_expert_agent",
                    }
                ],
        ...
        with patch("lobster.agents.supervisor.supervisor_agent") as mock_supervisor:
            mock_supervisor.return_value = {
                ...
                + [
                    {
                        "content": "Let me help you find an alternative dataset",
                        "sender": "supervisor_agent",
                    }
                ],
                "error_handled": True,
        ...
        with patch("lobster.agents.research_agent.research_agent") as mock_research:
            mock_research.return_value = {
                ...
                + [
                    {
                        "content": "Found alternative datasets",
                        "sender": "research_agent",
                    }
                ],
...
class TestAgentCommunication:
    ...
    def test_context_preservation_across_agents(
        self, mock_agent_client, mock_workflow_state
    ):
        ...
        assert (
            sc_result["context"]["dataset_loaded"] == "geo_gse123456"
        )  # From data expert
    ...
    def test_cross_agent_data_references(self, mock_agent_client, mock_workflow_state):
        ...
        assert (
            final_shared_data["datasets"]["clustered_data"]["literature_validated"]
            == True
        )
...
class TestWorkflowPerformanceMonitoring:
    ...
    def test_workflow_execution_timing(self, mock_agent_client, mock_workflow_state):
        ...
        with patch("lobster.agents.data_expert.data_expert_agent") as mock_data_expert:
            ...
            workflow_metrics["agent_timings"]["data_expert"] = data_result[
                "execution_time"
            ]
        ...
        with patch(
            ...
            workflow_metrics["agent_timings"]["singlecell_expert"] = sc_result[
                "execution_time"
            ]
            workflow_metrics["total_agents"] += 1
    ...
    def test_workflow_resource_monitoring(self, mock_agent_client, mock_workflow_state):
        ...
        resource_usage["memory_peak"] = max(
            resource_usage["memory_peak"], result["resource_usage"]["peak_memory_mb"]
        )
    ...
    def test_workflow_progress_tracking(self, mock_agent_client, mock_workflow_state):
        ...
        for i, step in enumerate(workflow_steps[:3]):  # Complete first 3 steps
            ...
            workflow_progress["progress_percentage"] = (
                len(workflow_progress["completed_steps"])
                / workflow_progress["total_steps"]
            ) * 100
...
class TestWorkflowStateManagement:
    ...
    def test_workflow_state_serialization(
        self, mock_agent_client, mock_workflow_state, temp_workspace
    ):
        ...
        checkpoint_state.update(
            {
                ...
                "recovery_data": {
                    ...
                    "preprocessing_results": {
                        "n_cells_after_qc": 4500,
                        "n_genes_filtered": 18000,
                    },
                },
            }
        )
    ...
    def test_workflow_branching_and_merging(
        self, mock_agent_client, mock_workflow_state
    ):
        ...
        merged_results = {
            ...
            "recommended_branch": (
                "standard_clustering"
                if branch_results["standard"]["silhouette_score"]
                > branch_results["high_res"]["silhouette_score"]
                else "high_res_clustering"
            ),
        }
