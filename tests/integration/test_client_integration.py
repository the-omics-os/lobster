...
Note: Tests automatically adapt to available clients:
- AgentClient tests (public): Always run
- APIAgentClient tests (private): Only run when API client is available
"""
...
def _create_mock_graph_metadata():
    """Create a mock GraphMetadata for tests."""
    mock_metadata = Mock()
...
def _mock_create_graph_return(mock_graph):
    ...
    return (mock_graph, _create_mock_graph_metadata())
...
class TestAgentClientIntegration:
    ...
    def test_client_initialization_with_real_workspace(
        ...
        with patch("lobster.core.client.create_bioinformatics_graph") as mock_graph:
            mock_graph.return_value = _mock_create_graph_return(Mock())
    ...
    def test_error_recovery_integration(self, temp_integration_workspace):
        ...
        with patch(
            ...
            history = client.get_conversation_history()
            # When query fails, no assistant message is added, so: 3 user + 2 assistant = 5
...
class TestClientPerformanceIntegration:
    """Performance tests for client components (public repo compatible)."""
