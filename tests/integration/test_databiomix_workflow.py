"""
Integration tests for DataBioMix publication processing workflow.
...
pytest tests/integration/test_databiomix_workflow.py -v

# Only unit tests (no network required)
pytest tests/integration/test_databiomix_workflow.py -v -m "not real_api"

# Only network tests (requires API access)
pytest tests/integration/test_databiomix_workflow.py -v -m real_api

# Specific test
pytest tests/integration/test_databiomix_workflow.py::TestCSVExport::test_csv_export_column_oriented_data -v
```
"""
...
class TestWorkspaceTools:
    ...
    def test_write_to_workspace_csv_format(self, workspace_service):
        ...
        sample_data = MetadataContent(
            identifier="databiomix_sample_mapping",
            content_type="sample_id_mapping",
            description="Sample ID mapping for DataBioMix project",
            data={
                "original_id": ["DB001", "DB002", "DB003", "DB004"],
            ...
            source="DataBioMixMappingService",
            cached_at=datetime.now().isoformat(),
        )
        ...
        assert csv_file.name == "databiomix_sample_mapping.csv"
        ...
        assert df["original_id"].tolist() == ["DB001", "DB002", "DB003", "DB004"]
...
def test_databiomix_workflow_integration(data_manager, workspace_service):
    """
    Integration test for complete DataBioMix workflow.
    ...
    This test simulates the real-world DataBioMix use case of
    ...
    sample_mapping = MetadataContent(
        identifier="databiomix_integration_test",
        content_type="sample_mapping",
        description="Integration test for DataBioMix workflow",
        data={
            "original_sample_id": ["SAMPLE_001", "SAMPLE_002", "SAMPLE_003"],
            "normalized_id": ["DB_NORM_001", "DB_NORM_002", "DB_NORM_003"],
            "source_dataset": ["GSE123456", "GSE123456", "GSE789012"],
            "tissue_type": ["brain", "liver", "brain"],
            "quality_pass": [True, True, False],
        },
        related_datasets=["GSE123456", "GSE789012"],
        source="DataBioMixIntegrationTest",
        cached_at=datetime.now().isoformat(),
    )
    ...
    assert csv_file.name == "databiomix_integration_test.csv"
    ...
    assert json_content["identifier"] == "databiomix_integration_test"
