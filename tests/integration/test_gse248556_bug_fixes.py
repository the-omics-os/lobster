...
class TestGSE248556AllBugFixes:
    ...
    def test_gse248556_download_with_retry_logic(
        self, geo_service, temp_workspace
    ):
        """
        ...
        try:
            ...
            large_files = [f for f in files if "size" in f and "MB" in str(f.get("size", ""))]
            assert len(large_files) > 0, "No large files found (expected 70-100MB files)"
            ...
            pytest.skip(f"GSE248556 download test skipped (network/GEO unavailable): {e}")

    def test_gse248556_vdj_duplicate_barcodes_accepted(
        self, geo_service, data_manager
    ):
        """
        ...
        assert duplicate_count == 3152, f"Expected 3,152 duplicates, got {duplicate_count}"
        assert 45 <= duplication_rate <= 50, f"Expected ~48% duplication, got {duplication_rate:.1f}%"

        print(f"✓ VDJ validation accepted {n_total_rows} rows with {duplicate_count} duplicates ({duplication_rate:.1f}%)")
    ...
    def test_gse248556_metadata_sanitization_for_h5ad(
        ...
        if hasattr(is_productive_col, 'cat'):
            ...
            assert 'True' in is_productive_col.cat.categories or 'False' in is_productive_col.cat.categories
        else:
        ...
        print(f"✓ Metadata sanitization successful: {len(adata_loaded.obs.columns)} columns preserved")
    ...
    def test_gse248556_full_workflow_integration(
        ...
        try:
            ...
            obs = pd.DataFrame({
                ...
                'is_productive': np.random.choice([True, False, None], n_total_rows),
                'clone_size': [10, None, 'large', 5, None] * (n_total_rows // 5),
                'confidence': [0.95, None, 'high', 0.87, None] * (n_total_rows // 5),
                'chain_type': np.random.choice(['TRA', 'TRB', 'IGH', 'IGL'], n_total_rows),
            }, index=all_barcodes)
            ...
            print(f"✓ Created VDJ data: {adata.shape[0]} observations, {adata.shape[1]} features")
            print(f"✓ Duplicate barcodes: {n_total_rows - n_unique_cells} ({48.0:.1f}%)")
            ...
            assert len(adata_reloaded.obs.columns) >= 3  # At least some obs columns preserved
            ...
            print(f"  ✓ Bug #2: VDJ duplicate barcodes accepted ({n_total_rows - n_unique_cells} duplicates)")
            print(f"  ✓ Bug #3: Metadata sanitization successful")

        except Exception as e:
            if "network" in str(e).lower() or "geo" in str(e).lower():
                pytest.skip(f"GSE248556 full workflow test skipped (network/GEO unavailable): {e}")
            else:
                raise
