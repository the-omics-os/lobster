    ...
    try:
        ...
        if expected_outcome == "ACCEPT":
            ...
            if geo_id in data_manager.metadata_store:
                modality_info = data_manager.metadata_store[geo_id].get("modality_detection")
                if modality_info:
            ...
            print(f"❌ FAIL: Dataset {geo_id} was ACCEPTED but should have been REJECTED")
            return False
        ...
        if expected_outcome == "REJECT":
            ...
            print(f"❌ FAIL: Dataset {geo_id} was REJECTED but should have been ACCEPTED")
        ...
        if expected_outcome == "REJECT":
            ...
            print(f"❌ FAIL: Dataset {geo_id} was REJECTED at Tier 1 but should have been ACCEPTED")
            print(f"   Error: {e.message}")
            return False
...
def main():
    ...
    for geo_id, expected, description in test_cases:
        if expected == "SKIP":
            ...
            print(f"{'='*70}")
            print(f"⚠️  SKIP: {geo_id} is a SuperSeries with empty metadata (known limitation)")
