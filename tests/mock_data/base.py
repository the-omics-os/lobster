...
class TestDataRegistry:
    ...
    def to_json(self) -> str:
        ...
        export_data = {
            name: config.to_dict() for name, config in self._metadata.items()
        }
...
HIGH_NOISE_CONFIG = MockDataConfig(
    seed=42, noise_level=0.5, sparsity_level=0.8, batch_effect_strength=0.5
)
...
BATCH_EFFECT_CONFIG = MockDataConfig(
    seed=42, batch_effects=True, n_batches=5, batch_effect_strength=0.8
)
