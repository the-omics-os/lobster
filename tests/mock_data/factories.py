...
from .base import MEDIUM_DATASET_CONFIG, MockDataConfig
...
class SingleCellDataFactory(BaseDataFactory):
    ...
    def _create(cls, model_class, **kwargs):
        ...
        X[:, housekeeping_idx] *= np.random.uniform(
            2, 5, size=(n_cells, n_housekeeping)
        )
        ...
        adata = ad.AnnData(
            X=X, obs=pd.DataFrame(index=cell_names), var=pd.DataFrame(index=gene_names)
        )
        ...
        adata.var["chromosome"] = np.random.choice(
            [f"chr{i}" for i in range(1, 23)] + ["chrX", "chrY"], size=n_genes
        )
        ...
        adata.obs["doublet_score"] = np.random.beta(
            2, 8, size=n_cells
        )  # Most cells low doublet score
...
class BulkRNASeqDataFactory(BaseDataFactory):
    ...
    def _create(cls, model_class, **kwargs):
        ...
        conditions = ["Treatment"] * samples_per_condition + ["Control"] * (
...
class ProteomicsDataFactory(BaseDataFactory):
    ...
    n_samples = LazyAttribute(
        lambda obj: obj.config.default_sample_count * 2
    )  # More samples for proteomics
    ...
    def _create(cls, model_class, **kwargs):
        ...
        adata.var["subcellular_location"] = np.random.choice(
            ["Cytoplasm", "Nucleus", "Membrane", "Mitochondria", "ER"], size=n_proteins
        )
...
class MultiModalDataFactory(BaseDataFactory):
    ...
    def _create(cls, model_class, **kwargs):
        ...
        return {"rna": rna_data, "protein": protein_data}
