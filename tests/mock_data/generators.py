...
def generate_mock_geo_response(
    ...
    for i in range(n_samples):
        ...
        if include_files:
            if data_type == "single_cell":
                ...
                sample["supplementary_files"] = [f"{sample['gsm_id']}_counts.txt.gz"]
            else:  # proteomics
                sample["supplementary_files"] = [f"{sample['gsm_id']}_intensities.xlsx"]
    ...
    return {
        ...
        "submission_date": (
            datetime.now() - timedelta(days=fake.random_int(30, 365))
        ).strftime("%Y-%m-%d"),
        "last_update": (
...
def generate_test_workspace_state(
    ...
    return {
        "workspace_path": (
            str(workspace_path) if workspace_path else "/tmp/test_workspace"
        ),
        "modalities": {
            name: {
...
def _get_keywords_for_data_type(
    data_type: str, tissue: str, treatment: str
) -> List[str]:
