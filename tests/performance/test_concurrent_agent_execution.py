...
class ConcurrentExecutionMonitor:
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "success_rate": (
                len(successful_executions) / len(self.execution_metrics)
            ...
            "queue_performance": {
                "mean_wait_time": (
                    float(np.mean(queue_wait_times)) if queue_wait_times else 0
                ),
                "max_wait_time": (
                    float(np.max(queue_wait_times)) if queue_wait_times else 0
                ),
            },
    ...
    def _get_agent_breakdown(self) -> Dict[str, Dict[str, Any]]:
        ...
        for agent_type in set(m.agent_type for m in self.execution_metrics):
            agent_metrics = [
                m for m in self.execution_metrics if m.agent_type == agent_type
            ]
...
def concurrent_test_datasets(concurrent_data_manager):
    ...
    medium_sc = SingleCellDataFactory(
...
class TestConcurrentAgentExecution:
    ...
    def test_basic_concurrent_agent_execution(
        self, mock_agent_clients, concurrent_task_definitions
    ):
        ...
        class ConcurrentAgentExecutor:
            ...
            def execute_concurrent_tasks(
                self, tasks: List[ConcurrentTask], max_workers: int = 4
            ):
        ...
        performance_summary = executor.execute_concurrent_tasks(
            concurrent_task_definitions, max_workers=3
        )
    ...
    def test_agent_resource_contention(
        self, mock_agent_clients, concurrent_data_manager
    ):
        ...
        class ResourceContentionTester:
            ...
            def test_memory_contention(self, num_memory_intensive_tasks: int = 5):
                ...
                with ThreadPoolExecutor(max_workers=3) as executor:
                    futures = [
                        executor.submit(memory_intensive_worker, task)
                ...
                return {
                    ...
                    "peak_concurrent_memory": (
                        max(
                            [
                                sample["memory_used_gb"]
                                for sample in monitor.resource_samples
                            ]
                        )
                        if monitor.resource_samples
                        else 0
                    ),
                }
            ...
            def test_cpu_contention(self, num_cpu_intensive_tasks: int = 4):
                ...
                with ThreadPoolExecutor(max_workers=mp.cpu_count()) as executor:
                    futures = [
                        executor.submit(cpu_intensive_worker, task)
            ...
            def test_io_contention(self, num_io_tasks: int = 6):
                ...
                def io_intensive_worker(task):
                    ...
                    temp_data = SingleCellDataFactory(
                        config={**SMALL_DATASET_CONFIG, "n_obs": 5000, "n_vars": 2000}
                    )
                ...
                with ThreadPoolExecutor(max_workers=4) as executor:
                    futures = [
                        executor.submit(io_intensive_worker, task) for task in io_tasks
                    ]
                ...
                return {
                    ...
                    "performance_summary": monitor.get_performance_summary(),
                    "avg_io_time": monitor.get_performance_summary()[
                        "execution_time_stats"
                    ]["mean"],
                }

        # Test resource contention
        contention_tester = ResourceContentionTester(
            mock_agent_clients, concurrent_data_manager
        )

        # Test memory contention
        memory_result = contention_tester.test_memory_contention(
            num_memory_intensive_tasks=4
        )

        assert memory_result["memory_contention_test"] == "completed"
        memory_summary = memory_result["performance_summary"]
        assert (
            memory_summary["success_rate"] >= 0.8
        ), "Memory contention caused too many failures"
        assert (
            memory_result["peak_concurrent_memory"] < 20.0
        ...
        assert (
            cpu_summary["success_rate"] >= 0.8
        ), "CPU contention caused too many failures"
        ...
        assert (
            io_summary["success_rate"] >= 0.8
        ), "I/O contention caused too many failures"
        assert (
            io_result["avg_io_time"] < 15.0
        ), "I/O operations too slow under contention"
    ...
    def test_agent_scalability_under_load(self, mock_agent_clients):
        ...
        class ScalabilityTester:
            ...
            def test_increasing_load(self, load_levels: List[int]):
                ...
                for num_tasks in load_levels:
                    ...
                    with ThreadPoolExecutor(max_workers=max_workers) as executor:
                        ...
                        futures = [
                            executor.submit(execute_mock_task, task) for task in tasks
                        ]
                    ...
                    scalability_results.append(
                        {
                            ...
                            "throughput_tasks_per_second": num_tasks
                            / (execution_end - execution_start),
                            "avg_task_latency": performance_summary[
                                "execution_time_stats"
                            ]["mean"],
            ...
            def _generate_tasks_for_load_test(
                self, num_tasks: int
            ) -> List[ConcurrentTask]:
            ...
            def _calculate_resource_efficiency(
                self, performance_summary: Dict, num_tasks: int, total_time: float
            ) -> float:
            ...
            def _analyze_scalability_trends(
                ...
                return {
                    ...
                    "throughput_degradation": (
                        (throughput_list[0] - throughput_list[-1]) / throughput_list[0]
                        if throughput_list[0] > 0
                        else 0
                    ),
                    ...
                    "efficiency_trend": (
                        "improving"
                        if efficiency_list[-1] > efficiency_list[0]
                        else "degrading"
                    ),
            ...
            def _calculate_scalability_score(self, results: List[Dict]) -> float:
                ...
                throughput_retention = min(
                    1.0,
                    results[-1]["throughput_tasks_per_second"]
                    / results[0]["throughput_tasks_per_second"],
                )

                latency_ratios = [r["avg_task_latency"] for r in results]
                latency_control = 1.0 / (
                    1.0 + (max(latency_ratios) - min(latency_ratios))
                )

                avg_efficiency = np.mean([r["resource_efficiency"] for r in results])

                return (
                    (throughput_retention * 0.4)
                    + (latency_control * 0.3)
                    + (avg_efficiency * 0.3)
                )
        ...
        assert (
            analysis["throughput_degradation"] < 0.7
        ), "Excessive throughput degradation under load"
    ...
    def test_agent_coordination_performance(
        ...
        class AgentCoordinationTester:
            ...
            def test_multi_agent_workflow(self, workflow_complexity: str = "medium"):
                ...
                return {
                    "workflow_coordination_successful": all(
                        r["result"]["success"] for r in workflow_results
                    ),
                    "total_workflow_time": workflow_end - workflow_start,
                    "workflow_steps": len(workflow_steps),
                    "coordination_overhead": sum(
                        r["metrics"].queue_wait_time for r in workflow_results
                    ),
            ...
            def _complex_workflow(self) -> List[Dict]:
                ...
                return [
                    {
                        "step_id": "data_load_rna",
                        "agent_type": "data",
                        "description": "Load RNA data",
                    },
                    {
                        "step_id": "data_load_protein",
                        "agent_type": "data",
                    ...
                    },
                    {
                        ...
                        "description": "Protein QC",
                    },
                    {
                        "step_id": "sc_preprocess_rna",
                        "agent_type": "singlecell",
                        "description": "RNA preprocessing",
                    },
                    {
                        "step_id": "prot_preprocess",
                        "agent_type": "proteomics",
                        "description": "Protein preprocessing",
                    },
                    {
                        "step_id": "multimodal_integration",
                        "agent_type": "singlecell",
        ...
        coordination_tester = AgentCoordinationTester(
            mock_agent_clients, concurrent_data_manager
        )
        ...
        for complexity in ["simple", "medium", "complex"]:
            ...
            assert (
                workflow_result["workflow_coordination_successful"] == True
            ), f"Workflow {complexity} failed"

            expected_max_time = {"simple": 30.0, "medium": 60.0, "complex": 120.0}
            assert (
                workflow_result["total_workflow_time"] < expected_max_time[complexity]
            ), f"Workflow {complexity} too slow"

            # Coordination overhead should be reasonable
            overhead_ratio = (
                workflow_result["coordination_overhead"]
                / workflow_result["total_workflow_time"]
            )
            assert (
                overhead_ratio < 0.2
            ), f"Coordination overhead too high for {complexity} workflow"

            # Performance summary should show good metrics
            perf_summary = workflow_result["performance_summary"]
            assert (
                perf_summary["success_rate"] >= 0.9
            ), f"Poor success rate for {complexity} workflow"
