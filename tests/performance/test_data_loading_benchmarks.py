        ...
        if all_times:
            summary["overall_stats"] = {
                ...
                "throughput_ops_per_second": (
                    success_count / np.sum(all_times) if np.sum(all_times) > 0 else 0
                ),
            }
    ...
    for config in file_configs:
        ...
        if config["format"] == "h5ad":
            ...
            adata = SingleCellDataFactory(
                config=config.get("data_config", SMALL_DATASET_CONFIG)
            )
            ...
            with h5py.File(file_path, "w") as f:
                ...
                f.create_dataset(
                    "gene_names", data=[f"gene_{i}".encode() for i in range(n_cols)]
                )
                f.create_dataset(
                    "cell_names", data=[f"cell_{i}".encode() for i in range(n_rows)]
                )
...
class TestFileFormatLoadingPerformance:
    ...
    def test_h5ad_loading_performance(
        self, benchmark_test_files, benchmark_data_manager
    ):
        ...
        direct_load_stats = performance_summary["operation_breakdown"].get(
            "direct_h5ad_load", {}
        )
        if direct_load_stats:
            assert direct_load_stats["avg_time"] < 30.0, "H5AD loading too slow"
            assert (
                direct_load_stats["success_rate"] >= 0.9
            ), "H5AD loading failure rate too high"

        return h5ad_results

    def test_csv_tsv_loading_performance(
        self, benchmark_test_files, benchmark_data_manager
    ):
        ...
        class CSVTSVLoadingBenchmark:
            ...
            def benchmark_csv_tsv_operations(self, files: List[Path]):
                ...
                for file_path in files:
                    if not (
                        file_path.name.endswith(".csv")
                        or file_path.name.endswith(".tsv")
                    ):
                        continue
                    ...
                    pandas_result = self.profiler.profile_loading_operation(
                        "pandas_csv_load", self._load_csv_pandas, file_path, separator
                    )

                    # Test chunked loading
                    chunked_result = self.profiler.profile_loading_operation(
                        "chunked_csv_load", self._load_csv_chunked, file_path, separator
                    )
            ...
            def _load_csv_chunked(
                ...
                for chunk in pd.read_csv(
                    file_path, sep=separator, index_col=0, chunksize=chunk_size
                ):
                    chunks.append(chunk)

                return chunks

            def _load_csv_optimized(
                self, file_path: Path, separator: str
            ) -> pd.DataFrame:
        ...
        csv_tsv_results = csv_tsv_benchmark.benchmark_csv_tsv_operations(
        ...
        if csv_tsv_results["csv_tsv_benchmark_results"]:
            ...
            pandas_stats = performance_summary["operation_breakdown"].get(
                "pandas_csv_load", {}
            )
            if pandas_stats:
    ...
    def test_sparse_matrix_loading_performance(self, benchmark_test_files):
            ...
            def benchmark_sparse_operations(self, files: List[Path]):
                ...
                for file_path in files:
                    ...
                    scipy_result = self.profiler.profile_loading_operation(
                        "scipy_mtx_load", self._load_mtx_scipy, file_path
                    )

                    # Test scanpy loading
                    scanpy_result = self.profiler.profile_loading_operation(
                        "scanpy_mtx_load", self._load_mtx_scanpy, file_path
                    )

                    # Test format conversion
                    conversion_result = self.profiler.profile_loading_operation(
                        "mtx_format_conversion", self._convert_mtx_formats, file_path
                    )
        ...
        sparse_results = sparse_benchmark.benchmark_sparse_operations(
            benchmark_test_files
        )
            ...
            def benchmark_hdf5_operations(self, files: List[Path]):
                ...
                for file_path in files:
                    ...
                    h5py_result = self.profiler.profile_loading_operation(
                        "h5py_load", self._load_hdf5_h5py, file_path
                    )

                    # Test partial loading
                    partial_result = self.profiler.profile_loading_operation(
                        "h5py_partial_load", self._load_hdf5_partial, file_path
                    )
...
class TestDataStreamingPerformance:
    ...
    def test_streaming_data_processing(
        self, benchmark_workspace, benchmark_data_manager
    ):
        ...
        class StreamingDataProcessor:
            ...
            def _process_data_batches(self, config: Dict):
                ...
                for batch_start in range(0, total_samples, batch_size):
                    ...
                    batch_data = np.random.randn(
                        batch_size_actual, config.get("n_features", 100)
                    )
            ...
            def _process_continuous_stream(self, config: Dict):
                ...
                while (time.time() - start_time) < stream_duration:
                    ...
                    stream_data = np.random.randn(
                        n_samples, config.get("n_features", 100)
                    )
                ...
                return {
                    ...
                    "throughput_samples_per_second": processed_samples
                    / actual_duration,
            ...
            def _process_buffered_stream(self, config: Dict):
                ...
                return {
                    ...
                    "avg_buffer_utilization": (
                        total_processed / (processed_buffers * buffer_size)
        ...
        for result in streaming_results["streaming_results"]:
            ...
            if "batch_processing" in result and result["batch_processing"]["success"]:
                ...
                assert (
                    batch_result["total_batches"] > 0
                ), f"No batches processed for {config_name}"

            if (
                "continuous_streaming" in result
                and result["continuous_streaming"]["success"]
            ):
                stream_result = result["continuous_streaming"]["result"]
                assert (
                    stream_result["throughput_samples_per_second"] > 0
                ), f"No throughput for {config_name}"

            if (
                "buffered_processing" in result
                and result["buffered_processing"]["success"]
            ):
                buffer_result = result["buffered_processing"]["result"]
                assert (
                    buffer_result["processed_buffers"] > 0
                ), f"No buffers processed for {config_name}"

        return streaming_results

    def test_concurrent_file_loading(
        self, benchmark_test_files, benchmark_data_manager
    ):
        ...
        class ConcurrentFileLoader:
            ...
            def benchmark_concurrent_loading(
                ...
                return {
                    ...
                    "success_rate": (
                        len(successful_loads) / len(results) if results else 0
                    ),
                    "total_execution_time": concurrent_end - concurrent_start,
                    "total_file_size_mb": total_file_size,
                    "total_load_time": total_load_time,
                    "avg_load_time": (
                        total_load_time / len(successful_loads)
                        if successful_loads
                        else 0
                    ),
                    "throughput_mb_per_second": total_file_size
        ...
        for max_workers in worker_counts:
            result = concurrent_loader.benchmark_concurrent_loading(
                benchmark_test_files, max_workers=max_workers
            )
        ...
        for worker_count, result in concurrent_results.items():
            ...
            ), f"No throughput for {worker_count}"
            assert (
                result["total_execution_time"] < 300.0
            ), f"Concurrent loading too slow for {worker_count}"

        # Compare performance across worker counts
        throughputs = [
            result["throughput_mb_per_second"] for result in concurrent_results.values()
        ]
...
class TestCachingAndValidationPerformance:
    ...
    def test_data_caching_performance(
        ...
        class DataCachingBenchmark:
            ...
            def _benchmark_cache_write(self, config: Dict):
                ...
                if data_size == "small":
                    ...
                    test_data = SingleCellDataFactory(
                        config={**SMALL_DATASET_CONFIG, "n_obs": 10000, "n_vars": 5000}
                    )

                cache_file = (
                    self.cache_dir / f"cache_test_{config['name']}.{cache_format}"
                )
        ...
        caching_benchmark = DataCachingBenchmark(
            benchmark_data_manager, benchmark_workspace
        )
        ...
        for result in caching_results["caching_results"]:
            ...
            if result["write_performance"]["success"]:
                assert (
                    result["write_performance"]["execution_time"] < 30.0
                ), f"Cache write too slow for {config_name}"

            # Read performance should be fast
            if result["read_performance"]["success"]:
                assert (
                    result["read_performance"]["execution_time"] < 10.0
                ), f"Cache read too slow for {config_name}"
    ...
    def test_data_validation_performance(
        ...
        class DataValidationBenchmark:
            ...
            def benchmark_validation_operations(self, files: List[Path]):
                ...
                for file_path in files:
                    ...
                    schema_result = self.profiler.profile_loading_operation(
                        "schema_validation", self._validate_data_schema, file_path
                    )

                    # Test integrity validation
                    integrity_result = self.profiler.profile_loading_operation(
                        "integrity_validation", self._validate_data_integrity, file_path
                    )
            ...
            def _validate_data_integrity(self, file_path: Path):
                ...
                try:
                    ...
                    return {"valid": overall_integrity, "checks": integrity_checks}
        ...
        validation_results = validation_benchmark.benchmark_validation_operations(
            benchmark_test_files
        )
        ...
        for operation, stats in performance_summary["operation_breakdown"].items():
            assert (
                stats["avg_time"] < 10.0
            ), f"Validation operation {operation} too slow"
