...
class PerformanceMetrics:
    ...
    def stop_monitoring(self):
        ...
        return {
            ...
            "memory_efficiency": (
                (memory_increase / execution_time) if execution_time > 0 else 0
            ),
        }
...
def create_large_dataset(
    n_obs: int, n_vars: int, sparse_ratio: float = 0.8
) -> ad.AnnData:
    ...
    obs_data = {
        ...
        "cell_type": np.random.choice(
            ["T_cells", "B_cells", "NK_cells", "Monocytes", "Dendritic_cells"], n_obs
        ),
...
class TestLargeDatasetLoading:
    ...
    @memory_benchmark
    def test_large_h5ad_loading_performance(
        self, temp_performance_workspace, large_single_cell_data
    ):
        ...
        # Save and load test
        io_results = loader.save_and_load_h5ad(
            large_single_cell_data, "large_dataset_performance"
        )

        # Verify performance benchmarks
        assert (
            io_results["save_time"] < 60.0
        ), f"Save time too slow: {io_results['save_time']}s"
        assert (
            io_results["load_time"] < 30.0
        ), f"Load time too slow: {io_results['load_time']}s"
        ...
        return {"io_performance": io_results, "chunked_performance": chunked_results}

    def test_memory_efficient_dataset_creation(
        self, large_dataset_sizes, performance_data_manager
    ):
        ...
        class MemoryEfficientCreator:
            ...
            def create_dataset_with_monitoring(self, n_obs, n_vars, modality_name):
                ...
                for start_idx in range(0, n_obs, chunk_size):
                    ...
                    # Create chunk
                    chunk_adata = create_large_dataset(
                        chunk_n_obs, n_vars, sparse_ratio=0.85
                    )

                    # Adjust cell IDs to maintain uniqueness
                    chunk_adata.obs.index = [
                        f"CELL_{i+start_idx:07d}" for i in range(chunk_n_obs)
                    ]
            ...
            def _calculate_memory_efficiency(self, adata, performance_data):
                ...
                theoretical_memory_gb = (adata.X.nnz * 8) / (
                    1024**3
                )  # 8 bytes per non-zero
        ...
        creation_result = creator.create_dataset_with_monitoring(
            n_obs, n_vars, modality_name
        )
        ...
        ), f"Creation too slow: {metrics['execution_time']}s"
        assert (
            metrics["memory_increase_gb"] < 10.0
        ), f"Memory usage too high: {metrics['memory_increase_gb']}GB"

        # Memory efficiency should be reasonable
        assert (
            creation_result["memory_efficiency_score"] > 0.1
    ...
    def test_concurrent_dataset_loading(
        self, temp_performance_workspace, performance_data_manager
    ):
        ...
        class ConcurrentLoader:
            ...
            def test_concurrent_loading(self, prepared_files, max_workers=3):
                ...
                return {
                    ...
                    "average_loading_time": np.mean(
                        [r["loading_time"] for r in results if r["success"]]
                    ),
                    "total_memory_increase": sum(
                        [r.get("memory_increase", 0) for r in results if r["success"]]
                    ),
                }
        ...
        # Test concurrent loading
        concurrent_results = loader.test_concurrent_loading(
            prepared_files, max_workers=3
        )
        ...
        assert (
            concurrent_results["total_loading_time"] < 120.0
        ), "Concurrent loading too slow"
        assert (
            concurrent_results["average_loading_time"] < 60.0
        ), "Average loading time too slow"
...
class TestLargeScaleProcessing:
    ...
    @memory_benchmark
    def test_large_scale_preprocessing_performance(
        self, large_single_cell_data, performance_services
    ):
        ...
        class LargeScalePreprocessor:
            ...
            def preprocess_with_monitoring(self, adata, processing_params):
                ...
                # Filter cells
                sc.pp.filter_cells(
                    adata, min_genes=processing_params.get("min_genes", 200)
                )
                adata = adata[
                    adata.obs["pct_counts_mt"]
                    < processing_params.get("max_mt_percent", 20),
                    :,
                ]
                ...
                sc.pp.normalize_total(
                    adata, target_sum=processing_params.get("target_sum", 1e4)
                )
                ...
                preprocessing_steps.append(
                    {
                        ...
                        "explained_variance": float(
                            adata_hvg.uns["pca"]["variance_ratio"][:10].sum()
                        ),
                    }
                )
                ...
                return {
                    ...
                    "total_execution_time": sum(
                        step["execution_time"] for step in preprocessing_steps
                    ),
                    "total_memory_increase": sum(
                        step["memory_increase"] for step in preprocessing_steps
                    ),
                    "final_dataset_shape": adata_hvg.shape,
                }

        # Test large-scale preprocessing
        preprocessor = LargeScalePreprocessor(
            performance_services["preprocessing_service"]
        )
        ...
        preprocessing_result = preprocessor.preprocess_with_monitoring(
            large_single_cell_data.copy(), processing_params
        )
        ...
        assert (
            preprocessing_result["total_execution_time"] < 300.0
        ), "Preprocessing too slow"
        assert (
            preprocessing_result["total_memory_increase"] < 15.0
    ...
    def test_large_scale_clustering_performance(
        self, large_single_cell_data, performance_services
    ):
        ...
        class LargeScaleClusterer:
            ...
            def cluster_with_monitoring(self, adata, clustering_params):
                ...
                for resolution in resolutions:
                    sc.tl.leiden(
                        adata, resolution=resolution, key_added=f"leiden_{resolution}"
                    )
                    ...
                    clustering_results[f"leiden_{resolution}"] = {
                        ...
                        "largest_cluster_size": adata.obs[f"leiden_{resolution}"]
                        .value_counts()
                        .max(),
                        "smallest_cluster_size": adata.obs[f"leiden_{resolution}"]
                        .value_counts()
                        .min(),
                    }
                ...
                return {
                    ...
                    "performance_metrics": performance_data,
                    "total_execution_time": sum(
                        step["execution_time"] for step in clustering_steps
                    ),
                }
                    ...
                    if clustering_key in adata.obs:
                        ...
                        quality_metrics[clustering_key] = {
                            ...
                            "largest_cluster_fraction": float(
                                cluster_sizes.max() / len(clusters)
                            ),
                        }
        ...
        clustering_result = clusterer.cluster_with_monitoring(
            large_single_cell_data.copy(), clustering_params
        )
        ...
        for resolution in clustering_params["resolutions"]:
            ...
            assert (
                clustering_key in clustered_adata.obs
            ), f"Clustering {clustering_key} not found"
            ...
            assert (
                quality["n_clusters"] < clustered_adata.n_obs // 10
            ), f"Too many clusters for {clustering_key}"
            assert (
                quality["largest_cluster_fraction"] < 0.8
            ), f"Dominant cluster too large for {clustering_key}"

        return clustering_result

    def test_memory_usage_scaling(
        self, extra_large_single_cell_data, temp_performance_workspace
    ):
        ...
        class MemoryScalingAnalyzer:
            ...
            def test_scaling_with_subsampling(self, full_adata, subsample_fractions):
                ...
                for fraction in subsample_fractions:
                    ...
                    subsample_indices = np.random.choice(
                        full_adata.n_obs, size=n_cells_subsample, replace=False
                    )
                    ...
                    scaling_results.append(
                        {
                            ...
                            "memory_overhead_ratio": (
                                actual_memory_mb / theoretical_memory_mb
                                if theoretical_memory_mb > 0
                                else 0
                            ),
            ...
            def _analyze_scaling_patterns(self, results):
                ...
                return {
                    ...
                    "max_memory_overhead": max(
                        [r["memory_overhead_ratio"] for r in results]
                    ),
                    "min_memory_overhead": min(
                        [r["memory_overhead_ratio"] for r in results]
                    ),
        ...
        scaling_result = analyzer.test_scaling_with_subsampling(
            extra_large_single_cell_data, subsample_fractions
        )
        ...
        assert (
            scaling_analysis["max_memory_overhead"] < 10.0
        ), "Memory overhead too high"
        assert (
            scaling_analysis["avg_memory_overhead"] > 0.5
        ), "Memory overhead too low (suspicious)"

        # Time scaling should be reasonable (sub-quadratic)
        largest_dataset = max(
            scaling_result["scaling_results"], key=lambda x: x["n_cells"]
        )
        assert (
            largest_dataset["execution_time"] < 600.0
        ), "Largest dataset processing too slow"
