...
class PerformanceMonitor:
    ...
    def stop_monitoring(self) -> PerformanceMetrics:
        ...
        return PerformanceMetrics(
            ...
            disk_io_read_mb=self.metrics[-1]["disk_read_mb"]
            - self.metrics[0]["disk_read_mb"],
            disk_io_write_mb=self.metrics[-1]["disk_write_mb"]
            - self.metrics[0]["disk_write_mb"],
        )
...
class EnhancedLobsterTestRunner:
    ...
    def __init__(
        self, log_level: str = "INFO", enable_performance_monitoring: bool = True
    ):
    ...
    def load_test_cases(self, input_file: Path) -> Dict[str, TestCase]:
        ...
        try:
                ...
                for test_id, test_data in raw_data.items():
                    if isinstance(test_data, dict):
                        test_cases[test_id] = TestCase(
                            ...
                            validation_criteria=test_data.get(
                                "validation_criteria", {}
                            ),
                        )
                    else:
            ...
            self.logger.info(
                f"Loaded {len(test_cases)} enhanced test cases from {input_file}"
            )
            ...
            for test_case in test_cases.values():
                categories[test_case.category] = (
                    categories.get(test_case.category, 0) + 1
                )
    ...
    def _infer_category(self, test_id: str) -> str:
        ...
        if any(
            keyword in test_id.lower()
            for keyword in ["advanced", "complex", "integration"]
        ):
            return "advanced"
        elif any(
            keyword in test_id.lower()
            ...
            keyword in test_id.lower() for keyword in ["error", "failure", "invalid"]
        ):
    ...
    def resolve_dependencies(self, test_cases: Dict[str, TestCase]) -> List[str]:
        ...
        while remaining:
            ...
            if not ready_tests:
                ...
                self.logger.warning(
                    f"Circular or missing dependencies detected for: {list(remaining.keys())}"
                )
    ...
    def run_single_test_enhanced(
        ...
        if workspace_dir is None:
            workspace_dir = Path(
                tempfile.mkdtemp(prefix=f"lobster_test_{test_case.test_id}_")
            )
            self._workspace_cleanup_registry.append(workspace_dir)

        try:
            self.logger.info(
                f"Starting test '{test_case.test_id}' (attempt {retry_attempt + 1})"
            )
            ...
            for i, user_input in enumerate(test_case.inputs):
                try:
                    self.logger.info(
                        f"Test '{test_case.test_id}' - Input {i+1}/{len(test_case.inputs)}: {user_input[:100]}..."
                    )
                    ...
                    if result.get("success", False):
                        ...
                        self.logger.info(
                            f"Test '{test_case.test_id}' - Response {i+1}: SUCCESS"
                        )
                        ...
                        if test_case.validation_criteria:
                            ...
                            if not validation_result.get("valid", True):
                                self.logger.warning(
                                    f"Validation failed for input {i+1}: {validation_result}"
                                )
                        ...
                        self.logger.error(
                            f"Test '{test_case.test_id}' - Response {i+1}: FAILED - {error_msg}"
                        )
                    ...
                    error_message = error_msg
                    self.logger.error(
                        f"Test '{test_case.test_id}' - Response {i+1}: EXCEPTION - {error_msg}"
                    )
            ...
            if duration > test_case.expected_duration * 1.5:
                self.logger.warning(
                    f"Test '{test_case.test_id}' took longer than expected: {duration:.2f}s > {test_case.expected_duration:.2f}s"
                )
            ...
            duration = (datetime.now() - start_time).total_seconds()
            error_msg = (
                f"Test setup/execution failed: {str(e)}\n{traceback.format_exc()}"
            )
    ...
    def run_tests_enhanced(
        ...
        self.logger.info(
            f"Running {len(test_cases)} tests {'in parallel' if parallel else 'sequentially'}"
        )

        if parallel:
            return self._run_tests_parallel_enhanced(
                test_cases, execution_order, max_workers
            )
        else:
            return self._run_tests_sequential_enhanced(test_cases, execution_order)

    def _run_tests_sequential_enhanced(
        self, test_cases: Dict[str, TestCase], execution_order: List[str]
    ) -> Dict[str, TestResult]:
        ...
        for test_id in execution_order:
            if test_id not in test_cases:
                self.logger.warning(
                    f"Test '{test_id}' in execution order but not in test cases"
                )
                continue
            ...
            self.logger.info(
                f"Test '{test_id}' [{result.category}]: {status} ({result.duration_seconds:.2f}s){perf_info}"
            )

            # Early termination on critical failures
            if not result.success and test_case.priority >= 9:  # High priority failure
                self.logger.error(
                    f"Critical test '{test_id}' failed, considering early termination"
                )

        return results

    def _run_tests_parallel_enhanced(
        self,
        test_cases: Dict[str, TestCase],
        ...
        dependency_levels = self._group_by_dependency_levels(
            test_cases, execution_order
        )

        for level, test_ids in dependency_levels.items():
            self.logger.info(
                f"Executing dependency level {level} with {len(test_ids)} tests"
            )

            level_test_cases = {
                tid: test_cases[tid] for tid in test_ids if tid in test_cases
            }

            with ThreadPoolExecutor(
                max_workers=min(max_workers, len(level_test_cases))
            ) as executor:
                future_to_test = {
                ...
                for future in as_completed(future_to_test):
                    ...
                    try:
                        ...
                        status = "PASSED" if result.success else "FAILED"
                        self.logger.info(
                            f"Test '{test_id}' [{result.category}]: {status} ({result.duration_seconds:.2f}s)"
                        )
    ...
    def _execute_test_with_retry(self, test_case: TestCase) -> TestResult:
        ...
        for attempt in range(test_case.retry_count + 1):
            ...
            if attempt < test_case.retry_count:
                self.logger.warning(
                    f"Test '{test_case.test_id}' failed attempt {attempt + 1}, retrying..."
                )
                time.sleep(2**attempt)  # Exponential backoff

        return last_result

    def _group_by_dependency_levels(
        self, test_cases: Dict[str, TestCase], execution_order: List[str]
    ) -> Dict[int, List[str]]:
        """Group tests by dependency levels for parallel execution."""
        levels = {}
            ...
            test_commands = [
                (
                    ...
                    [
                        ...
                        "--tb=short",
                        "--json-report",
                        "--json-report-file=unit_results.json",
                    ],
                ),
                (
                    "integration",
                ...
                (
                    ...
                    [
                        ...
                        "--tb=short",
                        "--json-report",
                        "--json-report-file=system_results.json",
                    ],
                ),
                (
                    "performance",
                    ...
                    if json_file.exists():
                        try:
                            with open(json_file, "r") as f:
                                pytest_results[category]["detailed_results"] = (
                                    json.load(f)
                                )
    ...
    def run_tests_parallel(
        self, test_cases: Dict[str, List[str]], max_workers: int = 4
    ) -> Dict[str, Any]:
        """Run tests in parallel using multiprocessing."""
        self.logger.info(
            f"Running {len(test_cases)} tests in parallel with {max_workers} workers"
        )
        ...
        for test_id in test_cases.keys():
            workspaces[test_id] = Path(
                tempfile.mkdtemp(prefix=f"lobster_test_{test_id}_")
            )
            ...
            with ProcessPoolExecutor(max_workers=max_workers) as executor:
                ...
                for future in as_completed(future_to_test):
                    ...
                    try:
                        ...
                        status = "PASSED" if result["success"] else "FAILED"
                        self.logger.info(
                            f"Test '{test_id}' completed: {status} ({result['duration_seconds']:.2f}s)"
                        )

                    except Exception as e:
                        error_msg = f"Process execution failed: {str(e)}"
                        self.logger.error(
                            f"Test '{test_id}' process error: {error_msg}"
                        )
                        results[test_id] = {
                            "test_id": test_id,
                            "success": False,
    ...
    def save_results(self, results: Dict[str, Any], output_file: Path):
        ...
        try:
            ...
            self.logger.info(
                f"Summary: {summary['passed_tests']}/{summary['total_tests']} tests passed"
            )
    ...
    def save_enhanced_results(
        ...
        try:
            ...
            if any(r.performance_metrics for r in results.values()):
                cpu_values = [
                    r.performance_metrics.get("cpu_percent_avg", 0)
                    for r in results.values()
                    if r.performance_metrics
                ]
                memory_values = [
                    r.performance_metrics.get("memory_mb_peak", 0)
                    for r in results.values()
                    if r.performance_metrics
                ]
            ...
            output_data = {"summary": summary, "manual_test_results": results_dict}
            ...
            if pytest_results:
                ...
                pytest_success_count = sum(
                    1 for r in pytest_results.values() if r.get("success", False)
                )
            ...
            self.logger.info(
                f"Summary: {passed_tests}/{total_tests} manual tests passed"
            )
            ...
            if pytest_results:
                ...
                self.logger.info(
                    f"Pytest: {pytest_passed}/{len(pytest_results)} test suites passed"
                )
...
class LobsterTestRunner(EnhancedLobsterTestRunner):
    ...
    def run_single_test(
        self, test_id: str, user_inputs: List[str], workspace_dir: Optional[Path] = None
    ) -> Dict[str, Any]:
...
def run_single_test_worker(
    test_id: str, user_inputs: List[str], workspace_dir: Path
) -> Dict[str, Any]:
  ...
  python tests/run_integration_tests.py --tags geo,analysis --priorities 1,2,3
    ...
    try:
        ...
        manual_results = runner.run_tests_enhanced(
            filtered_test_cases, parallel=args.parallel, max_workers=args.workers
        )
        ...
        if pytest_results:
            pytest_failed = sum(
                1 for r in pytest_results.values() if not r.get("success", False)
            )
        ...
        if total_failed > 0:
            runner.logger.error(
                f"{total_failed} tests failed! (Manual: {manual_failed}, Pytest: {pytest_failed})"
            )
            sys.exit(1)
        else:
