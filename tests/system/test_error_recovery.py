...
class ErrorInjector:
    ...
    @contextmanager
    def inject_error(
        self, error_type: str, error_location: str, error_config: Dict[str, Any]
    ):
    ...
    def simulate_network_error(self, duration: float = 5.0):
        ...
        return self.inject_error(
            "network", "connection", {"duration": duration, "type": "timeout"}
        )
    ...
    def simulate_memory_error(self, trigger_at_mb: int = 1000):
        ...
        return self.inject_error(
            "memory", "allocation", {"trigger_threshold_mb": trigger_at_mb}
        )
    ...
    def simulate_agent_failure(self, agent_type: str, failure_mode: str = "crash"):
        ...
        return self.inject_error(
            "agent_failure", agent_type, {"failure_mode": failure_mode}
        )
...
class RecoveryValidator:
    ...
    def capture_system_state(self, state_name: str):
        ...
        state_info = {
            ...
            "workspace_files": [],
            "provenance_entries": (
                len(self.data_manager.provenance_tracker.operations)
                if hasattr(self.data_manager, "provenance_tracker")
                else 0
            ),
        }
        ...
        if (
            hasattr(self.data_manager, "workspace_path")
            and self.data_manager.workspace_path.exists()
        ):
                        ...
                        state_info["workspace_files"].append(
                            {
                                "path": str(
                                    file_path.relative_to(
                                        self.data_manager.workspace_path
                                    )
                                ),
    ...
    def validate_data_integrity(self) -> Dict[str, Any]:
        ...
        if unexpected_modalities:
            validation_results["issues"].append(
                f"Unexpected modalities: {unexpected_modalities}"
            )
            ...
            if "error" in pre_info or "error" in post_info:
                ...
                if pre_info.get("shape") != post_info.get("shape"):
                    modality_issues.append(
                        f"Shape changed: {pre_info.get('shape')} -> {post_info.get('shape')}"
                    )
                    validation_results["integrity_preserved"] = False
        ...
        if missing_files:
            validation_results["issues"].append(
                f"Missing workspace files: {missing_files}"
            )
            validation_results["integrity_preserved"] = False
...
class ErrorRecoveryTester:
    ...
    def _setup_test_data(self):
        ...
        clustered_data.obs["leiden"] = np.random.randint(
    ...
    def _inject_scenario_error(self, scenario: ErrorScenario):
        ...
        if scenario.error_type == "network":
            ...
            with self.error_injector.simulate_agent_failure(
                scenario.error_location
            ) as error_id:
                yield error_id
        else:
            # Generic error injection
            with self.error_injector.inject_error(
                scenario.error_type, scenario.error_location, {}
            ) as error_id:
                yield error_id
            ...
            for modality_name in modalities:
                try:
                    ...
                    if adata.X is None or adata.n_obs == 0 or adata.n_vars == 0:
                        ...
                        restored_adata = SingleCellDataFactory(
    ...
    def _get_recovery_details(self, scenario: ErrorScenario) -> Dict[str, Any]:
        ...
        return {
            ...
            "system_state_after_recovery": (
                "functional"
                if len(self.data_manager.list_modalities()) > 0
                else "degraded"
            ),
        }
...
class TestErrorRecoveryMechanisms:
    ...
    def test_network_error_recovery(self, error_recovery_tester, error_scenarios):
        ...
        network_scenario = next(
            s for s in error_scenarios if s.scenario_id == "network_timeout"
        )
        ...
        assert (
            result["recovery_time"] <= network_scenario.recovery_time_limit
        ), f"Recovery took too long: {result['recovery_time']}s"
        assert (
            result["success"] == True
        ), f"Overall scenario failed: {result['error_details']}"

        # Verify system remains functional
        assert (
            len(error_recovery_tester.data_manager.list_modalities()) > 0
        ), "No data available after recovery"
    ...
    def test_disk_error_recovery(self, error_recovery_tester, error_scenarios):
        ...
        assert (
            result["recovery_time"] <= disk_scenario.recovery_time_limit
        ), f"Recovery took too long: {result['recovery_time']}s"
        assert (
            result["success"] == True
        ), f"Overall scenario failed: {result['error_details']}"
        ...
        test_file = (
            error_recovery_tester.data_manager.workspace_path / "post_recovery_test.txt"
        )
        test_file.write_text("test")
        assert (
            test_file.exists()
        ), "File operations not working after disk error recovery"
        test_file.unlink()
    ...
    def test_memory_error_recovery(self, error_recovery_tester, error_scenarios):
        ...
        memory_scenario = next(
            s for s in error_scenarios if s.scenario_id == "memory_exhaustion"
        )
        ...
        assert (
            result["recovery_time"] <= memory_scenario.recovery_time_limit
        ), f"Recovery took too long: {result['recovery_time']}s"
        assert (
            result["success"] == True
        ), f"Overall scenario failed: {result['error_details']}"
    ...
    def test_data_corruption_recovery(self, error_recovery_tester, error_scenarios):
        ...
        corruption_scenario = next(
            s for s in error_scenarios if s.scenario_id == "data_corruption"
        )
        ...
        assert (
            result["recovery_time"] <= corruption_scenario.recovery_time_limit
        ), f"Recovery took too long: {result['recovery_time']}s"
        assert (
            result["success"] == True
        ), f"Overall scenario failed: {result['error_details']}"
        ...
        for modality_name in modalities:
            try:
                ...
                assert (
                    adata.shape[0] > 0 and adata.shape[1] > 0
                ), f"Invalid data shape for {modality_name}"
            except Exception as e:
                pytest.fail(
                    f"Cannot access modality {modality_name} after recovery: {e}"
                )

    def test_agent_failure_recovery(self, error_recovery_tester, error_scenarios):
        """Test recovery from agent failure scenarios."""
        agent_scenario = next(
            s for s in error_scenarios if s.scenario_id == "agent_crash"
        )
        ...
        assert (
            result["recovery_time"] <= agent_scenario.recovery_time_limit
        ), f"Recovery took too long: {result['recovery_time']}s"
        assert (
            result["success"] == True
        ), f"Overall scenario failed: {result['error_details']}"
        ...
        assert recovery_details["system_state_after_recovery"] in [
            "functional",
            "degraded",
        ], "Unknown system state after recovery"
    ...
    def test_cascading_error_recovery(self, error_recovery_tester):
                    ...
                    with self.base_tester.error_injector.simulate_memory_error():
                        with self.base_tester.error_injector.simulate_disk_error():
                            ...
                            try:
                                ...
                                if modalities:
                                    test_adata = self.data_manager.get_modality(
                                        modalities[0]
                                    )
                                ...
                                # Test recovery
                                post_error_modalities = set(
                                    self.data_manager.list_modalities()
                                )

                                recovery_success = (
                                    len(post_error_modalities)
                                    >= len(initial_modalities) * 0.8
                                )
                                recovery_time = time.time() - recovery_start
                                ...
                                recovery_results.append(
                                    {
                                        ...
                                        "recovered_modalities": len(
                                            post_error_modalities
                                        ),
                                        "data_loss_percentage": (
                                            max(
                                                0,
                                                (
                                                    len(initial_modalities)
                                                    - len(post_error_modalities)
                                                )
                                                / len(initial_modalities)
                                                * 100,
                                            )
                                            if initial_modalities
                                            else 0
                                        ),
                                    }
                                )
            ...
            def test_sequential_error_chain(self):
                ...
                for error_type, duration in error_chain:
                    ...
                    try:
                        if error_type == "network":
                            with self.base_tester.error_injector.simulate_network_error(
                                duration
                            ):
                            ...
                            with (
                                self.base_tester.error_injector.simulate_memory_error()
                            ):
                                time.sleep(duration)
                            ...
                            with (
                                self.base_tester.error_injector.simulate_corruption_error()
                            ):
                                time.sleep(duration)
                ...
                return {
                    ...
                    "final_system_state": (
                        "functional"
                        if chain_results
                        and chain_results[-1].get("system_functional", False)
                        else "failed"
                    ),
                }
        ...
        for result in simultaneous_results:
            if result.get("success", False):
                assert (
                    result["recovery_time"] < 30.0
                ), f"Simultaneous error recovery too slow: {result['recovery_time']}s"
                assert (
                    result["data_loss_percentage"] < 50
                ), f"Excessive data loss: {result['data_loss_percentage']}%"
        ...
        assert (
            chain_results["total_recovery_time"] < 60.0
        ), f"Chain recovery too slow: {chain_results['total_recovery_time']}s"

        # At least partial functionality should be maintained
        functional_steps = sum(
            1
            for r in chain_results["chain_results"]
            if r.get("system_functional", False)
        )
        assert (
            functional_steps >= len(chain_results["chain_results"]) // 2
        ), "System lost functionality too often during error chain"
    ...
    def test_graceful_degradation(self, error_recovery_tester):
        ...
        class GracefulDegradationTester:
            ...
            def test_partial_functionality_preservation(self):
                ...
                for i in range(3):
                    ...
                    self.data_manager.modalities[f"additional_dataset_{i}"] = (
                        additional_data
                    )
                ...
                for severity in error_severities:
                    ...
                    if severity == "mild":
                        ...
                        with self.base_tester.error_injector.simulate_network_error(
                            ...
                            with (
                                ...
                                with (
                                    self.base_tester.error_injector.simulate_disk_error()
                                ):
                    ...
                    data_retention = (
                        len(current_modalities) / len(initial_modalities)
                        if initial_modalities
                        else 0
                    )
                    ...
                    degradation_results.append(
                        {
                            ...
                            "functionality_preserved": functionality_result[
                                "core_functions_working"
                            ],
                            "data_retention_percentage": data_retention * 100,
                            "accessible_operations": functionality_result[
                                "accessible_operations"
                            ],
                            "failed_operations": functionality_result[
                                "failed_operations"
                            ],
                ...
                return {
                    ...
                    "overall_resilience": self._assess_overall_resilience(
                        degradation_results
                    ),
                }
            ...
            def _test_core_functionality(self):
                ...
                core_functions = {
                    ...
                    "get_modality": lambda: (
                        self.data_manager.get_modality(
                            self.data_manager.list_modalities()[0]
                        )
                        if self.data_manager.list_modalities()
                        else None
                    ),
                ...
                return {
                    "core_functions_working": len(accessible_operations)
                    >= len(core_functions) // 2,
                    "accessible_operations": accessible_operations,
                    "failed_operations": failed_operations,
                    "functionality_percentage": len(accessible_operations)
                    / len(core_functions)
                    * 100,
                }
            ...
            def _test_workspace_access(self):
                ...
                try:
                    if (
                        ...
                        test_file = (
                            self.data_manager.workspace_path / "degradation_test.tmp"
                        )
                        test_file.write_text("test")
            ...
            def _calculate_degradation_score(
                self, functionality_result, data_retention
            ):
            ...
            def _assess_overall_resilience(self, degradation_results):
                ...
                avg_degradation_score = np.mean(
                    [r["graceful_degradation_score"] for r in degradation_results]
                )

                # Check if functionality is preserved even under severe errors
                severe_results = [
                    r for r in degradation_results if r["error_severity"] == "severe"
                ]
                severe_functionality = (
                    any(r["functionality_preserved"] for r in severe_results)
                    if severe_results
                    else False
                )
        ...
        assert (
            len(results["degradation_test_results"]) == 3
        ), "Not all severity levels tested"

        # Under mild errors, most functionality should be preserved
        mild_result = next(
            r
            for r in results["degradation_test_results"]
            if r["error_severity"] == "mild"
        )
        assert (
            mild_result["functionality_preserved"] == True
        ), "Functionality not preserved under mild errors"
        assert (
            mild_result["graceful_degradation_score"] >= 70
        ), f"Poor degradation score under mild errors: {mild_result['graceful_degradation_score']}"

        # Even under severe errors, some functionality should remain
        severe_result = next(
            r
            for r in results["degradation_test_results"]
            if r["error_severity"] == "severe"
        )
        assert (
            len(severe_result["accessible_operations"]) > 0
        ), "No operations accessible under severe errors"

        # Overall resilience should be reasonable
        resilience = results["overall_resilience"]
        assert resilience["resilience"] in [
            "moderate",
            "good",
            "excellent",
        ], f"Poor overall resilience: {resilience['resilience']}"
