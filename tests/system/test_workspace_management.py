...
class WorkspaceAnalyzer:
    ...
    def analyze_workspace(self, workspace_path: Path) -> WorkspaceState:
        ...
        export_files = (
            self._collect_files(workspace_path / "exports", pattern="*")
            if (workspace_path / "exports").exists()
            else []
        )
        cache_files = (
            self._collect_files(workspace_path / "cache", pattern="*")
...
class WorkspaceManager:
    ...
    def create_workspace(
        self, workspace_path: Path, initialize_structure: bool = True
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if initialize_structure:
                ...
                with open(provenance_file, "w") as f:
                    json.dump(
                        {"operations": [], "created": datetime.now().isoformat()}, f
                    )
    ...
    def backup_workspace(
        ...
        try:
            ...
            if compression == "zip":
                ...
                success = self._create_zip_backup(
                    workspace_path, backup_file, include_cache
                )
            elif compression == "tar":
                backup_file = backup_path.with_suffix(".tar.gz")
                success = self._create_tar_backup(
                    workspace_path, backup_file, include_cache
                )
            else:
                backup_file = backup_path
                success = self._create_directory_backup(
                    workspace_path, backup_file, include_cache
                )
            ...
            if success:
                backup_size_mb = (
                    backup_file.stat().st_size / (1024**2)
                ...
                result = {
                    ...
                    "operation_time": operation_time,
                    "compression_ratio": (
                        backup_size_mb / pre_backup_state.total_size_mb
                        if pre_backup_state.total_size_mb > 0
                        else 0
                    ),
    ...
    def restore_workspace(
        self, backup_path: Path, restore_path: Path, overwrite_existing: bool = False
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            elif backup_path.suffix in [".tar", ".gz"] or backup_path.name.endswith(
                ".tar.gz"
            ):
    ...
    def migrate_workspace(
        self, source_path: Path, target_path: Path, migration_type: str = "copy"
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if migration_success and target_path.exists():
                ...
                result = {
                    ...
                    "data_integrity_preserved": self._verify_migration_integrity(
    ...
    def cleanup_workspace(
        self, workspace_path: Path, cleanup_options: Dict[str, bool]
    ) -> Dict[str, Any]:
        ...
        try:
                    ...
                    for temp_file in workspace_path.rglob(pattern):
                        try:
                            ...
                            cleanup_summary["files_removed"].append(
                                str(temp_file.relative_to(workspace_path))
                            )
                            cleanup_summary["space_freed_mb"] += file_size / (1024**2)
                        except Exception as e:
                            cleanup_summary["errors"].append(
                                f"Temp file cleanup error: {str(e)}"
                            )
            ...
            if cleanup_options.get("remove_old_exports", False):
                ...
                if exports_path.exists():
                    cutoff_time = datetime.now() - timedelta(
                        days=cleanup_options.get("export_age_days", 30)
                    )
                        ...
                        if export_file.is_file():
                            try:
                                ...
                                if file_mtime < cutoff_time:
                                    ...
                                    export_file.unlink()
                                    cleanup_summary["files_removed"].append(
                                        str(export_file.relative_to(workspace_path))
                                    )
            ...
            if cleanup_options.get("optimize_storage", False):
                ...
                if data_path.exists():
                    ...
                    cleanup_summary["space_freed_mb"] += optimization_result.get(
                        "space_saved_mb", 0
                    )
            ...
            result = {
                ...
                "space_reduction_percentage": (
                    (
                        (
                            pre_cleanup_state.total_size_mb
                            - post_cleanup_state.total_size_mb
                        )
                        / pre_cleanup_state.total_size_mb
                        * 100
                    )
                    if pre_cleanup_state.total_size_mb > 0
                    else 0
                ),
            }
    ...
    def _create_zip_backup(
        self, workspace_path: Path, backup_file: Path, include_cache: bool
    ) -> bool:
    ...
    def _create_directory_backup(
        self, workspace_path: Path, backup_path: Path, include_cache: bool
    ) -> bool:
            ...
            if include_cache:
                ...
                for item in workspace_path.iterdir():
                    if item.name != "cache":
                        if item.is_dir():
                            shutil.copytree(
                                item, backup_path / item.name, dirs_exist_ok=True
                            )
                        else:
    ...
    def _verify_migration_integrity(
        self, source_state: WorkspaceState, target_state: WorkspaceState
    ) -> bool:
    ...
    def _optimize_data_storage(self, data_path: Path) -> Dict[str, Any]:
        ...
        optimization_result = {"files_processed": 0, "space_saved_mb": 0, "errors": []}
            ...
            for h5ad_file in data_path.glob("*.h5ad"):
                try:
                    ...
                    if new_size < original_size:
                        ...
                        optimization_result["space_saved_mb"] += (
                    ...
                    optimization_result["errors"].append(
                        f"Error optimizing {h5ad_file.name}: {str(e)}"
                    )

        except Exception as e:
            optimization_result["errors"].append(
                f"General optimization error: {str(e)}"
            )
...
def concurrent_workspace_configs():
    ...
    medium_config = replace(
        SMALL_DATASET_CONFIG, default_cell_count=3000, default_gene_count=1500
    )
...
class TestWorkspaceManagement:
    ...
    def test_workspace_creation_and_initialization(
        self, workspace_test_base, workspace_manager
    ):
        """Test workspace creation with proper initialization."""
        workspace_path = workspace_test_base / "test_creation_workspace"

        # Test workspace creation
        creation_result = workspace_manager.create_workspace(
            workspace_path, initialize_structure=True
        )
    ...
    def test_workspace_backup_operations(
        self, sample_workspace_with_data, workspace_test_base, workspace_manager
    ):
        ...
        zip_result = workspace_manager.backup_workspace(
            workspace_path, zip_backup_path, compression="zip", include_cache=False
        )
        ...
        tar_result = workspace_manager.backup_workspace(
            workspace_path, tar_backup_path, compression="tar", include_cache=True
        )
        ...
        dir_result = workspace_manager.backup_workspace(
            workspace_path, dir_backup_path, compression="none", include_cache=False
        )
    ...
    def test_workspace_restore_operations(
        self, sample_workspace_with_data, workspace_test_base, workspace_manager
    ):
        ...
        backup_result = workspace_manager.backup_workspace(
            workspace_path, backup_path, compression="zip", include_cache=True
        )
        ...
        restore_result = workspace_manager.restore_workspace(
            Path(backup_result["backup_path"]), restore_path, overwrite_existing=False
        )
        ...
        if data_path.exists():
            for h5ad_file in data_path.glob("*.h5ad"):
                ...
                restored_data_manager.modalities[modality_name] = ad.read_h5ad(
                    h5ad_file
                )
        ...
        overwrite_success_result = workspace_manager.restore_workspace(
            Path(backup_result["backup_path"]), restore_path, overwrite_existing=True
        )

        assert overwrite_success_result["success"] == True

    def test_workspace_migration_operations(
        self, sample_workspace_with_data, workspace_test_base, workspace_manager
    ):
        ...
        copy_result = workspace_manager.migrate_workspace(
            workspace_path, target_copy_path, migration_type="copy"
        )
        ...
        if data_path.exists():
            for h5ad_file in data_path.glob("*.h5ad"):
                modality_name = h5ad_file.stem
                migrated_data_manager.modalities[modality_name] = ad.read_h5ad(
                    h5ad_file
                )
    ...
    def test_workspace_cleanup_operations(
        self, sample_workspace_with_data, workspace_manager
    ):
        ...
        cleanup_result = workspace_manager.cleanup_workspace(
            workspace_path, cleanup_options
        )
        ...
        selective_result = workspace_manager.cleanup_workspace(
            workspace_path, selective_cleanup_options
        )
        assert selective_result["success"] == True

    def test_workspace_concurrent_operations(
        self, workspace_test_base, concurrent_workspace_configs
    ):
        ...
        class ConcurrentWorkspaceOperator:
            ...
            def execute_concurrent_operations(self, configs: List[Dict]) -> List[Dict]:
                        ...
                        if config["operation_type"] == "create_and_populate":
                            ...
                            if create_result["success"]:
                                ...
                                if backup_result["success"]:
                                    # Restore
                                    restore_path = (
                                        self.base_path / f"{workspace_id}_restored"
                                    )
                                    restore_result = (
                            ...
                            if create_result["success"]:
                                ...
                                # Migrate
                                migrate_path = (
                                    self.base_path / f"{workspace_id}_migrated"
                                )
                                migrate_result = (
                                    self.workspace_manager.migrate_workspace(
                                        workspace_path,
                                        migrate_path,
                                        migration_type="copy",
                                    )
                ...
                return {
                    ...
                    "concurrent_success_rate": sum(1 for r in results if r["success"])
                    / len(results),
                    "operations_summary": {
        ...
        concurrent_results = concurrent_operator.execute_concurrent_operations(
            concurrent_workspace_configs
        )

        # Verify concurrent execution
        assert len(concurrent_results["concurrent_results"]) == len(
            concurrent_workspace_configs
        )
        assert (
        ...
        assert (
            concurrent_results["total_execution_time"] < 180.0
        )  # Should complete within 3 minutes

        # Verify individual operations
        successful_operations = [
            r for r in concurrent_results["concurrent_results"] if r["success"]
        ]
        ...
        for operation_type in [
            ...
            if operation_type in operations_summary:
                ...
                assert (
                    len(successful_type_ops) > 0
                ), f"No successful {operation_type} operations"

    def test_workspace_integrity_validation(
        self, sample_workspace_with_data, workspace_manager
    ):
        ...
        class WorkspaceIntegrityValidator:
            ...
            def validate_workspace_integrity(self) -> Dict[str, Any]:
                ...
                try:
                    ...
                    validation_results["suggestions"] = (
                        self._generate_improvement_suggestions(validation_results)
                    )
            ...
            def _validate_directory_structure(self) -> Dict[str, Any]:
                ...
                for req_dir in required_dirs:
                    ...
                    if not dir_path.exists():
                        ...
                        structure_issues.append(
                            f"Required directory is not a directory: {req_dir}"
                        )
                ...
                for req_file in required_files:
                    ...
                    if not file_path.exists():
                        ...
                        structure_issues.append(
                            f"Required file is not a file: {req_file}"
                        )

                return {
                    "valid": len(structure_issues) == 0,
                    "issues": structure_issues,
                    "required_dirs_present": len(required_dirs)
                    - sum(
                        1
                ...
                if metadata_file.exists():
                    try:
                        ...
                        if metadata.get("version") not in ["1.0", "2.0"]:
                            metadata_issues.append(
                                f"Unsupported workspace version: {metadata.get('version')}"
                            )
                        ...
                        metadata_issues.append(
                            f"Error reading workspace metadata: {str(e)}"
                        )
                ...
                if provenance_file.exists():
                    try:
                        ...
                        if "operations" not in provenance:
                            metadata_issues.append(
                                "Provenance file missing operations field"
                            )

                    except json.JSONDecodeError:
                        metadata_issues.append("Provenance file is not valid JSON")

                return {"valid": len(metadata_issues) == 0, "issues": metadata_issues}
            ...
            def _validate_data_files(self) -> Dict[str, Any]:
                ...
                if data_path.exists():
                    for h5ad_file in data_path.glob("*.h5ad"):
                        try:
                            ...
                            if adata.n_obs == 0:
                                data_issues.append(
                                    f"Data file {h5ad_file.name} has no observations"
                                )

                            if adata.n_vars == 0:
                                data_issues.append(
                                    f"Data file {h5ad_file.name} has no variables"
                                )

                            if adata.X is None:
                                data_issues.append(
                                    f"Data file {h5ad_file.name} missing expression matrix"
                                )
                            ...
                            data_issues.append(
                                f"Cannot read data file {h5ad_file.name}: {str(e)}"
                            )
                            corrupted_files += 1
                ...
                return {
                    ...
                    "corruption_rate": (
                        corrupted_files / (validated_files + corrupted_files)
                        if (validated_files + corrupted_files) > 0
                        else 0
                    ),
                }
            ...
            def _validate_performance_characteristics(self) -> Dict[str, Any]:
                ...
                if cache_path.exists():
                    ...
                    if cache_size > 1000:  # 1GB
                        performance_issues.append(
                            "Cache directory is very large (>1GB)"
                        )
                ...
                return {
                    ...
                    "performance_score": max(
                        0, 100 - len(performance_issues) * 20
                    ),  # Simple scoring
                }
            ...
            def _generate_improvement_suggestions(
                ...
                if "structure" in validation_results["validation_details"]:
                    ...
                    if not structure["valid"]:
                        suggestions.append(
                            "Run workspace repair to fix missing directories/files"
                        )
                ...
                if "data" in validation_results["validation_details"]:
                    ...
                    if data["corruption_rate"] > 0:
                        ...
                        suggestions.append(
                            "Restore corrupted files from backup if available"
                        )
        ...
        if list(data_path.glob("*.h5ad")):
            ...
            assert any(
                "Cannot read data file" in issue
                for issue in corrupted_validation["issues_found"]
            )
            assert (
                corrupted_validation["validation_details"]["data"]["corrupted_files"]
                > 0
            )
