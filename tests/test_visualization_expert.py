...
from lobster.agents.visualization_expert import (
...
class TestVisualizationStateManagement:
    ...
    def test_add_visualization_record(self, mock_data_manager):
        ...
        def mock_add_visualization_record(pid, meta):
            mock_data_manager.visualization_state["history"].append(
                {"plot_id": pid, "timestamp": pd.Timestamp.now(), "metadata": meta}
            )
            mock_data_manager.visualization_state["plot_registry"][pid] = meta

        mock_data_manager.add_visualization_record.side_effect = (
            mock_add_visualization_record
        )
        ...
        mock_data_manager.add_visualization_record.assert_called_once_with(
            plot_id, metadata
        )
    ...
    def test_visualization_settings_management(self, mock_data_manager):
        ...
        assert (
            mock_data_manager.visualization_state["settings"]["default_width"] == 1200
        )
        assert (
            mock_data_manager.visualization_state["settings"]["color_scheme"]
            == "viridis"
        )
...
class TestVisualizationTools:
    ...
    def test_create_umap_plot_with_uuid(
        self, mock_settings, mock_viz_service, mock_data_manager, sample_adata
    ):
    ...
    def test_violin_plot_gene_validation(self, mock_data_manager, sample_adata):
        ...
        available_genes = [
            gene for gene in requested_genes if gene in sample_adata.var_names
        ]
...
class TestErrorHandling:
    ...
    def test_visualization_service_error_handling(
        ...
        with patch(
            ...
            mock_service.return_value.create_umap_plot.side_effect = Exception(
                "Service error"
            )
...
class TestVisualizationReadiness:
    ...
    def test_check_visualization_readiness_complete_data(
        self, mock_data_manager, sample_adata
    ):
...
class TestBackwardCompatibility:
    ...
    def test_plot_metadata_structure(self, mock_data_manager):
        ...
        sample_metadata = {
            ...
            "color_by": "leiden",
            "n_cells": 1000,
            "parameters": {"point_size": 5, "title": None},
        }
...
class TestDataValidation:
    ...
    def test_gene_list_validation(self, sample_adata):
        ...
        available_genes = [
            gene for gene in valid_genes if gene in sample_adata.var_names
        ]
        ...
        available_genes = [
            gene for gene in mixed_genes if gene in sample_adata.var_names
        ]
