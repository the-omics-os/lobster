...
class TestVCFAdapterCore:
    ...
    def test_load_vcf_basic(self, vcf_path):
        ...
        assert adata.n_obs > 2000, f"Expected >2000 samples (1000 Genomes), got {adata.n_obs}"
    ...
    def test_vcf_sparse_matrix_conversion(self, vcf_path):
        ...
        if hasattr(adata.X, "toarray"):
            ...
            assert sparsity > 0.5, f"Expected sparsity > 50% for rare variants, got {sparsity:.1%}"
...
class TestPLINKAdapterCore:
    ...
    def test_plink_fam_metadata(self, plink_prefix):
        ...
        expected_cols = ["individual_id", "family_id", "father_id", "mother_id", "sex", "phenotype"]
    ...
    def test_plink_genotype_encoding(self, plink_prefix):
        ...
        unique_values = np.unique(gt[~np.isnan(gt)])
        assert all(v in [0, 1, 2] for v in unique_values), f"Invalid genotype values: {unique_values}"
...
class TestPLINKAdapterFiltering:
    ...
    def test_plink_maf_filter(self, plink_prefix):
        ...
        assert n_variants_filtered <= n_variants_all, "MAF filter should reduce or equal variant count"
