...
class TestAgentRegistryManagement:
    ...
    def test_register_agent_duplicate(self, temp_agent_registry, mock_agent_config):
        ...
        with pytest.raises(
            ValueError, match="Agent 'test_agent' is already registered"
        ):
            # register_agent(mock_agent_config)  # Function not implemented
            pass
...
class TestAgentFactoryInstantiation:
    ...
    def test_agent_instantiation_with_data_manager(self, mock_data_manager):
        ...
        agent = mock_agent_factory(name="data_agent", data_manager=mock_data_manager)
...
class TestRegistryPersistenceLoading:
    ...
    def test_registry_modification_isolation(self, temp_agent_registry):
        ...
        # Registry should be populated with agents from entry points
        initial_count = len(temp_agent_registry)
        assert initial_count > 0  # Should have agents loaded
        ...
        # Verify config can be created properly
