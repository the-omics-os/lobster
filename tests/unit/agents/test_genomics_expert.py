...
class TestGenomicsExpertConfiguration:
    ...
    def test_agent_config_in_registry(self):
        ...
        assert "genomics_expert" in AGENT_REGISTRY, "genomics_expert should be in AGENT_REGISTRY"
        config = AGENT_REGISTRY["genomics_expert"]

        # Verify basic configuration
        assert config.name == "genomics_expert"
        assert config.display_name == "Genomics Expert"
        assert config.factory_function == "lobster.agents.genomics.genomics_expert.genomics_expert"
        assert config.handoff_tool_name == "handoff_to_genomics_expert"

        # Verify agent is supervisor-accessible
        assert config.supervisor_accessible is True, "genomics_expert should be supervisor-accessible"
...
class TestGenomicsExpertPrompts:
    ...
    def test_system_prompt_exists(self):
        ...
        try:
            ...
            if hasattr(prompts, "create_system_prompt"):
                ...
                # Check key content
                assert "genomics" in prompt.lower() or "genotype" in prompt.lower() or "vcf" in prompt.lower()
    ...
    def test_system_prompt_mentions_tools(self):
        ...
        try:
            ...
            key_terms = ["genomics", "vcf", "plink", "gwas", "variant", "genotype", "qc", "filter"]
            found_count = sum(1 for term in key_terms if term.lower() in prompt.lower())

            assert found_count >= 3, f"System prompt should mention genomics concepts, found {found_count}/{len(key_terms)} terms"
