...
class TestMapSamplesByID:
    ...
    def test_map_samples_success(
        ...
        assert (
            "10.0%" in log_call[1]["description"] or "0.1" in log_call[1]["description"]
        )
...
class TestUnexpectedErrors:
    ...
    def test_map_samples_unexpected_error(
        ...
        mock_data_manager.get_modality.side_effect = lambda x: (
            mock_adata1 if x == "dataset1" else mock_adata2
        )
...
class TestFilterSamplesBy:
    ...
    @patch(
        "lobster.agents.metadata_assistant.metadata_assistant.DiseaseStandardizationService"
    )
