...
def cached_publication_abstract(mock_data_manager):
    ...
    metadata_path = (
        mock_data_manager.workspace_path / "metadata" / f"{pub_id}_metadata.json"
    )
...
def cached_publication_methods(mock_data_manager):
    ...
    methods_path = (
        mock_data_manager.workspace_path / "metadata" / f"{pub_id}_methods.json"
    )
...
class TestPhase2LLMAbstractExtraction:
    ...
    def test_llm_abstract_extraction_mocked(
        self, sample_data, mock_data_manager, cached_publication_abstract, mock_llm
    ):
        ...
        enriched, log = _phase2_llm_abstract_extraction(
            sample_data, mock_data_manager, confidence_threshold=0.8, llm=mock_llm
        )
    ...
    def test_llm_abstract_extraction_low_confidence_rejected(
        self, sample_data, mock_data_manager, cached_publication_abstract, mock_llm
    ):
    ...
    def test_llm_abstract_extraction_no_cached_file(
        self, sample_data, mock_data_manager, mock_llm
    ):
    ...
    def test_llm_abstract_extraction_groups_by_publication(
        self, mock_data_manager, cached_publication_abstract, mock_llm
    ):
        ...
        enriched, log = _phase2_llm_abstract_extraction(
            samples, mock_data_manager, confidence_threshold=0.8, llm=mock_llm
        )
...
class TestPhase3LLMMethodsExtraction:
    ...
    def test_llm_methods_extraction_success(
        self, sample_data, mock_data_manager, cached_publication_methods, mock_llm
    ):
    ...
    def test_llm_methods_extraction_skips_enriched_samples(
        self, sample_data, mock_data_manager, cached_publication_methods, mock_llm
    ):
...
class TestProvenanceTracking:
    ...
    def test_phase2_provenance_fields(
        self, sample_data, mock_data_manager, cached_publication_abstract, mock_llm
    ):
...
class TestEnrichmentIntegration:
    ...
    def test_hybrid_mode_runs_phases_sequentially(
        self,
        sample_data,
        mock_data_manager,
        ...
        samples = [
            ...
            },  # Phase 2 will catch
            {
                "run_accession": "SRR3",
                "publication_entry_id": "pub_queue_doi_10_5678",
                "publication_title": "Test2",
            },  # Phase 3 will catch
            {
                "run_accession": "SRR4",
                "publication_entry_id": "pub_manual",
                "publication_title": "Test3",
            },  # Phase 4 will catch
        ]
