...
def mock_publication_service():
    ...
    with patch('lobster.tools.publication_service.PublicationService') as MockService:
        ...
        mock_service.search_literature.return_value = "Found 2 relevant papers about topic"
        mock_service.search_datasets_directly.return_value = "Found 2 datasets in GEO related to topic"
        ...
        mock_service.find_datasets_from_publication.return_value = "Found datasets from publication"
        mock_service.get_provider_capabilities.return_value = "Provider capabilities listed"
        yield mock_service
...
class TestResearchAgentCore:
    ...
    def test_agent_with_handoff_tools(self, mock_data_manager, mock_publication_service):
    ...
    def test_agent_with_callback_handler(self, mock_data_manager, mock_publication_service):
...
class TestResearchAgentIntegration:
    ...
    def test_agent_message_processing(self, mock_data_manager, mock_publication_service):
        """Test that agent can process messages."""
        agent = research_agent(mock_data_manager)

        # Create a simple state for testing
        test_state = {
            "messages": [{"role": "human", "content": "Find papers about T cell exhaustion"}]
        }
    ...
    def test_agent_configuration(self, mock_data_manager, mock_publication_service):
        ...
        with patch('lobster.agents.research_agent.get_settings') as mock_settings:
            ...
            mock_settings_instance.get_agent_llm_params.return_value = {"temperature": 0.1}
            ...
            with patch('lobster.agents.research_agent.create_llm') as mock_create_llm:
                ...
                agent = research_agent(mock_data_manager, agent_name="test_research_agent")

                assert agent is not None
                # Verify settings were retrieved for the agent
                mock_settings_instance.get_agent_llm_params.assert_called_once_with('research_agent')
                # Verify LLM was created with correct parameters
                mock_create_llm.assert_called_once_with('research_agent', {"temperature": 0.1})
...
class TestServiceIntegration:
    ...
    def test_publication_service_integration(self, mock_data_manager):
        ...
        with patch('lobster.agents.research_agent.PublicationService') as MockService:
            ...
            mock_service.search_datasets_directly.return_value = "Dataset search results"
            mock_service.extract_publication_metadata.return_value = PublicationMetadata(
                uid="123", title="Test", authors=[], journal="Test Journal", published="2023"
            )
    ...
    def test_research_assistant_integration(self, mock_data_manager, mock_publication_service):
        """Test integration with ResearchAgentAssistant."""
        with patch('lobster.agents.research_agent.ResearchAgentAssistant') as MockAssistant:
...
class TestToolFunctions:
    ...
    def test_search_literature_function_logic(self, mock_data_manager, mock_publication_service):
    ...
    def test_publication_service_method_calls(self, mock_data_manager):
        ...
        with patch('lobster.agents.research_agent.PublicationService') as MockService:
            ...
            mock_service.extract_publication_metadata.return_value = PublicationMetadata(
                uid="123", title="Test", authors=[], journal="Test", published="2023"
            )
...
class TestMockDataIntegration:
    ...
    def test_with_research_state_fixture(self, research_state, mock_data_manager, mock_publication_service):
        ...
        assert research_state.messages[0].content == "Find papers about T cell exhaustion"

        # Verify agent was created
        assert agent is not None

    def test_with_mock_data_factories(self, mock_data_manager, mock_publication_service):
