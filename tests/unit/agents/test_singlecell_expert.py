...
class TestSingleCellExpertCore:
    ...
    def test_filter_and_normalize_data(self, mock_preprocessing_service, mock_data_manager):
    ...
    def test_find_marker_genes(self, mock_singlecell_service, mock_data_manager):
        ...
        mock_adata.obs['leiden'] = pd.Categorical([str(i % 5) for i in range(mock_adata.n_obs)])
        ...
        mock_service.find_marker_genes.return_value = (
            mock_adata,
            {
                ...
                'top_markers_per_group': {
                    '0': [{'gene': 'CD3D', 'score': 0.8}, {'gene': 'CD3E', 'score': 0.75}],
                    '1': [{'gene': 'CD79A', 'score': 0.9}, {'gene': 'CD79B', 'score': 0.85}]
                }
            }
        )
...
class TestAgentConfiguration:
    ...
    def test_agent_with_custom_name(self, mock_data_manager):
        ...
        agent = singlecell_expert(
            data_manager=mock_data_manager,
            agent_name="custom_singlecell_agent"
        )
