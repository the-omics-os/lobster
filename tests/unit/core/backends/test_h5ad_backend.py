...
class TestH5ADDataSanitization:
    ...
    def test_sanitize_categorical_non_string_categories(self):
        ...
        if hasattr(adata_loaded.obs["numeric_cat"], "cat"):
            ...
            assert all(
                isinstance(cat, str)
                for cat in adata_loaded.obs["numeric_cat"].cat.categories
            )
        else:
    ...
    def test_sanitize_completely_empty_columns(self):
        ...
        if hasattr(mostly_empty_col, "cat"):
            assert (
                "NA" in mostly_empty_col.cat.categories
                or "" in mostly_empty_col.cat.categories
            )
        else:
