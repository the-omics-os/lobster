...
def mock_file_operations():
    ...
    with (
        ...
        yield {"csv": mock_csv, "excel": mock_excel, "h5ad": mock_h5ad, "mtx": mock_mtx}
...
class TestErrorHandlingEdgeCases:
    ...
    def test_mtx_format_missing_files(self, mock_file_operations):
        ...
        mock_file_operations["mtx"].side_effect = FileNotFoundError(
    ...
    def test_corrupted_data_handling(self):
        ...
        corrupted_df = pd.DataFrame(
            {"Gene1": [np.inf, -np.inf, np.nan, 100], "Gene2": [50, 200, 150, np.inf]}
        )
    ...
    def test_memory_efficient_processing(self):
        ...
        sparse_data = pd.DataFrame(
            np.random.choice(
                [0, 1, 2, 5], size=(n_cells, n_genes), p=[0.8, 0.1, 0.05, 0.05]
            ),
...
class TestIntegrationScenarios:
    ...
    def test_complete_proteomics_workflow(self, mock_file_operations):
        ...
        missing_mask = np.random.choice(
            [True, False], size=ms_data.shape, p=[0.15, 0.85]
        )
        ...
        # Load and process
        result = adapter.from_source(
            "proteomics_data.csv", missing_value_strategy="median"
        )
