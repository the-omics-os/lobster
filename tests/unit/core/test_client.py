...
def mock_data_manager_v2():
    ...
    mock_dm.has_data.return_value = (
        False  # Default to False, can be overridden in tests
    )
...
@pytest.fixture
def mock_graph_metadata():
    """Mock GraphMetadata for testing."""
    mock_metadata = Mock()
    mock_metadata.subscription_tier = "free"
    mock_metadata.available_agents = []
    mock_metadata.supervisor_accessible_agents = ["data_expert_agent", "research_agent"]
    mock_metadata.filtered_out_agents = []
    mock_metadata.to_dict.return_value = {
        "subscription_tier": "free",
        "available_agents": [],
        "supervisor_accessible_agents": ["data_expert_agent", "research_agent"],
        "filtered_out_agents": [],
        "agent_count": 0,
        "supervisor_accessible_count": 2,
    }
    return mock_metadata


@pytest.fixture
def mock_create_bioinformatics_graph(mock_langgraph_graph, mock_graph_metadata):
    """Mock the graph creation function."""
    with patch("lobster.core.client.create_bioinformatics_graph") as mock_create:
        # Return tuple of (graph, metadata) to match new signature
        mock_create.return_value = (mock_langgraph_graph, mock_graph_metadata)
        yield mock_create
...
class TestAgentClientInitialization:
    ...
    def test_basic_initialization(
        self, temp_workspace, mock_create_bioinformatics_graph
    ):
    ...
    def test_initialization_with_custom_params(
        ...
        assert (
            kwargs["manual_model_params"] == agent_client_config["manual_model_params"]
        )

    @patch.dict(os.environ, {"LANGFUSE_PUBLIC_KEY": "test_key"})
    def test_initialization_with_langfuse(
        self, temp_workspace, mock_create_bioinformatics_graph
    ):
        """Test initialization with Langfuse enabled."""
        mock_callback = Mock()
        mock_langfuse_module = MagicMock()
        mock_langfuse_module.CallbackHandler.return_value = mock_callback
        with patch.dict(
            sys.modules,
            {"langfuse": MagicMock(), "langfuse.langchain": mock_langfuse_module},
        ):
            client = AgentClient(workspace_path=temp_workspace, enable_langfuse=True)

            assert mock_callback in client.callbacks

    def test_initialization_with_custom_callbacks(
        self, temp_workspace, mock_create_bioinformatics_graph
    ):
        ...
        client = AgentClient(
            workspace_path=temp_workspace, custom_callbacks=[custom_callback]
        )
...
class TestAgentClientQueryProcessing:
    ...
    def test_query_with_reasoning_enabled(
        self, temp_workspace, mock_create_bioinformatics_graph, mock_data_manager_v2
    ):
        ...
        mock_graph.stream.return_value = [{"supervisor": {"messages": [mock_message]}}]
        mock_create_bioinformatics_graph.return_value = (mock_graph, Mock())
    ...
    def test_query_with_reasoning_disabled(
        self, temp_workspace, mock_create_bioinformatics_graph, mock_data_manager_v2
    ):
    ...
    def test_query_with_data_plots_response(
        self, temp_workspace, mock_create_bioinformatics_graph, mock_data_manager_v2
    ):
    ...
    @pytest.mark.parametrize(
        "user_input,expected_in_response",
        [
    ...
    def test_query_with_various_inputs(
        self,
        temp_workspace,
        mock_create_bioinformatics_graph,
        mock_data_manager_v2,
...
class TestAgentClientFileOperations:
    ...
    def test_detect_file_type_bioinformatics_formats(
        self, temp_workspace, mock_create_bioinformatics_graph
    ):
    ...
    def test_detect_file_type_compressed_files(
        self, temp_workspace, mock_create_bioinformatics_graph
    ):
    ...
    def test_locate_file_absolute_path(
        self, temp_workspace, mock_create_bioinformatics_graph
    ):
    ...
    def test_locate_file_relative_path_search(
        self, temp_workspace, mock_create_bioinformatics_graph, mock_data_manager_v2
    ):
    ...
    def test_locate_file_not_found(
        self, temp_workspace, mock_create_bioinformatics_graph
    ):
    ...
    def test_load_data_file_h5ad_format(
        self,
        temp_workspace,
        mock_create_bioinformatics_graph,
        mock_data_manager_v2,
    ...
    def test_load_data_file_csv_format(
        self,
        temp_workspace,
        mock_create_bioinformatics_graph,
        mock_data_manager_v2,
    ...
    def test_load_data_file_name_collision(
        self, temp_workspace, mock_create_bioinformatics_graph, mock_data_manager_v2
    ):
        """Test loading file when modality name already exists."""
        mock_data_manager_v2.list_modalities.side_effect = lambda: [
            "test_data",
            "test_data_1",
        ]
        ...
        assert result["success"] is True
        assert (
            result["modality_name"] == "test_data_2"
        )  # Should increment to avoid collision

    def test_load_data_file_unsupported_format(
        self, temp_workspace, mock_create_bioinformatics_graph, mock_data_manager_v2
    ):
...
class TestAgentClientStateManagement:
    ...
    def test_get_status_without_data(
        self, temp_workspace, mock_create_bioinformatics_graph, mock_data_manager_v2
    ):
    ...
    def test_export_session_with_data(
        ...
        mock_data_manager_v2.has_data.return_value = True
        mock_data_manager_v2.create_data_package.return_value = str(
            temp_workspace / "package.zip"
        )
    ...
    def test_export_session_without_data(
        self, temp_workspace, mock_create_bioinformatics_graph, mock_data_manager_v2
    ):
...
class TestBaseClientCompliance:
    ...
    def test_implements_base_client(
        self, temp_workspace, mock_create_bioinformatics_graph
    ):
    ...
    def test_optional_methods_implemented(
        self, temp_workspace, mock_create_bioinformatics_graph
    ):
...
class TestErrorHandlingAndEdgeCases:
    ...
    def test_query_with_empty_events(
        self, temp_workspace, mock_create_bioinformatics_graph, mock_data_manager_v2
    ):
    ...
    def test_query_with_malformed_events(
        self, temp_workspace, mock_create_bioinformatics_graph, mock_data_manager_v2
    ):
    ...
    def test_extract_content_from_various_message_formats(
        self, temp_workspace, mock_create_bioinformatics_graph, mock_data_manager_v2
    ):
    ...
    def test_file_operations_with_permission_errors(
        self, temp_workspace, mock_create_bioinformatics_graph
    ):
...
class TestPerformanceAndConcurrency:
    ...
    def test_memory_usage_with_large_conversation(
        self, temp_workspace, mock_create_bioinformatics_graph, mock_data_manager_v2
    ):
    ...
    def test_file_listing_performance(
        self, temp_workspace, mock_create_bioinformatics_graph, n_files
    ):
...
class TestClientIntegrationScenarios:
    ...
    def test_session_export_and_metadata(
        self, temp_workspace, mock_create_bioinformatics_graph, mock_data_manager_v2
    ):
    ...
    def test_client_resource_cleanup(
        self, temp_workspace, mock_create_bioinformatics_graph, mock_data_manager_v2
    ):
...
class TestParametrizedScenarios:
    ...
    def test_reasoning_toggle_behavior(
        self,
        temp_workspace,
        mock_create_bioinformatics_graph,
        mock_data_manager_v2,
    ...
    def test_file_type_detection_parametrized(
        self,
        temp_workspace,
        mock_create_bioinformatics_graph,
        file_extension,
    ...
    def test_initialization_parameter_combinations(
        ...
        with patch.dict(
            os.environ, {"LANGFUSE_PUBLIC_KEY": "test_key"} if enable_langfuse else {}
        ):
            with patch.dict(
                sys.modules,
        ...
        if enable_langfuse:
            assert (
                len(client.callbacks) >= 2
            )  # TokenTrackingCallback + LangfuseCallback
        else:
