...
def mock_entry_points():
    ...
    mock_service_ep.load.return_value = type("TestService", (), {"__name__": "TestService", "__module__": "test_package.services"})
...
class TestRegistryInitialization:
    ...
    def test_registry_starts_unloaded(self, fresh_registry):
        ...
        assert fresh_registry._custom_agents == {}
    ...
    def test_load_components_idempotent(self, fresh_registry):
        ...
        agents_after_first = dict(fresh_registry._custom_agents)

        # Second load should be no-op
        fresh_registry.load_components()
        assert fresh_registry._loaded is True
        assert fresh_registry._services == services_after_first
        assert fresh_registry._custom_agents == agents_after_first
    ...
    def test_load_components_calls_entry_point_groups(self, mock_load_ep, fresh_registry):
        """Loading should discover both services and agents entry point groups."""
        fresh_registry.load_components()

        # Verify both entry point groups were loaded
...
class TestServiceAPI:
    ...
    def test_get_service_returns_loaded_service(self, mock_load_ep, fresh_registry):
        ...
        mock_service_class = type("MockService", (), {"__name__": "MockService", "__module__": "test"})
    ...
    def test_list_services_includes_module_paths(self, mock_load_ep, fresh_registry):
        ...
        mock_service_class = type("TestService", (), {"__name__": "TestService", "__module__": "test.module"})
...
class TestAgentAPI:
    ...
    def test_get_agent_raises_when_required(self, fresh_registry):
        ...
        assert "Available custom agents:" in str(exc_info.value)
    ...
    def test_get_agent_returns_loaded_agent(self, mock_load_ep, fresh_registry):
        ...
        fresh_registry._custom_agents["custom_agent"] = mock_agent_config
        fresh_registry._loaded = True
    ...
    def test_has_agent_returns_true_for_existing(self, mock_load_ep, fresh_registry):
        ...
        fresh_registry._custom_agents["existing_agent"] = Mock()
        fresh_registry._loaded = True
    ...
    def test_list_agents_merges_core_and_custom(self, mock_load_ep, fresh_registry):
        """list_agents should merge core agents with custom agents."""
        # Add a custom agent
        mock_agent_config = Mock(name="premium_agent", display_name="Premium Agent")
        fresh_registry._custom_agents["premium_agent"] = mock_agent_config
        fresh_registry._loaded = True

        all_agents = fresh_registry.list_agents()

        # Should contain both core and custom
        assert "research_agent" in all_agents  # Core
        assert "premium_agent" in all_agents  # Custom
...
class TestUtilityMethods:
    ...
    def test_get_info_returns_diagnostic_dict(self, fresh_registry):
        ...
        assert "count" in info["custom_agents"]
        assert "names" in info["custom_agents"]
        assert isinstance(info["custom_agents"]["names"], list)

        assert isinstance(info["total_agents"], int)
    ...
    def test_get_info_counts_correct(self, mock_load_ep, fresh_registry):
        ...
        # Add mock data
        fresh_registry._services["svc1"] = type("Svc1", (), {"__name__": "Svc1", "__module__": "test"})
        fresh_registry._services["svc2"] = type("Svc2", (), {"__name__": "Svc2", "__module__": "test"})
        fresh_registry._custom_agents["agent1"] = Mock()
        fresh_registry._loaded = True
        ...
        assert info["custom_agents"]["count"] == 1
        assert info["custom_agents"]["names"] == ["agent1"]
    ...
    def test_reset_clears_registry_state(self, fresh_registry):
        ...
        fresh_registry._custom_agents["test"] = Mock()
...
class TestEntryPointLoading:
    ...
    def test_load_entry_point_success(self, fresh_registry):
        ...
        mock_service = type("TestService", (), {"__name__": "TestService", "__module__": "test"})
        success_ep = Mock()
...
class TestEdgeCases:
    ...
    def test_agent_value_error_message_format(self, mock_load_ep, fresh_registry):
        ...
        fresh_registry._custom_agents["custom1"] = Mock()
        fresh_registry._loaded = True
