...
class TestWorkspaceProviderConfig:
    ...
    def test_per_agent_provider_validation(self):
        ...
        config = WorkspaceProviderConfig(
            per_agent_providers={"supervisor": "ollama", "data_expert": "bedrock"}
        )
        assert config.per_agent_providers["supervisor"] == "ollama"

        # Invalid per-agent provider
        with pytest.raises(ValidationError) as excinfo:
            WorkspaceProviderConfig(per_agent_providers={"supervisor": "invalid"})
        assert "Invalid provider" in str(excinfo.value)
...
class TestConfigResolver:
    ...
    @pytest.mark.skip(
        reason="LEGACY: Global config provider selection removed, only workspace config supported"
    )
    ...
    @pytest.mark.skip(
        reason="LEGACY: Environment variable provider selection removed in favor of explicit configuration"
    )
    ...
    @pytest.mark.skip(
        reason="LEGACY: Auto-detection removed in favor of explicit configuration"
    )
    def test_resolve_provider_auto_detect_ollama(self, temp_workspace, clean_env):
    ...
    @pytest.mark.skip(
        reason="LEGACY: Default fallback removed in favor of explicit configuration"
    )
    ...
    def test_resolve_model_workspace_global(self, temp_workspace):
        ...
        workspace_config = WorkspaceProviderConfig(ollama_model="mixtral:8x7b-instruct")
        workspace_config.save(temp_workspace)
    ...
    @pytest.mark.skip(
        reason="LEGACY: Profile env variable behavior changed - now only via --profile flag"
    )
...
class TestEdgeCases:
    ...
    @pytest.mark.skip(
        reason="LEGACY: Resolver now requires explicit configuration, no defaults"
    )
    ...
    @pytest.mark.skip(
        reason="LEGACY: Invalid runtime provider now raises error instead of falling back"
    )
