...
class TestDataManagerV2Initialization:
    ...
    def test_provenance_initialization(
        ...
        dm_with_prov = DataManagerV2(
            workspace_path=temp_workspace, enable_provenance=True, auto_scan=False
        )
        ...
        dm_without_prov = DataManagerV2(
            workspace_path=temp_workspace, enable_provenance=False, auto_scan=False
        )
...
class TestBackendAdapterManagement:
    ...
    def test_register_backend_duplicate_name(
        ...
        with pytest.raises(
            ValueError, match="Backend 'test_backend' already registered"
        ):
            dm.register_backend("test_backend", mock_backend)
    ...
    def test_register_adapter_duplicate_name(
        ...
        with pytest.raises(
            ValueError, match="Adapter 'test_adapter' already registered"
        ):
            dm.register_adapter("test_adapter", mock_adapter)
...
class TestModalityManagement:
    ...
    def test_load_modality_unregistered_adapter(self, temp_workspace):
        ...
        with pytest.raises(ValueError, match="Adapter 'nonexistent' not registered"):
            dm.load_modality(
                name="test_modality", source="/path/to/data.csv", adapter="nonexistent"
            )
    ...
    def test_load_modality_validation_failure(self, temp_workspace, mock_adapter):
        ...
        mock_adapter.from_source.return_value = SingleCellDataFactory(
            config=SMALL_DATASET_CONFIG
        )

        with pytest.raises(
            ValueError, match="Validation failed for modality 'test_modality'"
        ):
            dm.load_modality(
                name="test_modality",
                source="/path/to/data.csv",
    ...
    def test_save_modality_success(self, temp_workspace, mock_backend):
        ...
        result_path = dm.save_modality(
            name="test_modality", path="test_file.h5ad", backend="test_backend"
        )
...
class TestQualityValidation:
    ...
    def test_get_quality_metrics_all_modalities(
        ...
        dm.modalities["transcriptomics_single_cell_mod1"] = SingleCellDataFactory(
            config=SMALL_DATASET_CONFIG
        )
        dm.modalities["transcriptomics_single_cell_mod2"] = SingleCellDataFactory(
            config=SMALL_DATASET_CONFIG
        )
    ...
    def test_get_quality_metrics_no_matching_adapter(
        ...
        dm = DataManagerV2(workspace_path=temp_workspace, auto_scan=False)
        dm.modalities["unknown_type"] = SingleCellDataFactory(
            config=SMALL_DATASET_CONFIG
        )
    ...
    def test_validate_modalities_success(
        ...
        dm.modalities["transcriptomics_single_cell_test"] = SingleCellDataFactory(
            config=SMALL_DATASET_CONFIG
        )
    ...
    def test_match_modality_to_adapter_bulk(self, temp_workspace):
        ...
        bulk_names = ["bulk_rnaseq", "bulk_transcriptomics", "rna_bulk_analysis"]
...
class TestWorkspaceManagement:
    ...
    def test_get_workspace_status_with_provenance(
        self, temp_workspace, mock_provenance
    ):
    ...
    def test_clear_workspace_without_confirmation(self, temp_workspace):
        ...
        with pytest.raises(
            ValueError, match="Must set confirm=True to clear workspace"
        ):
    ...
    def test_to_mudata_success(
        ...
        with patch("lobster.core.data_manager_v2.mudata") as mock_mudata:
            ...
            dm = DataManagerV2(
                workspace_path=temp_workspace, auto_scan=False, enable_provenance=False
            )
            dm.modalities["rna"] = SingleCellDataFactory(config=SMALL_DATASET_CONFIG)
            dm.modalities["protein"] = ProteomicsDataFactory(
                config=SMALL_DATASET_CONFIG
            )
    ...
    def test_to_mudata_missing_modalities(self, temp_workspace):
        ...
        with patch("lobster.core.data_manager_v2.MUDATA_AVAILABLE", True):
            ...
            with pytest.raises(
                ValueError, match="Modalities not found: \\['nonexistent'\\]"
            ):
    ...
    def test_save_processed_data_success(
        ...
        with patch.object(dm, "save_modality") as mock_save:
            ...
            result = dm.save_processed_data(
                processing_step="filtered", data_source="GEO", dataset_id="GSE123456"
            )
...
class TestErrorHandling:
    ...
    def test_workspace_operations_with_permission_errors(self, temp_workspace):
        ...
        with patch.object(
            dm, "save_modality", side_effect=PermissionError("Access denied")
        ):
            dm.modalities["test_mod"] = Mock()
...
class TestIntegrationScenarios:
    ...
    def test_full_workflow_single_cell_analysis(
        ...
        adata = dm.load_modality(
            "sc_data", "path/to/data.h5ad", "transcriptomics_single_cell"
        )
