...
def compliant_transcriptomics_data():
    ...
    adata.var["highly_variable"] = np.random.choice(
        [True, False], adata.n_vars, p=[0.1, 0.9]
    )
...
def compliant_proteomics_data():
    ...
    adata.var["is_contaminant"] = np.random.choice(
        [True, False], adata.n_vars, p=[0.05, 0.95]
    )
...
class TestFlexibleValidator:
    ...
    def test_flexible_validator_initialization(self):
        ...
        schema = {
            "obs": {"required": ["test_field"], "optional": []},
            "var": {"required": ["test_var"], "optional": []},
        }
    ...
    def test_validate_schema_compliance_success(self):
        ...
        schema = {
            "obs": {"required": ["sample_id"], "optional": []},
            "var": {"required": ["gene_id"], "optional": []},
        }
    ...
    def test_validate_data_types(self):
        ...
        schema = {
            "obs": {
                "required": [],
                "optional": [],
                "types": {"numeric_field": "numeric", "string_field": "string"},
            }
        }
        validator = FlexibleValidator(schema)
...
class TestTranscriptomicsSchema:
    ...
    def test_validate_strict_vs_permissive(self, non_compliant_transcriptomics_data):
        ...
        permissive_result = validator.validate(
            non_compliant_transcriptomics_data, strict=False
        )

        # Strict mode should convert warnings to errors
        strict_result = validator.validate(
            non_compliant_transcriptomics_data, strict=True
        )
    ...
    def test_validate_mitochondrial_genes(self):
        ...
        adata = ad.AnnData(
            X=np.random.randint(0, 100, (10, 5)), var=pd.DataFrame(index=gene_names)
        )
        adata.var["mt"] = adata.var_names.str.startswith("MT-")
...
class TestProteomicsSchema:
    ...
    def test_validate_contaminants(self):
        ...
        adata = ad.AnnData(
            X=np.random.lognormal(3, 1, (20, 4)), var=pd.DataFrame(index=protein_names)
        )
...
class TestSchemaEvolution:
    ...
    def test_optional_field_handling(self):
        ...
        minimal_data.obs["total_counts"] = np.array(
            minimal_data.X.sum(axis=1)
        ).flatten()
    ...
    def test_custom_schema_extension(self):
        ...
        custom_schema["custom_obs"] = custom_schema.get("optional_obs", []) + [
            "experiment_id",
            "batch_id",
        ]
    ...
    def test_schema_field_deprecation(self):
        ...
        deprecated_data.obs["total_umis"] = np.array(
            deprecated_data.X.sum(axis=1)
        ).flatten()  # Deprecated name
    ...
    def test_custom_validator_creation(self):
        ...
        class CustomTranscriptomicsValidator(SchemaValidator):
            ...
            def __init__(self):
                ...
                custom_schema["obs"]["required"] = custom_schema["obs"]["required"] + [
                    "experiment_date"
                ]
...
class TestNewAPIFeatures:
    ...
    def test_transcriptomics_qc_thresholds(self):
        ...
        sc_thresholds = TranscriptomicsSchema.get_recommended_qc_thresholds(
            "single_cell"
        )
    ...
    def test_proteomics_qc_thresholds(self):
        ...
        ms_thresholds = ProteomicsSchema.get_recommended_qc_thresholds(
            "mass_spectrometry"
        )
    ...
    def test_custom_validation_rules(self):
        ...
        # Add a duplicate
        adata.var.loc[adata.var.index[1], "gene_symbol"] = adata.var[
            "gene_symbol"
        ].iloc[0]
        ...
        has_duplicate_warning = any(
            "duplicate" in str(msg).lower() for msg in result.warnings
        )
    ...
    def test_ignore_warnings_functionality(self):
        ...
        has_sparsity_warning = any(
            "sparse" in str(msg).lower() for msg in result.warnings
        )
...
class TestSchemaEdgeCases:
    ...
    def test_unicode_and_special_characters(self):
        ...
        adata = ad.AnnData(
            X=np.random.randint(0, 100, (10, 5)), var=pd.DataFrame(index=unicode_genes)
        )
