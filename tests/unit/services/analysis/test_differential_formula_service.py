...
class TestFormulaParsingBasic:
    ...
    def test_parse_formula_with_covariates(self, formula_service, sample_metadata):
        ...
        result = formula_service.parse_formula(
            "~condition + batch + sex", sample_metadata
        )
    ...
    def test_parse_formula_with_interaction(self, formula_service, sample_metadata):
        ...
        result = formula_service.parse_formula(
            "~condition + batch + condition*batch", sample_metadata
        )
        ...
        interaction_term = [
            t for t in result["predictor_terms"] if t["type"] == "interaction"
        ][0]
    ...
    def test_parse_formula_with_reference_levels(
        self, formula_service, sample_metadata
    ):
...
class TestFormulaParsingValidation:
    ...
    def test_parse_formula_missing_reference_level(
        self, formula_service, sample_metadata
    ):
        ...
        assert (
            result["variable_info"]["condition"]["reference_level"] == "Control"
    ...
    def test_clean_formula_normalization(self, formula_service):
        ...
        assert (
            formula_service._clean_formula("condition + batch") == "~condition + batch"
        )
        assert (
            formula_service._clean_formula("  ~condition   +   batch  ")
            == "~condition + batch"
        )
    ...
    def test_split_formula_variations(self, formula_service):
        ...
        response, predictors = formula_service._split_formula(
            "response ~ condition + batch"
        )
    ...
    def test_parse_terms_complex(self, formula_service):
        ...
        terms = formula_service._parse_terms(
            "condition + batch + age + condition*batch"
        )
...
class TestDesignMatrixConstruction:
    ...
    def test_construct_design_matrix_with_batch(self, formula_service, sample_metadata):
        ...
        formula_components = formula_service.parse_formula(
            "~condition + batch", sample_metadata
        )
        result = formula_service.construct_design_matrix(
            formula_components, sample_metadata
        )
    ...
    def test_construct_design_matrix_continuous(self, formula_service, sample_metadata):
        ...
        np.testing.assert_array_equal(
            design_matrix[:, age_col_idx], sample_metadata["age"].values
        )

    def test_construct_design_matrix_interaction(
        self, formula_service, sample_metadata
    ):
    ...
    def test_design_matrix_validation_nan_values(self, formula_service):
        ...
        with pytest.raises(
            DesignMatrixError, match="Design matrix contains NaN values"
        ):
            formula_service._validate_design_matrix(
                design_matrix, ["Intercept", "var1", "var2"]
            )
    ...
    def test_design_matrix_validation_infinite_values(self, formula_service):
        ...
        with pytest.raises(
            DesignMatrixError, match="Design matrix contains infinite values"
        ):
            formula_service._validate_design_matrix(
                design_matrix, ["Intercept", "var1", "var2"]
            )
...
class TestContrastSpecification:
    ...
    def test_construct_contrast_with_design_matrix(
        self, formula_service, sample_metadata
    ):
    ...
    def test_construct_contrast_continuous_variable(
        self, formula_service, sample_metadata
    ):
...
class TestDesignMatrixProperties:
    ...
    def test_design_matrix_dimensions(self, formula_service, sample_metadata):
        ...
        assert design_matrix.shape[1] == len(
            result["coefficient_names"]
        )  # One col per coefficient
...
class TestSimpleDesignCreation:
    ...
    def test_create_simple_design_condition_only(
        self, formula_service, sample_metadata
    ):
    ...
    def test_create_simple_design_reference_condition(
        self, formula_service, sample_metadata
    ):
...
class TestExperimentalDesignValidation:
    ...
    def test_validate_experimental_design_small_sample(self, formula_service):
        ...
        small_metadata = pd.DataFrame(
            {"condition": ["Control", "Treatment"], "batch": ["Batch1", "Batch1"]}
        )
    ...
    def test_validate_experimental_design_insufficient_replicates(
    ...
    def test_validate_experimental_design_missing_values(self, formula_service):
        ...
        assert (
            "missing" in warning_str
            or "na" in warning_str
...
class TestFormulaServiceHelperMethods:
    ...
    def test_validate_variables(self, formula_service, sample_metadata):
        ...
        terms = [{"variables": ["condition", "batch"]}, {"variables": ["sex"]}]
    ...
    def test_analyze_variables(self, formula_service, sample_metadata):
        ...
        terms = [{"variables": ["condition", "age", "batch"]}]
    ...
    def test_get_variable_columns_categorical(self, formula_service, sample_metadata):
        ...
        var_info = {"type": "categorical", "levels": ["Control", "Treatment"]}

        columns = formula_service._get_variable_columns(
            "condition", var_info, sample_metadata
        )
    ...
    def test_get_variable_columns_continuous(self, formula_service, sample_metadata):
        ...
        var_info = {"type": "continuous", "levels": None}

        columns = formula_service._get_variable_columns(
            "age", var_info, sample_metadata
        )
...
class TestFormulaServiceErrorHandling:
    ...
    def test_construct_design_matrix_error_wrapping(
        ...
        with patch.object(
            formula_service, "_add_main_effect", side_effect=ValueError("Unexpected")
        ):
            with pytest.raises(
                DesignMatrixError, match="Failed to construct design matrix"
            ):
