...
def stratified_adata():
    ...
    for j in range(n_variants):
        if j < 50:
            ...
            genotypes[:100, j] = np.random.choice([0, 1, 2], size=100, p=[0.3, 0.4, 0.3])
            # Population 2 has lower alt allele freq
            genotypes[100:, j] = np.random.choice([0, 1, 2], size=100, p=[0.7, 0.25, 0.05])
        else:
...
class TestGWASAnalysis:
    ...
    def test_run_gwas_ir_structure(self, gwas_service, simple_gwas_adata):
        ...
        # Verify IR fields
        assert ir.operation == "sgkit.gwas_linear_regression"  # Note: underscore, not dot
...
class TestGWASEdgeCases:
    ...
    def test_pca_single_variant(self, gwas_service, simple_gwas_adata):
        ...
        try:
            adata_pca, stats, ir = gwas_service.calculate_pca(adata_small, n_components=3)
...
class TestGWASParameterValidation:
    ...
    def test_pca_invalid_n_components(self, gwas_service, simple_gwas_adata):
        ...
        try:
            adata_pca, stats, ir = gwas_service.calculate_pca(simple_gwas_adata, n_components=1000)
            # If no error, service clamped to min(n_samples, n_variants)
            actual_components = adata_pca.obsm["X_pca"].shape[1]
            assert actual_components <= min(simple_gwas_adata.n_obs, simple_gwas_adata.n_vars), \
        ...
        try:
            adata_pca, stats, ir = gwas_service.calculate_pca(simple_gwas_adata, n_components=0)
        ...
        try:
            adata_pca, stats, ir = gwas_service.calculate_pca(simple_gwas_adata, n_components=-1)
...
class TestGWASServiceIntegration:
    ...
    def test_gwas_with_vcf_data(self, gwas_service):
        ...
        assert adata_gwas.n_vars >= 90, f"Expected ~100 variants, got {adata_gwas.n_vars}"
