...
def mock_adata_with_stats():
    ...
    adata.uns["statistical_tests"] = {
        ...
        "parameters": {"group_column": "condition", "test_method": "t_test"},
    }
...
class TestStatisticalTesting:
    ...
    def test_perform_statistical_testing_mann_whitney(
    ...
    def test_perform_statistical_testing_multigroup_anova(
        self, service, mock_adata_with_groups
    ):
    ...
    def test_perform_statistical_testing_missing_values_skip(
        self, service, mock_adata_with_groups
    ):
    ...
    def test_perform_statistical_testing_missing_values_impute_mean(
        self, service, mock_adata_with_groups
    ):
    ...
    def test_perform_statistical_testing_missing_values_impute_median(
        self, service, mock_adata_with_groups
    ):
    ...
    def test_perform_statistical_testing_invalid_group_column(
        self, service, mock_adata_with_groups
    ):
    ...
    def test_statistical_testing_statistics_calculation(
        self, service, mock_adata_with_groups
    ):
...
class TestDimensionalityReduction:
    ...
    def test_perform_dimensionality_reduction_tsne(
        self, service, mock_adata_with_groups
    ):
    ...
    def test_perform_dimensionality_reduction_umap_like(
        self, service, mock_adata_with_groups
    ):
    ...
    def test_perform_dimensionality_reduction_invalid_method(
        ...
        with pytest.raises(ProteomicsAnalysisError) as exc_info:
            service.perform_dimensionality_reduction(
                mock_adata_with_groups, method="invalid_method"
            )

        assert "Unknown dimensionality reduction method" in str(exc_info.value)

    def test_dimensionality_reduction_with_missing_values(
        self, service, mock_adata_with_groups
    ):
...
class TestClusteringAnalysis:
    ...
    def test_perform_clustering_analysis_hierarchical(
        self, service, mock_adata_with_groups
    ):
    ...
    def test_perform_clustering_analysis_gaussian_mixture(
        self, service, mock_adata_with_groups
    ):
    ...
    def test_perform_clustering_analysis_with_pca(
        self, service, mock_adata_with_groups
    ):
    ...
    def test_perform_clustering_analysis_without_pca(
        self, service, mock_adata_with_groups
    ):
    ...
    def test_perform_clustering_analysis_invalid_method(
        self, service, mock_adata_with_groups
    ):
        """Test clustering analysis with invalid method."""
        with pytest.raises(ProteomicsAnalysisError) as exc_info:
            service.perform_clustering_analysis(
                mock_adata_with_groups, clustering_method="invalid_method"
            )
...
class TestPathwayEnrichment:
    ...
    def test_perform_pathway_enrichment_default_proteins(
    ...
    def test_pathway_enrichment_fallback_protein_selection(
        self, service, mock_adata_with_groups
    ):
        ...
        # Provide explicit protein list since no is_significant column
        protein_list = list(mock_adata_with_groups.var_names[:5])
        result_adata, stats, ir = service.perform_pathway_enrichment(
            mock_adata_with_groups, protein_list=protein_list
        )

        assert result_adata is not None
        assert stats["n_genes_input"] > 0
...
class TestHelperMethods:
    ...
    def test_perform_tsne_helper(self, service):
        ...
        result = service._perform_tsne(
            X, n_components=2, perplexity=5.0, random_state=42
        )
...
class TestErrorHandlingAndEdgeCases:
    ...
    def test_all_missing_protein_statistical_testing(
        self, service, mock_adata_with_groups
    ):
...
class TestIntegrationScenarios:
    ...
    def test_complete_analysis_workflow(self, service, mock_adata_with_groups):
        ...
        # random test data may not have significant proteins
        protein_list = adata_clustered.var_names[:10].tolist()
        adata_enriched, _, _ = service.perform_pathway_enrichment(
            adata_clustered, protein_list=protein_list
        )
    ...
    def test_multiple_statistical_methods_consistency(
        self, service, mock_adata_with_groups
    ):
        ...
        assert (
            results["t_test"]["samples_processed"]
            == results["mann_whitney"]["samples_processed"]
        )
        assert (
            results["t_test"]["proteins_processed"]
            == results["mann_whitney"]["proteins_processed"]
        )

    def test_multiple_reduction_methods_consistency(
        self, service, mock_adata_with_groups
    ):
