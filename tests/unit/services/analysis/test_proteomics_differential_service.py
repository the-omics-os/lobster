...
def mock_adata_time_course():
    ...
    adata.obs["replicate_id"] = [
        f"rep_{i % n_replicates + 1}" for i in range(n_samples)
    ]
...
class TestCorrelationAnalysis:
    ...
    def test_perform_correlation_analysis_basic(self, service, mock_adata_correlation):
        ...
        mock_adata_correlation.obs["target"] = np.random.randn(
            mock_adata_correlation.n_obs
        )
...
class TestIntegrationScenarios:
    ...
    def test_complete_differential_analysis_workflow(
        ...
        adata_corr, _, _ = service.perform_correlation_analysis(
            adata_time, target_column="target"
        )
...
class TestPerformanceAndMemory:
    ...
    def test_memory_efficient_correlation_analysis(self, service):
        ...
        result_adata, stats, ir = service.perform_correlation_analysis(
            adata,
            target_column="target",
            min_correlation=0.8,  # Higher threshold to reduce memory usage
        )
