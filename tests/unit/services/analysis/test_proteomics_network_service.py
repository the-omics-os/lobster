...
class TestWGCNALiteService:
        ...
        obs = pd.DataFrame(
            {
                ...
                "continuous_trait": module1_signal * 2 + np.random.randn(n_samples) * 0.5,
            },
    ...
    def test_identify_modules_finds_expected_modules(self, service, sample_adata_with_modules):
        ...
        assert stats["n_modules"] >= 2, f"Expected >=2 modules, got {stats['n_modules']}"
    ...
    def test_module_eigengenes_stored_correctly(self, service, sample_adata_with_modules):
    ...
    def test_correlate_finds_expected_correlation(self, service, sample_adata_with_modules):
    ...
    def test_get_module_proteins(self, service, sample_adata_with_modules):
        ...
        module_colors = [c for c in adata_modules.var["module"].unique() if c != GREY_MODULE]
    ...
    def test_missing_module_assignments_error(self, service, small_adata):
        ...
        with pytest.raises(ProteomicsNetworkError, match="Module assignments not found"):
...
class TestWGCNALiteServiceEdgeCases:
    ...
    def test_module_uns_storage(self, service, ):
