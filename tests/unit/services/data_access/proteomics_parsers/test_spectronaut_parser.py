...
FIXTURES_DIR = Path(__file__).parent.parent.parent.parent.parent / "fixtures" / "proteomics" / "spectronaut"
...
class TestSpectronautParserLongFormat:
    ...
    def test_parse_long_format_reverse_filtering(self, parser, long_format_file):
        ...
        adata_filtered, _, _ = parser.parse(
            str(long_format_file), filter_reverse=True
        )
    ...
    def test_parse_long_format_protein_metadata(self, parser, long_format_file):
        ...
        assert "protein_groups" in adata.var.columns or "protein_id" in adata.var.columns
...
class TestSpectronautParserMatrixFormat:
    ...
    def test_parse_matrix_format_contaminant_filtering(self, parser, matrix_format_file):
...
class TestSpectronautParserIntegration:
    ...
    def test_provenance_ir_generation(self):
        ...
        assert "from lobster.services.data_access.proteomics_parsers import SpectronautParser" in ir.code_template
...
class TestSpectronautParserCriticalEdgeCases:
    ...
    def test_nan_protein_groups(self, parser):
        ...
        try:
            ...
            assert adata.n_vars >= 2
            assert "UNKNOWN" in list(adata.var_names) or any("UNKNOWN" in str(x) for x in adata.var["protein_id"])
        finally:
            os.unlink(temp_path)
    ...
    def test_qvalue_greater_than_one(self, parser):
        ...
        try:
            ...
            adata, stats, _ = parser.parse(temp_path, qvalue_threshold=1.0, log_transform=False)
    ...
    def test_duplicate_sample_names_after_cleaning(self, parser):
        ...
        with tempfile.NamedTemporaryFile(mode="w", suffix=".tsv", delete=False) as f:
            f.write("R.FileName\tPG.ProteinGroups\tPG.Genes\tPG.Quantity\tPG.Qvalue\n")
            f.write("Sample1.raw\tP12345\tEGFR\t1000000\t0.001\n")
            f.write("Sample1.d\tP12345\tEGFR\t2000000\t0.001\n")  # Becomes "Sample1" after cleaning
...
class TestSpectronautParserHighPriorityEdgeCases:
    ...
    def test_scientific_notation_intensity(self, parser):
        ...
        with tempfile.NamedTemporaryFile(mode="w", suffix=".tsv", delete=False) as f:
            f.write("R.FileName\tPG.ProteinGroups\tPG.Genes\tPG.Quantity\tPG.Qvalue\n")
            f.write("Sample1.raw\tP12345\tEGFR\t1.23E+06\t0.001\n")  # Scientific notation
    ...
    def test_qvalue_exactly_at_threshold(self, parser):
        ...
        with tempfile.NamedTemporaryFile(mode="w", suffix=".tsv", delete=False) as f:
            f.write("R.FileName\tPG.ProteinGroups\tPG.Genes\tPG.Quantity\tPG.Qvalue\n")
            f.write("Sample1.raw\tP12345\tEGFR\t1000000\t0.01\n")  # Exactly at threshold
            ...
            temp_path = f.name
        try:
            adata, _, _ = parser.parse(temp_path, qvalue_threshold=0.01, log_transform=False)
    ...
    def test_protein_id_with_special_chars(self, parser):
        ...
        with tempfile.NamedTemporaryFile(mode="w", suffix=".tsv", delete=False) as f:
            f.write("R.FileName\tPG.ProteinGroups\tPG.Genes\tPG.Quantity\tPG.Qvalue\n")
            f.write("Sample1.raw\tsp|P12345|EGFR_HUMAN\tEGFR\t1000000\t0.001\n")  # UniProt format
    ...
    def test_gene_symbols_empty_strings(self, parser):
        ...
        with tempfile.NamedTemporaryFile(mode="w", suffix=".tsv", delete=False) as f:
            ...
            temp_path = f.name
        try:
            adata, _, _ = parser.parse(temp_path, use_genes_as_index=True, log_transform=False)
