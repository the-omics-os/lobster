...
class TestNoneTypeHandling:
    ...
    def test_list_publication_queue_entries_handles_none_from_queue(
        self, temp_workspace
    ):
...
class TestExportsDirectory:
    ...
    def test_get_exports_directory_creates_directory(
        self, workspace_service, temp_workspace
    ):
    ...
    def test_get_exports_directory_without_create(
        self, workspace_service, temp_workspace
    ):
    ...
    def test_list_export_files_with_csv_files(self, workspace_service, temp_workspace):
        ...
        assert len(files) == 2
        assert all(f["path"].endswith(".csv") for f in files)
        assert {f["name"] for f in files} == {"samples.csv", "metadata.csv"}

    def test_list_export_files_with_pattern_filter(
        self, workspace_service, temp_workspace
    ):
    ...
    def test_list_export_files_with_category_filter(
        self, workspace_service, temp_workspace
    ):
...
class TestUnifiedMetadataSources:
    ...
    def test_get_all_metadata_sources_with_exports(
        ...
        assert sources["exports"][0]["identifier"] == "aggregated_samples"
    ...
    def test_get_all_metadata_sources_with_workspace_files(
        ...
        assert sources["workspace_files"][0]["identifier"] == "sra_prjna123_samples"
    ...
    def test_get_all_metadata_sources_with_deprecated_location(
        ...
        assert sources["deprecated"][0]["identifier"] == "old_export"
    ...
    def test_get_all_metadata_sources_with_in_memory(
        ...
        keys = {
            item.get("identifier", item.get("key")) for item in sources["in_memory"]
        }
