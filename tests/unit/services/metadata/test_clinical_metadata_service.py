...
class TestCreateResponderGroups:
    ...
    def test_orr_vs_dcr_different_results(self, service):
        ...
        # ORR grouping
        orr_groups, orr_stats, _ = service.create_responder_groups(
            df, grouping_strategy="orr"
        )
        assert orr_stats["responder_count"] == 2  # CR + PR
        assert orr_stats["non_responder_count"] == 2  # SD + PD

        # DCR grouping
        dcr_groups, dcr_stats, _ = service.create_responder_groups(
            df, grouping_strategy="dcr"
        )
    ...
    def test_invalid_grouping_strategy_raises(self, service, sample_metadata_df):
        ...
        with pytest.raises(ValueError) as exc_info:
            service.create_responder_groups(
                sample_metadata_df, grouping_strategy="invalid"
            )
...
class TestEdgeCases:
    ...
    def test_empty_dataframe(self, service):
        ...
        empty_df = pd.DataFrame(columns=["sample_id", "response_status", "timepoint"])
