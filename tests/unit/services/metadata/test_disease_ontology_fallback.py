...
class TestDiseaseStandardizationServiceFallback:
    ...
    def test_fallback_to_hardcoded_mappings(self):
        ...
        with patch.dict(sys.modules, {"lobster.services.metadata.disease_ontology_service": None}):
            # Force reimport with service unavailable
            import importlib
            if "lobster.services.metadata.disease_standardization_service" in sys.modules:
                del sys.modules["lobster.services.metadata.disease_standardization_service"]
            ...
            metadata = pd.DataFrame({"disease": ["colorectal cancer", "ulcerative colitis", "crohn's disease"]})
...
class TestMetadataAssistantFallback:
    ...
    def test_fallback_to_hardcoded_keywords(self):
        ...
        with patch.dict(sys.modules, {"lobster.services.metadata.disease_ontology_service": None}):
            # Force reimport
            if "lobster.agents.metadata_assistant" in sys.modules:
                del sys.modules["lobster.agents.metadata_assistant"]
...
class TestBothPathsProduceSameResults:
    ...
    def test_disease_standardization_parity(self):
        ...
        # Test data
        test_diseases = ["colorectal cancer", "ulcerative colitis", "crohn's disease", "healthy"]
        ...
        result_premium, stats_premium, _ = service_premium.standardize_disease_terms(metadata, "disease")
        ...
        with patch.dict(sys.modules, {"lobster.services.metadata.disease_ontology_service": None}):
            ...
            assert result_premium["disease"].tolist() == result_public["disease"].tolist()
            assert stats_premium["standardization_rate"] == stats_public["standardization_rate"]
