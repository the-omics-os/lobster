...
from lobster.services.metadata.manual_annotation_service import (
    AnnotationState,
    ClusterInfo,
...
class TestManualAnnotationService:
    ...
    def test_initialize_annotation_session(self, annotation_service, mock_adata):
        ...
        state = annotation_service.initialize_annotation_session(
            adata=mock_adata, cluster_key="leiden"
        )
    ...
    def test_apply_annotations_to_adata(self, annotation_service, mock_adata):
        ...
        adata_annotated = annotation_service.apply_annotations_to_adata(
            adata=mock_adata, cluster_key="leiden", cell_type_column="cell_type_manual"
        )
        ...
        assert all(
            adata_annotated.obs.loc[t_cell_mask, "cell_type_manual"] == "T cells"
        )

        b_cell_mask = adata_annotated.obs["leiden"] == "1"
        assert all(
            adata_annotated.obs.loc[b_cell_mask, "cell_type_manual"] == "B cells"
        )
    ...
    def test_suggest_debris_clusters(self, annotation_service, mock_adata):
        ...
        mock_adata.obs.loc[mock_adata.obs["leiden"] == "0", "n_genes_by_counts"] = (
            100  # Very low
        )
        mock_adata.obs.loc[mock_adata.obs["leiden"] == "1", "pct_counts_mt"] = (
            60  # Very high
        )
        mock_adata.obs.loc[mock_adata.obs["leiden"] == "2", "total_counts"] = (
            200  # Very low
        )

        # Get suggestions
        suggestions = annotation_service.suggest_debris_clusters(
            adata=mock_adata, min_genes=200, max_mt_percent=50, min_umi=500
        )
    ...
    def test_validate_annotation_coverage(self, annotation_service, mock_adata):
        ...
        mock_adata.obs.loc[
            mock_adata.obs["leiden"].isin(["0", "1"]), "cell_type_manual"
        ] = "T cells"
        mock_adata.obs.loc[mock_adata.obs["leiden"] == "2", "cell_type_manual"] = (
            "B cells"
        )
        mock_adata.obs.loc[mock_adata.obs["leiden"] == "9", "cell_type_manual"] = (
            "Debris"
        )

        # Validate coverage
        validation = annotation_service.validate_annotation_coverage(
            adata=mock_adata, annotation_col="cell_type_manual"
        )
    ...
    def test_export_functionality(
        self, mock_prompt_ask, mock_json_dump, mock_open, annotation_service, mock_adata
    ):
    ...
    def test_import_functionality(
        self, mock_prompt_ask, mock_json_load, mock_open, annotation_service, mock_adata
    ):
...
class TestErrorHandling:
    ...
    def test_empty_adata(self, annotation_service):
        ...
        empty_adata = sc.AnnData(
            X=np.empty((0, 100)), obs=pd.DataFrame(columns=["leiden"])
        )
