...
def test_export_for_ml_framework_pytorch_unavailable(
    ml_service, sample_adata, output_dir
):
...
def test_detect_modality_type_proteomics(ml_service):
    ...
    assert (
        ml_service._detect_modality_type("ms_proteomics")
...
def test_generate_ml_recommendations_transcriptomics_specific(ml_service):
    ...
    assert any(
        "negative" in rec.lower() or "preprocessing" in rec.lower()
...
def test_ir_includes_code_template(ml_service, sample_adata):
    ...
    # Code template should contain ML readiness assessment logic
    assert (
        "readiness" in ir.code_template.lower()
        or "ml" in ir.code_template.lower()
        or "check" in ir.code_template.lower()
    )
...
def test_ir_includes_imports(ml_service, sample_adata):
    ...
    # Imports should include standard ML/data science libraries
    all_imports = " ".join(ir.imports)
    assert "numpy" in all_imports or "sklearn" in all_imports or "pandas" in all_imports
