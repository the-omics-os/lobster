...
def small_genomics_adata():
    ...
    genotypes = np.random.choice([0, 1, 2], size=(n_samples, n_variants), p=[0.70, 0.25, 0.05])
...
def perfect_quality_adata():
    ...
    genotypes = np.random.choice([0, 1, 2], size=(n_samples, n_variants), p=[0.49, 0.42, 0.09])
    genotypes = genotypes.astype(float)
...
class TestQCMetricsCalculation:
    ...
    def test_assess_quality_returns_3tuple(self, genomics_service, small_genomics_adata):
    ...
    def test_assess_quality_adds_sample_metrics(self, genomics_service, small_genomics_adata):
    ...
    def test_assess_quality_adds_variant_metrics(self, genomics_service, small_genomics_adata):
    ...
    def test_assess_quality_stats_dict_structure(self, genomics_service, small_genomics_adata):
...
class TestSampleFiltering:
    ...
    def test_filter_samples_returns_3tuple(self, genomics_service, small_genomics_adata):
    ...
    def test_filter_samples_removes_low_call_rate(self, genomics_service, high_missing_adata):
        ...
        assert adata_filtered.n_obs < adata_qc.n_obs, "Expected some samples to be filtered"
        assert stats["samples_removed"] > 0

    def test_filter_samples_removes_heterozygosity_outliers(self, genomics_service, small_genomics_adata):
        ...
        adata_filtered, stats, ir = genomics_service.filter_samples(
            adata_qc, het_sd_threshold=2.0  # Stricter threshold
        )
    ...
    def test_filter_samples_preserves_passing_samples(self, genomics_service, perfect_quality_adata):
...
class TestVariantFiltering:
    ...
    def test_filter_variants_returns_3tuple(self, genomics_service, small_genomics_adata):
    ...
    def test_filter_variants_removes_low_maf(self, genomics_service, small_genomics_adata):
    ...
    def test_filter_variants_removes_low_call_rate(self, genomics_service, high_missing_adata):
    ...
    def test_filter_variants_removes_hwe_failures(self, genomics_service, small_genomics_adata):
...
class TestStatisticalAccuracy:
    ...
    def test_call_rate_calculation(self, genomics_service):
        ...
        ] + [[0, 1, 2, 0, 1]] * 7, dtype=float)  # Rest: 100%
    ...
    def test_maf_calculation(self, genomics_service):
        ...
        assert adata_qc.var["maf"].iloc[2] == pytest.approx(0.0, abs=0.01)  # All alt → MAF=0
...
class TestParameterValidation:
    ...
    def test_invalid_min_maf(self, genomics_service, small_genomics_adata):
        ...
        if result is not None:
            ...
            assert adata_filtered.n_vars == 0 or adata_filtered.n_vars <= adata_qc.n_vars
...
class TestServiceAdapterIntegration:
    ...
    def test_qc_workflow_with_vcf_adapter(self, genomics_service):
        ...
        # Load VCF
        vcf_path = Path(__file__).parent.parent.parent.parent.parent / "test_data" / "genomics" / "chr22.vcf.gz"
        ...
        adata_filtered_samples, stats_s, ir_s = genomics_service.filter_samples(adata_qc)
        adata_filtered_variants, stats_v, ir_v = genomics_service.filter_variants(adata_filtered_samples)
    ...
    def test_qc_workflow_with_plink_adapter(self, genomics_service):
        ...
        # Load PLINK
        plink_prefix = Path(__file__).parent.parent.parent.parent.parent / "test_data" / "genomics" / "plink_test" / "test_chr22"
