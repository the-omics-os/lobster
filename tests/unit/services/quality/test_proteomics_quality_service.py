...
from lobster.services.quality.proteomics_quality_service import (
...
def mock_adata_with_replicates():
    ...
    for group in range(4):
        ...
        for rep in range(3):
            ...
            X[rep_idx, :] = group_mean + np.random.normal(
                0, group_mean * 0.1, n_proteins
            )
    ...
    adata.obs["replicate_group"] = (
        ["group_A"] * 3 + ["group_B"] * 3 + ["group_C"] * 3 + ["group_D"] * 3
    )
...
class TestMissingValuePatterns:
    ...
    def test_assess_missing_value_patterns_basic(
    ...
    def test_assess_missing_value_patterns_custom_thresholds(
        self, service, mock_adata_with_missing
    ):
    ...
    def test_missing_value_pattern_identification(
        self, service, mock_adata_with_missing
    ):
...
class TestCoefficientVariation:
    ...
    def test_assess_coefficient_variation_basic(
    ...
    def test_assess_coefficient_variation_custom_threshold(
        self, service, mock_adata_with_replicates
    ):
    ...
    def test_assess_coefficient_variation_no_replicate_column(
        self, service, mock_adata_with_replicates
    ):
    ...
    def test_assess_coefficient_variation_invalid_column(
        self, service, mock_adata_with_replicates
    ):
        ...
        with pytest.raises(ProteomicsQualityError) as exc_info:
            service.assess_coefficient_variation(
                mock_adata_with_replicates, replicate_column="nonexistent_column"
            )
    ...
    def test_coefficient_variation_calculation_accuracy(self, service):
        ...
        X = np.array(
            [[100, 200], [150, 300], [50, 100]]  # CV = 0.5, 0.5  # for each protein
        )
        adata = ad.AnnData(X=X)
...
class TestContaminantDetection:
    ...
    def test_detect_contaminants_basic(self, service, mock_adata_with_contaminants):
        ...
        assert any(
            col.startswith("is_")
            for col in result_adata.var.columns
            if col != "is_contaminant"
        )
    ...
    def test_detect_contaminants_protein_id_column(self, service):
...
class TestDynamicRangeEvaluation:
    ...
    def test_dynamic_range_calculation_accuracy(self, service):
        ...
        # Create data with known dynamic range using enough samples for
        # stable percentile estimates (5th/95th percentile based)
        np.random.seed(42)
        n_samples = 100
        # Column 0: values spanning 1 to 100 (dynamic range ~2 orders of magnitude)
        col0 = np.linspace(1, 100, n_samples)
        # Column 1: values spanning 10 to 100 (dynamic range ~1 order of magnitude)
        col1 = np.linspace(10, 100, n_samples)
        # Column 2: values spanning 100 to 1000 (dynamic range ~1 order of magnitude)
        col2 = np.linspace(100, 1000, n_samples)
        ...
        # Check dynamic range calculations (percentile-based, so not exact min/max)
        calculated_ranges = result_adata.var["dynamic_range_log10"].values

        assert len(calculated_ranges) == 3
        # Column 0: ~2 orders of magnitude range
        assert calculated_ranges[0] > 1.0
        # Column 1: ~1 order of magnitude range
        assert calculated_ranges[1] > 0.5
        # Column 2: ~1 order of magnitude range
        assert calculated_ranges[2] > 0.5
        # All ranges should be positive
        assert all(r > 0 for r in calculated_ranges)
...
class TestPCAOutlierDetection:
    ...
    def test_detect_pca_outliers_custom_components(
        ...
        assert stats.get("n_components", stats.get("n_components_computed")) == 5
    ...
    def test_pca_outlier_detection_with_missing_values(
        self, service, mock_adata_with_missing
    ):
...
class TestTechnicalReplicateAssessment:
    ...
    def test_assess_technical_replicates_basic(
        self, service, mock_adata_with_replicates
    ):
    ...
    def test_assess_technical_replicates_basic_stats(
        self, service, mock_adata_with_replicates
    ):
        """Test replicate assessment returns basic statistics."""
        result_adata, stats, ir = service.assess_technical_replicates(
            mock_adata_with_replicates,
            replicate_column="replicate_group",
        )
    ...
    def test_assess_technical_replicates_insufficient_replicates(self, service):
        """Test replicate assessment with single-sample groups (no actual replicates)."""
        ...
        assert result_adata is not None
        # With single-sample groups, no valid replicate groups exist
        assert (
            stats.get("n_valid_replicate_groups", 0) == 0
            or stats.get("n_replicate_groups", 0) == 5
        )
...
class TestErrorHandlingAndEdgeCases:
    ...
    def test_error_handling_empty_data(self, service):
        """Test handling of empty data (graceful degradation)."""
        adata = ad.AnnData(X=np.array([]).reshape(0, 0))

        # Service handles empty data gracefully (no error raised)
        result_adata, stats, ir = service.assess_missing_value_patterns(adata)
        assert result_adata is not None
        assert stats["total_possible_values"] == 0
...
class TestIntegrationScenarios:
    ...
    def test_complete_quality_assessment_workflow(
        self, service, mock_adata_with_missing
    ):
        """Test complete quality assessment workflow."""
        # Add additional metadata for complete testing
        mock_adata_with_missing.obs["replicate_group"] = [
            f"group_{i // 3}" for i in range(mock_adata_with_missing.n_obs)
        ]
    ...
    def test_quality_metrics_preservation(self, service, mock_adata_with_missing):
        ...
        adata_step1, _, _ = service.assess_missing_value_patterns(
            mock_adata_with_missing
        )
