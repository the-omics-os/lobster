...
def mock_batch_data():
    ...
    batch_effects = {"batch_1": 1.0, "batch_2": 1.2, "batch_3": 0.8}
...
class TestQualityControlFiltering:
    ...
    def test_filter_cells_by_genes(self, preprocessing_service, mock_noisy_data):
        ...
        filtered_adata = preprocessing_service.filter_cells(
            adata, min_genes=200, max_genes=5000
        )
    ...
    def test_filter_cells_by_counts(self, preprocessing_service, mock_noisy_data):
        ...
        filtered_adata = preprocessing_service.filter_cells(
            adata, min_counts=1000, max_counts=50000
        )
    ...
    def test_filter_cells_by_mitochondrial(
        self, preprocessing_service, mock_noisy_data
    ):
        """Test cell filtering by mitochondrial gene percentage."""
        adata = mock_noisy_data.copy()
        initial_cells = adata.n_obs

        filtered_adata = preprocessing_service.filter_cells(adata, max_pct_mt=20.0)
    ...
    def test_filter_genes_by_cells(self, preprocessing_service, mock_single_cell_data):
        ...
        filtered_adata = preprocessing_service.filter_genes(adata, min_cells=3)
    ...
    def test_filter_genes_by_counts(self, preprocessing_service, mock_single_cell_data):
        ...
        filtered_adata = preprocessing_service.filter_genes(adata, min_counts=10)
    ...
    def test_detect_outlier_cells(self, preprocessing_service, mock_noisy_data):
        ...
        outliers = preprocessing_service.detect_outliers(
            adata, method="modified_z_score", threshold=3.5
        )
...
class TestNormalization:
    ...
    def test_counts_per_million_normalization(
        self, preprocessing_service, mock_single_cell_data
    ):
        ...
        normalized_adata = preprocessing_service.normalize(
            adata, method="cpm", target_sum=1e6
        )
    ...
    def test_log_normalization(self, preprocessing_service, mock_single_cell_data):
        ...
        normalized_adata = preprocessing_service.normalize(
            adata, method="log1p", target_sum=1e4
        )

        # Check that data is log-transformed (no negative values, bounded)
        assert (
            np.all(normalized_adata.X.data >= 0)
            if sp.issparse(normalized_adata.X)
            else np.all(normalized_adata.X >= 0)
        )
    ...
    def test_quantile_normalization(self, preprocessing_service, mock_bulk_data):
        ...
        normalized_adata = preprocessing_service.normalize(adata, method="quantile")
        ...
        cv_before = np.std(np.percentile(adata.X, 50, axis=1)) / np.mean(
            np.percentile(adata.X, 50, axis=1)
        )
    ...
    def test_zscore_normalization(self, preprocessing_service, mock_single_cell_data):
        ...
        normalized_adata = preprocessing_service.normalize(
            adata, method="zscore", axis=0  # Normalize genes
        )
    ...
    def test_sctransform_normalization(
        ...
        with patch.object(preprocessing_service, "sctransform") as mock_sct:
            ...
            mock_sct.return_value.var["highly_variable_sct"] = np.random.choice(
                [True, False], adata.n_vars
            )

            normalized_adata = preprocessing_service.normalize(
                adata, method="sctransform"
            )
    ...
    def test_size_factor_normalization(self, preprocessing_service, mock_bulk_data):
        ...
        normalized_adata = preprocessing_service.normalize(adata, method="deseq2")
    ...
    def test_batch_aware_normalization(self, preprocessing_service, mock_batch_data):
        ...
        normalized_adata = preprocessing_service.normalize(
            adata, method="log1p", batch_key="batch"
        )
...
class TestFeatureSelection:
    ...
    def test_highly_variable_genes_seurat(
        self, preprocessing_service, mock_single_cell_data
    ):
        ...
        hvg_info = preprocessing_service.find_highly_variable_genes(
            adata, method="seurat", n_top_genes=2000
        )
    ...
    def test_highly_variable_genes_cellranger(
        self, preprocessing_service, mock_single_cell_data
    ):
        ...
        hvg_info = preprocessing_service.find_highly_variable_genes(
            adata, method="cell_ranger", n_top_genes=1500
        )

        assert "highly_variable" in adata.var.columns
        assert np.sum(adata.var["highly_variable"]) <= 1500

    def test_highly_variable_genes_seurat_v3(
        self, preprocessing_service, mock_single_cell_data
    ):
        ...
        hvg_info = preprocessing_service.find_highly_variable_genes(
            adata, method="seurat_v3", n_top_genes=2000
        )
    ...
    def test_feature_selection_by_expression(
        self, preprocessing_service, mock_single_cell_data
    ):
    ...
    def test_remove_ribosomal_genes(self, preprocessing_service, mock_single_cell_data):
        ...
        adata.var.index = np.concatenate(
            [adata.var.index[:20], ribo_genes, adata.var.index[40:]]
        )
        ...
        assert not any(
            gene.startswith("RPS") or gene.startswith("RPL")
            for gene in filtered_adata.var.index
        )

    def test_remove_mitochondrial_genes(
        self, preprocessing_service, mock_single_cell_data
    ):
        ...
        mt_genes = [
            f"MT-{gene}" for gene in ["ATP6", "ATP8", "COX1", "COX2", "ND1", "ND2"]
        ]
        adata.var.index = np.concatenate(
            [adata.var.index[:20], mt_genes, adata.var.index[26:]]
        )
...
class TestDimensionalityReduction:
    ...
    def test_principal_component_analysis(
        ...
        pca_result = preprocessing_service.compute_pca(
            adata, n_comps=50, random_state=42
        )
    ...
    def test_pca_gene_loadings(self, preprocessing_service, mock_single_cell_data):
        ...
        pca_result = preprocessing_service.compute_pca(
            adata, n_comps=25, compute_loadings=True
        )
    ...
    def test_incremental_pca(self, preprocessing_service):
        ...
        pca_result = preprocessing_service.compute_pca(
            adata, n_comps=50, method="incremental", batch_size=1000
        )
    ...
    def test_sparse_pca(self, preprocessing_service, mock_single_cell_data):
        ...
        with patch.object(
            preprocessing_service, "compute_sparse_pca"
        ) as mock_sparse_pca:
            mock_result = {
            ...
            result = preprocessing_service.compute_sparse_pca(
                adata, n_comps=30, alpha=0.1
            )
    ...
    def test_pca_elbow_detection(self, preprocessing_service, mock_single_cell_data):
        ...
        elbow_result = preprocessing_service.find_pca_elbow(
            adata, max_comps=100, method="kneedle"
        )
    ...
    def test_batch_effect_pca(self, preprocessing_service, mock_batch_data):
        ...
        pca_result = preprocessing_service.compute_pca(
            adata, n_comps=50, batch_key="batch"
        )
    ...
    def test_harmony_integration(self, preprocessing_service, mock_batch_data):
        ...
        with patch.object(preprocessing_service, "harmony") as mock_harmony:
            ...
            result = preprocessing_service.harmony(
                adata, batch_key="batch", n_components=50
            )
...
class TestPreprocessingPipelines:
    ...
    def test_standard_single_cell_pipeline(
        self, preprocessing_service, mock_noisy_data
    ):
    ...
    def test_custom_pipeline(self, preprocessing_service, mock_single_cell_data):
        ...
        pipeline_result = preprocessing_service.run_pipeline(
            adata, pipeline="custom", steps=custom_steps
        )

        assert len(pipeline_result["steps_completed"]) == 4
        assert all(
            step["name"] in pipeline_result["steps_completed"] for step in custom_steps
        )

    def test_pipeline_with_batch_correction(
        self, preprocessing_service, mock_batch_data
    ):
    ...
    def test_pipeline_error_handling(
        ...
        with pytest.raises(ValueError):
            preprocessing_service.run_pipeline(
                adata, pipeline="custom", steps=failing_steps, stop_on_error=True
            )
...
class TestPreprocessingErrorHandling:
    ...
    def test_missing_batch_key_handling(
        self, preprocessing_service, mock_single_cell_data
    ):
    ...
    def test_insufficient_hvg_handling(self, preprocessing_service):
        ...
        low_var_data = ad.AnnData(
            X=np.ones((100, 50)) + np.random.normal(0, 0.001, (100, 50))
        )

        hvg_result = preprocessing_service.find_highly_variable_genes(
            low_var_data, n_top_genes=2000, method="seurat"  # More than available
        )
    ...
    def test_memory_efficient_processing(self, preprocessing_service):
        ...
        qc_result = preprocessing_service.calculate_qc_metrics(
            large_adata, memory_efficient=True, chunk_size=1000
        )
