...
from lobster.tools.visualization_service import (
    SingleCellVisualizationService as VisualizationService,  # FIXME ensure that the different visualization services are dynamically tested
)
...
class TestDimensionalityReductionPlots:
    ...
    def test_plot_umap_basic(
        self, visualization_service, mock_clustered_data, temp_plot_dir
    ):
        ...
        with patch("matplotlib.pyplot.savefig") as mock_savefig:
            plot_info = visualization_service.plot_umap(
                adata, color="leiden", save_path=str(temp_plot_dir / "umap_basic.png")
            )
    ...
    def test_plot_umap_continuous_color(
        self, visualization_service, mock_clustered_data
    ):
        ...
        with patch("matplotlib.pyplot.savefig") as mock_savefig:
            plot_info = visualization_service.plot_umap(
                adata, color="total_counts", color_map="viridis"
            )

            assert plot_info["color_by"] == "total_counts"
            assert plot_info["color_type"] == "continuous"

    def test_plot_umap_multiple_colors(
        self, visualization_service, mock_clustered_data
    ):
        ...
        with patch("matplotlib.pyplot.subplots") as mock_subplots:
            ...
            plot_info = visualization_service.plot_umap(
                adata, color=["leiden", "cell_type", "total_counts"], ncols=3
            )
    ...
    def test_plot_tsne(self, visualization_service, mock_clustered_data):
        ...
        with patch("matplotlib.pyplot.savefig") as mock_savefig:
            plot_info = visualization_service.plot_tsne(
                adata, color="cell_type", legend_loc="right"
            )
    ...
    def test_plot_pca(self, visualization_service, mock_clustered_data):
        ...
        with patch("matplotlib.pyplot.savefig") as mock_savefig:
            plot_info = visualization_service.plot_pca(
                adata, color="leiden", components=[1, 2]  # PC1 vs PC2
            )
    ...
    def test_plot_pca_variance(self, visualization_service, mock_clustered_data):
        ...
        with patch("matplotlib.pyplot.savefig") as mock_savefig:
            plot_info = visualization_service.plot_pca_variance(adata, n_components=20)

            assert plot_info["plot_type"] == "pca_variance"
            assert plot_info["n_components"] == 20

    def test_plot_embedding_comparison(
        self, visualization_service, mock_clustered_data
    ):
        ...
        with patch("matplotlib.pyplot.subplots") as mock_subplots:
            ...
            plot_info = visualization_service.plot_embedding_comparison(
                adata, embeddings=["X_pca", "X_umap", "X_tsne"], color="leiden"
            )
...
class TestStatisticalPlots:
    ...
    def test_plot_violin_gene_expression(
        self, visualization_service, mock_clustered_data
    ):
        ...
        with patch("matplotlib.pyplot.savefig") as mock_savefig:
            plot_info = visualization_service.plot_violin(
                adata, keys=genes, groupby="cell_type"
            )
    ...
    def test_plot_histogram_distribution(
        self, visualization_service, mock_clustered_data
    ):
        ...
        with patch("matplotlib.pyplot.savefig") as mock_savefig:
            plot_info = visualization_service.plot_histogram(
                adata, key="total_counts", bins=50, density=True
            )
    ...
    def test_plot_box_plot(self, visualization_service, mock_clustered_data):
        ...
        with patch("matplotlib.pyplot.savefig") as mock_savefig:
            plot_info = visualization_service.plot_box(
                adata, key="pct_counts_mt", groupby="cell_type"
            )
    ...
    def test_plot_correlation_matrix(self, visualization_service, mock_clustered_data):
        ...
        with patch("matplotlib.pyplot.savefig") as mock_savefig:
            plot_info = visualization_service.plot_correlation_matrix(
                adata, keys=qc_metrics, method="pearson"
            )
...
class TestGeneExpressionPlots:
    ...
    def test_plot_heatmap_marker_genes(
        self, visualization_service, mock_clustered_data
    ):
        ...
        with patch("matplotlib.pyplot.savefig") as mock_savefig:
            plot_info = visualization_service.plot_heatmap(
                adata, var_names=marker_genes, groupby="leiden", show_gene_labels=True
            )
    ...
    def test_plot_dotplot_marker_genes(
        self, visualization_service, mock_clustered_data
    ):
    ...
    def test_plot_stacked_violin(self, visualization_service, mock_clustered_data):
        ...
        with patch("matplotlib.pyplot.savefig") as mock_savefig:
            plot_info = visualization_service.plot_stacked_violin(
                adata, var_names=genes, groupby="leiden"
            )
    ...
    def test_plot_feature_plot(self, visualization_service, mock_clustered_data):
        ...
        with patch("matplotlib.pyplot.savefig") as mock_savefig:
            plot_info = visualization_service.plot_feature(
                adata, features=["Gene1", "Gene2"], embedding="X_umap", ncols=2
            )
...
class TestDifferentialExpressionPlots:
    ...
    def test_plot_de_heatmap(self, visualization_service, mock_bulk_data):
        ...
        with patch("matplotlib.pyplot.savefig") as mock_savefig:
            plot_info = visualization_service.plot_heatmap(
                adata, var_names=top_genes, groupby="condition", standard_scale="var"
            )
...
class TestInteractivePlots:
    ...
    def test_create_interactive_umap(self, visualization_service, mock_clustered_data):
        ...
        with patch.object(
            visualization_service, "create_interactive_plot"
        ) as mock_interactive:
    ...
    def test_create_interactive_heatmap(
        self, visualization_service, mock_clustered_data
    ):
        ...
        with patch.object(
            visualization_service, "create_interactive_heatmap"
        ) as mock_heatmap:
            ...
            interactive_heatmap = visualization_service.create_interactive_heatmap(
                adata, var_names=["Gene1", "Gene2", "Gene3"], groupby="leiden"
            )
    ...
    def test_export_interactive_plot(self, visualization_service, temp_plot_dir):
        ...
        with patch("builtins.open", mock_open()) as mock_file:
            export_info = visualization_service.export_interactive_plot(
                mock_plot_data, str(export_path)
            )
...
class TestPlotCustomization:
    ...
    def test_add_plot_annotations(self, visualization_service, mock_clustered_data):
        ...
        with patch("matplotlib.pyplot.savefig") as mock_savefig:
            plot_info = visualization_service.plot_umap(
                adata, color="cell_type", annotations=annotations
            )
...
class TestPlotExportFormats:
    ...
    def test_export_png_format(
        ...
        with patch("matplotlib.pyplot.savefig") as mock_savefig:
            export_info = visualization_service.plot_umap(
                adata, color="leiden", save_path=str(export_path), format="png", dpi=300
            )
    ...
    def test_export_pdf_format(
        ...
        with patch("matplotlib.pyplot.savefig") as mock_savefig:
            export_info = visualization_service.plot_umap(
                adata, color="leiden", save_path=str(export_path), format="pdf"
            )
    ...
    def test_export_svg_format(
        ...
        with patch("matplotlib.pyplot.savefig") as mock_savefig:
            export_info = visualization_service.plot_umap(
                adata, color="leiden", save_path=str(export_path), format="svg"
            )

            mock_savefig.assert_called()

    def test_batch_plot_export(
        self, visualization_service, mock_clustered_data, temp_plot_dir
    ):
    ...
    def test_plot_gallery_creation(
        ...
        with patch.object(visualization_service, "create_plot_gallery") as mock_gallery:
            ...
            gallery_info = visualization_service.create_plot_gallery(
                adata, output_dir=str(temp_plot_dir)
            )
...
class TestVisualizationErrorHandling:
    ...
    def test_missing_color_variable_handling(
        self, visualization_service, mock_clustered_data
    ):
    ...
    def test_invalid_plot_parameters_handling(
        self, visualization_service, mock_clustered_data
    ):
    ...
    def test_memory_efficient_plotting(self, visualization_service):
        ...
        with patch("matplotlib.pyplot.savefig") as mock_savefig:
            plot_info = visualization_service.plot_umap(
                adata, color="cluster", rasterized=True, alpha=0.6  # For large datasets
            )

            assert plot_info["rasterized"] == True
            assert plot_info["n_points"] == adata.n_obs

    def test_concurrent_plotting_safety(
        self, visualization_service, mock_clustered_data
    ):
        ...
        def plotting_worker(worker_id):
            ...
            try:
                ...
                with patch("matplotlib.pyplot.savefig") as mock_savefig:
                    plot_info = visualization_service.plot_umap(
                        adata, color="leiden", title=f"Plot from worker {worker_id}"
                    )
