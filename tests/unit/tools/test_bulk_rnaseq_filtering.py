...
class TestDEGFiltering:
    ...
    def test_filtering_statistics_tracked_correctly(self, bulk_service, simple_de_adata):
...
class TestIntegration:
    ...
    def test_de_adds_confidence_columns_to_adata(self, bulk_service, simple_de_adata):
        ...
        if de_stats['post_filter_gene_count'] > 0:
            ...
            assert 'gene_confidence' not in adata_de.var.columns or len(adata_de.var) == 0
...
class TestEdgeCases:
    ...
    def test_sparse_matrix_handling(self, bulk_service):
        ...
        np.random.seed(42)
        X_dense = np.random.rand(n_samples, n_genes) * 50 + 10  # Higher baseline expression
