...
def pseudobulk_adata():
    ...
    expression_matrix = np.random.negative_binomial(10, 0.3, (6, 100)).astype(
        int
    ...
    adata = ad.AnnData(
        ...
        obs=pd.DataFrame(
            ...
            index=[
                f"S{i//3+1}_{['T_cell', 'B_cell', 'Monocyte'][i%3]}" for i in range(6)
            ],
        ),
...
@pytest.fixture
def bulk_rnaseq_service(tmp_path):
    """Create BulkRNASeqService for testing."""
    return BulkRNASeqService(results_dir=tmp_path / "bulk_results")
...
class TestPyDESeq2InputValidation:
    ...
    def test_validate_deseq2_inputs_valid(
        self, bulk_rnaseq_service, count_matrix, sample_metadata
    ):
    ...
    def test_validate_deseq2_inputs_empty_count_matrix(
        self, bulk_rnaseq_service, sample_metadata
    ):
    ...
    def test_validate_deseq2_inputs_non_numeric(
        self, bulk_rnaseq_service, sample_metadata
    ):
        """Test validation with non-numeric count matrix."""
        non_numeric_matrix = pd.DataFrame(
            {"S1": ["high", "low", "medium"], "S2": ["low", "high", "low"]},
    ...
    def test_validate_deseq2_inputs_negative_values(
        self, bulk_rnaseq_service, sample_metadata
    ):
    ...
    def test_validate_deseq2_inputs_sample_mismatch(
        self, bulk_rnaseq_service, count_matrix
    ):
        """Test validation with sample mismatch between count matrix and metadata."""
        mismatched_metadata = pd.DataFrame(
            {"condition": ["Control", "Treatment"]}, index=["S1", "S99"]
        )  # S99 not in count matrix
    ...
    def test_validate_deseq2_inputs_invalid_contrast_format(
        self, bulk_rnaseq_service, count_matrix, sample_metadata
    ):
    ...
    def test_validate_deseq2_inputs_contrast_factor_not_found(
        self, bulk_rnaseq_service, count_matrix, sample_metadata
    ):
    ...
    def test_validate_deseq2_inputs_contrast_level_not_found(
        self, bulk_rnaseq_service, count_matrix, sample_metadata
    ):
...
class TestPyDESeq2AnalysisMethod:
    ...
    def test_run_pydeseq2_analysis_basic(
        ...
        # Mock all pydeseq2 modules in sys.modules to prevent real imports
        mock_dds_cls = MagicMock()
        mock_stats_cls = MagicMock()
        mock_inference_cls = MagicMock()

        mock_pydeseq2_dds = MagicMock()
        mock_pydeseq2_dds.DeseqDataSet = mock_dds_cls

        mock_pydeseq2_ds = MagicMock()
        mock_pydeseq2_ds.DeseqStats = mock_stats_cls

        mock_pydeseq2_inference = MagicMock()
        mock_pydeseq2_inference.DefaultInference = mock_inference_cls
        ...
        with patch.dict(
            ...
            {
                "pydeseq2": MagicMock(),
                "pydeseq2.dds": mock_pydeseq2_dds,
                "pydeseq2.ds": mock_pydeseq2_ds,
                "pydeseq2.default_inference": mock_pydeseq2_inference,
            },
        ):
            with patch.object(
                bulk_rnaseq_service,
                "validate_pydeseq2_setup",
                return_value={
                    "pydeseq2": True,
                ...
                mock_stats_cls.return_value = mock_ds_instance

                contrast = ["condition", "Treatment", "Control"]
                formula = "~condition"

                # run_pydeseq2_analysis returns 3-tuple: (results_df, stats, ir)
                results_df, stats, ir = bulk_rnaseq_service.run_pydeseq2_analysis(
                ...
                mock_dds_cls.assert_called_once()
                mock_stats_cls.assert_called_once()

    def test_run_pydeseq2_analysis_missing_dependencies(
        self, bulk_rnaseq_service, count_matrix, sample_metadata
    ):
    ...
    def test_run_pydeseq2_analysis_with_shrinkage(
        ...
        with patch.dict(
            ...
            with patch.object(
                ...
                mock_stats_cls.return_value = mock_ds
...
class TestPyDESeq2PseudobulkIntegration:
    ...
    def test_run_pydeseq2_from_pseudobulk_basic(
        self, bulk_rnaseq_service, pseudobulk_adata
    ):
        """Test running pyDESeq2 analysis on pseudobulk data."""
        with patch.object(
            bulk_rnaseq_service, "run_pydeseq2_analysis"
        ) as mock_pydeseq2:
            # Mock pyDESeq2 results (must return 3-tuple: results_df, stats, ir)
            mock_results_df = pd.DataFrame(
                {
                    "baseMean": np.random.lognormal(5, 2, 100),
                    "log2FoldChange": np.random.normal(0, 1.5, 100),
            ...
            mock_stats = {"method": "pydeseq2"}
            mock_ir = MagicMock()  # Mock AnalysisStep IR
            mock_pydeseq2.return_value = (mock_results_df, mock_stats, mock_ir)
    ...
    def test_run_pydeseq2_from_pseudobulk_with_layer(
        self, bulk_rnaseq_service, pseudobulk_adata
    ):
        ...
        with patch.object(
            ...
            mock_results = pd.DataFrame(
                {"log2FoldChange": [1.0, -1.5], "padj": [0.01, 0.05]},
                index=["Gene1", "Gene2"],
            )
            mock_stats = {"method": "pydeseq2"}
            mock_ir = MagicMock()
            mock_pydeseq2.return_value = (mock_results, mock_stats, mock_ir)
    ...
    def test_pydeseq2_from_pseudobulk_statistics(
        self, bulk_rnaseq_service, pseudobulk_adata
    ):
        ...
        with patch.object(
            ...
            assert (
                analysis_stats["n_upregulated"] == 1
            )  # Only Gene1 (significant and upregulated)
            assert (
                analysis_stats["n_downregulated"] == 2
            )  # Gene2, Gene4 (significant and downregulated)
...
class TestFormulaDesignIntegration:
    ...
    def test_create_formula_design_error_handling(
        self, bulk_rnaseq_service, sample_metadata
    ):
        """Test error handling in formula design creation."""
        with pytest.raises(FormulaError):
            bulk_rnaseq_service.create_formula_design(
                sample_metadata, condition_col="nonexistent_column"
            )
    ...
    def test_validate_experimental_design_error_handling(
        self, bulk_rnaseq_service, sample_metadata
    ):
...
class TestPyDESeq2ResultsEnhancement:
    ...
    def test_enhance_deseq2_results(self, bulk_rnaseq_service):
        ...
        assert (
            enhanced_results.loc["Gene1", "regulation"] == "upregulated"
        )  # LFC=2.0, significant
        assert (
            enhanced_results.loc["Gene2", "regulation"] == "downregulated"
        )  # LFC=-1.5, significant
        assert (
            enhanced_results.loc["Gene3", "regulation"] == "unchanged"
        )  # Not significant
...
class TestPyDESeq2ErrorHandling:
    ...
    def test_pydeseq2_analysis_error_wrapping(
        self, bulk_rnaseq_service, count_matrix, sample_metadata
    ):
        """Test that unexpected errors are wrapped in PyDESeq2Error."""
        with patch.object(
            bulk_rnaseq_service,
            "_validate_deseq2_inputs",
    ...
    def test_pydeseq2_specific_error_propagation(
        self, bulk_rnaseq_service, count_matrix, sample_metadata
    ):
    ...
    def test_pydeseq2_from_pseudobulk_error_handling(
        self, bulk_rnaseq_service, pseudobulk_adata
    ):
        ...
        with patch.object(
            ...
            with pytest.raises(
                PyDESeq2Error, match="pyDESeq2 pseudobulk analysis failed"
            ):
                bulk_rnaseq_service.run_pydeseq2_from_pseudobulk(
                    pseudobulk_adata,
                    "~condition",
...
class TestPyDESeq2Integration:
    ...
    def test_formula_service_integration(self, bulk_rnaseq_service, sample_metadata):
        ...
        assert isinstance(
            bulk_rnaseq_service.formula_service, DifferentialFormulaService
        )

    def test_design_matrix_integration(
        self, bulk_rnaseq_service, count_matrix, sample_metadata
    ):
    ...
    def test_count_matrix_orientation(
        self, bulk_rnaseq_service, count_matrix, sample_metadata
    ):
        ...
        with patch.dict(
            ...
            with patch.object(
                ...
                call_args = mock_dds_cls.call_args
