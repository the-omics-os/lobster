...
class TestConcatenationService:
    ...
    def test_concatenate_samples_success(
        self, mock_data_manager, sample_anndata_objects
    ):
        ...
        with patch.object(
            service.strategies[ConcatenationStrategy.SMART_SPARSE], "concatenate"
        ) as mock_concat:
    ...
    def test_concatenate_samples_failure(
        self, mock_data_manager, sample_anndata_objects
    ):
        ...
        with patch.object(
            ...
            mock_result = ConcatenationResult(success=False, error_message="Test error")
            mock_concat.return_value = mock_result

            with pytest.raises(ConcatenationError, match="Test error"):
                service.concatenate_samples(sample_anndata_objects)

    def test_concatenate_from_modalities_success(
        self, mock_data_manager, sample_anndata_objects
    ):
    ...
    def test_concatenate_from_modalities_missing_modalities(self, mock_data_manager):
        ...
        with pytest.raises(
            ConcatenationError, match="No valid modalities could be loaded"
        ):
            service.concatenate_from_modalities(
                modality_names=["missing_modality"], output_name="test_output"
            )

    def test_validate_concatenation_inputs(
        self, mock_data_manager, sample_anndata_objects
    ):
        ...
        with patch.object(
            service.strategies[ConcatenationStrategy.SMART_SPARSE], "validate"
        ) as mock_validate:
...
class TestSmartSparseStrategy:
    ...
    @pytest.mark.skipif(
        not pytest.importorskip("anndata", reason="anndata not available"),
        reason="anndata required",
    )
...
class TestMemoryEfficientStrategy:
    ...
    def test_concatenate_with_memory_monitoring(self, sample_anndata_objects):
        ...
        with patch("psutil.Process") as mock_process:
            ...
            result = strategy.concatenate(
                sample_anndata_objects, use_intersecting_genes_only=True
            )
...
class TestValidationFunctions:
    ...
    def test_validate_matrices_multithreaded(self, mock_data_manager):
        ...
        valid_matrix = pd.DataFrame(
            np.random.rand(100, 50), columns=[f"gene_{i}" for i in range(50)]
        )

        invalid_matrix = pd.DataFrame(
            np.random.rand(5, 5), columns=[f"gene_{i}" for i in range(5)]  # Too small
        )
    ...
    def test_is_valid_expression_matrix_non_numeric(self, mock_data_manager):
        ...
        # Non-numeric matrix
        text_matrix = pd.DataFrame([["a", "b", "c"], ["d", "e", "f"]])
...
class TestErrorHandling:
    ...
    def test_concatenation_error_with_invalid_strategy(
        self, mock_data_manager, sample_anndata_objects
    ):
...
class TestIntegration:
    ...
    def test_determine_adapter_from_data(self, mock_data_manager):
        ...
        assert (
            service._determine_adapter_from_data(mock_adata_sc)
            == "transcriptomics_single_cell"
        )
        assert (
            service._determine_adapter_from_data(mock_adata_bulk)
            == "transcriptomics_bulk"
        )
        assert (
            service._determine_adapter_from_data(mock_adata_proteomics)
            == "proteomics_ms"
        )
    ...
    def test_end_to_end_concatenation_workflow(
        self, mock_data_manager, sample_anndata_objects
    ):
        ...
        # Test validation
        validation_result = service.validate_concatenation_inputs(
            sample_anndata_objects
        )
