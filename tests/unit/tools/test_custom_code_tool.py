...
class TestCreateExecuteCustomCodeTool:
    ...
    def test_factory_with_agent_name(self, mock_data_manager, mock_service):
        ...
        assert "Custom code executed successfully" in result
    ...
    def test_mutual_exclusivity_validation(self, mock_data_manager, mock_service):
        ...
        assert "Error: Cannot specify both" in result
    ...
    def test_post_processor_called_on_success(self, mock_data_manager, mock_service):
        ...
        assert "Post-processed message!" in result

    def test_post_processor_not_called_on_failure(self, mock_data_manager, mock_service):
    ...
    def test_post_processor_error_handled(self, mock_data_manager, mock_service):
        ...
        assert "Custom code executed successfully" in result
        # Warning about post-processor failure
        assert "Post-processing failed" in result
    ...
    def test_response_formatting(self, mock_data_manager, mock_service):
        ...
        assert "Custom code executed successfully" in result
        assert "**Duration**: 2.50s" in result  # Format uses {:.2f}
        assert "Warnings (1):" in result
        assert "Import not in standard stack" in result
        assert "{'sample_count': 100}" in result
...
class TestMetadataStorePostProcessor:
    ...
    def test_in_place_update(self, mock_data_manager):
        ...
        assert mock_data_manager.metadata_store["existing_key"]["samples"] == result["samples"]
    ...
    def test_new_key_creation(self, mock_data_manager):
        ...
        msg = metadata_store_post_processor(
            result, {}, mock_data_manager, None, None
        )

        # Should create new key
        assert "filtered_samples" in mock_data_manager.metadata_store
        assert mock_data_manager.metadata_store["filtered_samples"]["samples"] == result["samples"]
        assert mock_data_manager.metadata_store["filtered_samples"]["filter_criteria"] == "body_site"
...
class TestIntegration:
    ...
    def test_metadata_assistant_workflow(self, mock_data_manager, mock_service):
        ...
        # Response should include both execution and persistence info
        assert "Custom code executed successfully" in result
        assert "Created metadata_store['filtered']" in result
        assert "write_to_workspace" in result
    ...
    def test_data_expert_workflow(self, mock_data_manager, mock_service):
        ...
        # Response should only include execution info
        assert "Custom code executed successfully" in result
        assert "3.5" in result
        assert "Persisted" not in result  # No persistence message
