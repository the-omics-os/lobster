...
class TestPMCIDResolution:
    ...
    def test_get_pmc_id_from_pmid_success(self, pmc_provider, sample_pmc_id_response):
...
class TestPMCXMLFetching:
    ...
    def test_fetch_full_text_xml_success(self, pmc_provider, sample_pmc_xml):
...
class TestPMCXMLParsing:
    ...
    def test_parse_pmc_xml_complete(self, pmc_provider, sample_pmc_xml):
        ...
        assert (
            len(result.software_tools) >= 0
        )  # May find tools or not depending on extraction

        # Verify GitHub repos
        assert len(result.github_repos) >= 1
        assert any(
            "github.com/example/scrnaseq-analysis" in repo
...
class TestIntegration:
    ...
    def test_complete_workflow_pmid_to_full_text(self, pmc_provider, sample_pmc_xml):
        """Test complete workflow: PMID → PMC ID → XML → Parsed content."""
        from unittest.mock import patch

        with (
            patch.object(pmc_provider, "get_pmc_id") as mock_get_id,
