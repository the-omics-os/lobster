...
class TestWaitForSlot:
    ...
    def test_timeout_when_limit_not_available(self, rate_limiter, fake_redis):
        ...
        # Prevent key from expiring during test by continuously resetting TTL
        key = "ratelimit:ncbi_esearch:user_timeout_test"
        fake_redis.persist(key)  # Remove TTL so key never expires
        fake_redis.set(key, str(rate_limiter.rate_limit))  # Ensure at limit

        start = time.time()
        success = rate_limiter.wait_for_slot(
            "ncbi_esearch",
            "user_timeout_test",
            max_wait=0.5,  # Should timeout
        )
        elapsed = time.time() - start

        assert success is False
        assert 0.4 <= elapsed <= 0.8  # Should timeout around 0.5s
...
class TestGetCurrentUsage:
    ...
    def test_accurate_usage_tracking(self, rate_limiter):
        ...
        # Make requests up to the rate limit (2 without NCBI_API_KEY, 9 with)
...
class TestConcurrentAccess:
    ...
    def test_concurrent_users_isolated(self, rate_limiter):
        ...
        limit = rate_limiter.rate_limit  # 2 without key, 9 with key
        ...
        for user_results in results:
            # All requests up to limit should be allowed since each user has independent limits
...
class TestMultiDomainRateLimiter:
    ...
    def test_get_rate_limit(self, multi_limiter):
        ...
        assert multi_limiter.get_rate_limit("pmc.ncbi.nlm.nih.gov") == 10.0
        assert multi_limiter.get_rate_limit("nature.com") == 3.0
        assert multi_limiter.get_rate_limit("default") == 3
    ...
    def test_check_rate_limit_blocks_over_limit(self, multi_limiter):
        ...
        domain = multi_limiter.detect_domain(url)
        rate_limit = int(multi_limiter.get_rate_limit(domain))
    ...
    def test_different_domains_independent(self, multi_limiter):
        ...
        url1 = "https://www.frontiersin.org/articles/test"
        url2 = "https://www.mdpi.com/articles/test"

        domain1 = multi_limiter.detect_domain(url1)
        rate_limit1 = int(multi_limiter.get_rate_limit(domain1))

        # Fill frontiers limit
        for _ in range(rate_limit1):
            multi_limiter.check_rate_limit(url1)
    ...
    def test_different_users_independent(self, multi_limiter):
        ...
        # Fill user1 limit
        for _ in range(rate_limit):
            multi_limiter.check_rate_limit(url, user_id="user1")
    ...
    def test_get_max_retries(self, multi_limiter):
        ...
        # All configured domains have rate >= 1.0 req/s, so they get 3 retries
        assert multi_limiter.get_max_retries("frontiersin.org") == 3
        assert multi_limiter.get_max_retries("eutils.ncbi.nlm.nih.gov") == 3
        assert multi_limiter.get_max_retries("nature.com") == 3
        assert multi_limiter.get_max_retries("default") == 3
